<div class="space-y-6">
    <div class="flex items-center justify-between gap-4 p-4 bg-white rounded-xl shadow-sm border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
        <div class="flex-1">
            <input 
                ...
                type="text" 
                placeholder="Поиск товаров или поставщиков (улучшено ИИ)..." 
        ...
        <div class="flex items-center gap-2 px-4 py-2 bg-primary-50 rounded-lg dark:bg-primary-900/20">
            <span class="text-sm font-medium text-primary-700 dark:text-primary-300">Лимит бюджета:</span>
            ...
            <div class="group relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-300 dark:bg-gray-800 dark:border-gray-700">
                @if($product->is_ai_featured)
                    <div class="absolute top-3 right-3 z-10">
                        <span class="px-2 py-1 text-[10px] font-bold uppercase tracking-wider bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full shadow-sm">Лучшее совпадение ИИ</span>
                    </div>
                @endif
                ...
                <div class="p-5">
                    ...
                    <div class="space-y-3 mb-6">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400">Оценка совпадения</span>
                            <span class="font-bold text-green-600">{{ $product->ai_match_score ?? '94' }}%</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400">Цена</span>
                            <span class="font-bold text-gray-900 dark:text-white">₽{{ number_format($product->base_wholesale_Цена, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                    ...
                    <div class="flex gap-2">
                        ...
                        >
                            <span wire:loading.remove wire:target="smartPurchase({{ $product->id }})">Умная покупка</span>
                            <span wire:loading wire:target="smartPurchase({{ $product->id }})">Обработка...</span>
                        </button>
                    </div>
                </div>
            </div>
    ...
    @if(count($recommendations) === 0)
        <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Совпадений не найдено</h3>
            <p class="text-gray-500 dark:text-gray-400">Попробуйте изменить фильтры или условия поиска.</p>
        </div>
    @endif
</div>
