<template>
  <div class="furniture-products">
    ...
    <div class="filters"><select v-model="categoryFilter"><option value="">All Categories</option><option value="sofas">Sofas</option><option value="tables">Tables</option><option value="chairs">Chairs</option></select></div>
...
const addProduct = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/furniture/products'); list.value = await res.json() })
</script>
<style scoped>
.furniture-products { padding: 20px }
