<template>
  <div class="toys-orders">
    <div class="header"><h2>Orders</h2><button @click="addOrder" class="btn-primary">Add</button></div>
    <div class="table"><table><thead><tr><th>ID</th><th>Customer</th><th>Items</th><th>Total</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="o in orders" :key="o.id"><td>#{{ o.id }}</td><td>{{ o.customer }}</td><td>{{ o.items }}</td><td>{{ o.total }}</td><td><span :class="['badge', o.status]">{{ o.status }}</span></td><td><button @click="view(o)" class="btn-sm">View</button></td></tr></tbody></table></div>
  </div>
</template>
...
const addOrder = () => {}
const view = (o: O) => {}
onMounted(async () => { const res = await fetch('/api/toys/orders'); orders.value = await res.json() })
</script>
<style scoped>
.toys-orders { padding: 20px }
