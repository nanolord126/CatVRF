<template>
  <div class="toys-products">
    ...
    <div class="filters"><select v-model="categoryFilter"><option value="">All Categories</option><option value="toys">Toys</option><option value="games">Games</option><option value="educational">Educational</option></select></div>
...
const addProduct = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/toys/products'); list.value = await res.json() })
</script>
<style scoped>
.toys-products { padding: 20px }
