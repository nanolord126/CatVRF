<template>
  <div class="office-orders">
    ...
    <div class="table"><table><thead><tr><th>ID</th><th>Company</th><th>Date</th><th>Total</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="o in filtered" :key="o.id"><td>#{{ o.id }}</td><td>{{ o.company }}</td><td>{{ o.date }}</td><td>{{ o.total }}</td><td><span :class="['badge', o.status]">{{ o.status }}</span></td><td><button @click="view(o)" class="btn-sm">View</button></td></tr></tbody></table></div>
  </div>
</template>
<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'
interface O { id: number; company: string; date: string; total: string; status: string }
const list = ref<O[]>([])
const statusFilter = ref('')
const filtered = computed(() => statusFilter.value ? list.value.filter(x => x.status === statusFilter.value) : list.value)
const addOrder = () => {}
const view = (o: O) => {}
onMounted(async () => { const res = await fetch('/api/officecatering/orders'); list.value = await res.json() })
</script>
<style scoped>
.office-orders { padding: 20px }
