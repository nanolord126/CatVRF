<template>
  <div class="sn-products">
    ...
    <div class="table"><table><thead><tr><th>ID</th><th>Name</th><th>Category</th><th>Price</th><th>Stock</th><th>Actions</th></tr></thead><tbody><tr v-for="p in products" :key="p.id"><td>#{{ p.id }}</td><td>{{ p.name }}</td><td>{{ p.category }}</td><td>{{ p.price }}</td><td>{{ p.stock }}</td><td><button @click="view(p)" class="btn-sm">View</button></td></tr></tbody></table></div>
  </div>
</template>
...
const addProduct = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/sportsnutrition/products'); products.value = await res.json() })
</script>
<style scoped>
.sn-products { padding: 20px }
