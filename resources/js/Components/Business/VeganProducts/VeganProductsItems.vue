<template>
  <div class="vegan-products">
    <div class="header"><h2>Products</h2><button @click="addProduct" class="btn-primary">Add</button></div>
    <div class="filters"><select v-model="categoryFilter"><option value="">All Categories</option><option value="food">Food</option><option value="cosmetics">Cosmetics</option><option value="clothing">Clothing</option></select></div>
...
const addProduct = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/vegan/products'); list.value = await res.json() })
</script>
<style scoped>
.vegan-products { padding: 20px }
