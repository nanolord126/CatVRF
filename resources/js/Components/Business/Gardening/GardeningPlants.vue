<template>
  <div class="gardening-plants">
    <div class="header"><h2>Plants</h2><button @click="addPlant" class="btn-primary">Add</button></div>
    <div class="filters"><select v-model="categoryFilter"><option value="">All Categories</option><option value="indoor">Indoor</option><option value="outdoor">Outdoor</option><option value="seeds">Seeds</option></select></div>
...
const addPlant = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/gardening/plants'); list.value = await res.json() })
</script>
<style scoped>
.gardening-plants { padding: 20px }
