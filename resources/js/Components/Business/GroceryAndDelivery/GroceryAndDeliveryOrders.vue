<template>
  <div class="grocery-orders">
    <div class="header"><h2>Orders</h2><button @click="addOrder" class="btn-primary">Add</button></div>
    <div class="filters"><select v-model="statusFilter"><option value="">All Status</option><option value="pending">Pending</option><option value="delivering">Delivering</option><option value="delivered">Delivered</option></select></div>
...
const addOrder = () => {}
const view = (o: O) => {}
onMounted(async () => { const res = await fetch('/api/grocery/orders'); list.value = await res.json() })
</script>
<style scoped>
.grocery-orders { padding: 20px }
