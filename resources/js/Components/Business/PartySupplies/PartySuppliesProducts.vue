<template>
  <div class="party-products">
    ...
    <div class="filters"><select v-model="categoryFilter"><option value="">All Categories</option><option value="decorations">Decorations</option><option value="tableware">Tableware</option><option value="balloons">Balloons</option></select></div>
...
const addProduct = () => {}
const view = (p: P) => {}
onMounted(async () => { const res = await fetch('/api/partysupplies/products'); list.value = await res.json() })
</script>
<style scoped>
.party-products { padding: 20px }
