...
use App\Http\Controllers\Account\ComplianceController;
use App\Http\Controllers\MeshController;
use App\Http\Controllers\StreamController;
use App\Livewire\User\AIConstructor;
use App\Livewire\User\Addresses;
use App\Livewire\User\Dashboard;
use App\Livewire\User\DeliveryTrack;
...
Route::prefix('admin/marketplace/beauty')->group(function (): void {
    $renderBeautyPage = static function (string $title, string $tableTestId): Response|RedirectResponse {
        if (! auth()->check()) {
            return redirect('/login');
        }
...
Route::middleware(['auth'])->prefix('account/integrations')->group(function () {
    Route::get('/', [ComplianceController::class, 'index'])->name('account.integrations.index');
    Route::post('/{type}/test', [ComplianceController::class, 'test'])->name('account.integrations.test');
    Route::post('/{type}/connect', [ComplianceController::class, 'connect'])->name('account.integrations.connect');
    Route::delete('/{type}', [ComplianceController::class, 'disconnect'])->name('account.integrations.disconnect');
});

// WEBRTC LIVE STREAMING MESH ROUTES
// ══════════════════════════════════════════════════════════════════════════════
Route::middleware(['auth', 'tenant'])->prefix('mesh')->group(function () {
    Route::post('/join', [MeshController::class, 'join'])->name('mesh.join');
    Route::post('/offer', [MeshController::class, 'offer'])->name('mesh.offer');
    Route::post('/answer', [MeshController::class, 'answer'])->name('mesh.answer');
    Route::post('/ice-candidate', [MeshController::class, 'iceCandidate'])->name('mesh.ice-candidate');
    Route::post('/connected', [MeshController::class, 'connected'])->name('mesh.connected');
    Route::post('/failed', [MeshController::class, 'failed'])->name('mesh.failed');
});
...
Route::middleware(['auth', 'verified'])->prefix('cabinet')->name('user.')->group(function () {
    ...
    Route::get('/ai-constructor', AIConstructor::class)->name('ai-constructor');
    Route::get('/addresses', Addresses::class)->name('addresses');
    Route::get('/delivery', DeliveryTrack::class)->name('delivery-track');
});

// ══════════════════════════════════════════════════════════════════════════════
// FITNESS TRAINER DASHBOARD
// Livewire component for trainer personal dashboard
// ══════════════════════════════════════════════════════════════════════════════
// TODO: TrainerDashboard class not found - need to create or fix this route
// Route::middleware(['auth', 'verified'])->prefix('fitness/trainer')->name('fitness.trainer.')->group(function () {
//     Route::get('/dashboard', TrainerDashboard::class)->name('dashboard');
// });

// ══════════════════════════════════════════════════════════════════════════════
// SUPERMARKET BUYER PAGES
// Vue/Inertia pages for Supermarket buyer dashboard and subscriptions
// ══════════════════════════════════════════════════════════════════════════════
Route::middleware(['auth', 'verified', 'tenant'])->prefix('supermarket')->name('supermarket.')->group(function () {
    Route::get('/dashboard', function () {
        return inertia('SupermarketBuyerDashboard');
    })->name('dashboard');
    
    Route::prefix('subscriptions')->group(function () {
        Route::get('/', function () {
            return inertia('SupermarketSubscriptionList');
        })->name('subscriptions');
        
        Route::get('/create', function () {
            return inertia('SupermarketSubscriptionCreate');
        })->name('subscriptions.create');
        
        Route::get('/{id}', function ($id) {
            return inertia('SupermarketSubscriptionDetail', ['id' => $id]);
