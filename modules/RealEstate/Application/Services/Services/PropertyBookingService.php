...
final class PropertyBookingService
{
    ...
    public function createBooking(array $data): PropertyBooking
    {
        ...
        try {
            $this->logAction(
                ...
                entityType: 'real_estate_booking',
                context: [
            ...
            if ($fraudScore > 0.7) {
                $this->log->channel('fraud')->warning('High fraud score detected for booking', [
                    'correlation_id' => $correlationId,
            ...
            $lockAcquired = $this->redis->set($lockKey, '1', 'EX', self::BOOKING_LOCK_TTL, 'NX');
            ...
            try {
                $booking = $this->db->transaction(function () use ($data, $property, $fraudScore, $holdMinutes, $isB2B, $tenantId, $businessGroupId, $correlationId) {
                    ...
                    $this->cache->tags(['realestate', 'property', "property:{$data['property_id']}", "tenant:{$tenantId}"])->forget("property_availability:{$data['property_id']}");

                    return $booking;
                });

                $this->log->channel('audit')->info('real_estate.booking.create.success', [
                    'correlation_id' => $correlationId,
            ...
            $this->log->channel('audit')->critical('real_estate.booking.create.error', [
                'correlation_id' => $correlationId,
                'error' => $e->getMessage(),
    ...
    public function confirmBooking(int $bookingId, ?string $correlationId = null): PropertyBooking
    {
        ...
        try {
            $this->log->channel('audit')->info('real_estate.booking.confirm.start', [
                'correlation_id' => $correlationId,
            ...
            $booking = $this->db->transaction(function () use ($booking, $correlationId) {
                ...
                $this->log->channel('audit')->info('real_estate.booking.confirm.success', [
                    'correlation_id' => $correlationId,
                    'booking_id' => $booking->id,
                ...
                return PropertyBooking::findOrFail($booking->id);
            });

            return $booking;
        } catch (\Throwable $e) {
            $this->log->channel('audit')->critical('real_estate.booking.confirm.error', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
    ...
    public function completeDeal(int $bookingId, ?string $correlationId = null): PropertyBooking
    {
        ...
        try {
            $this->log->channel('audit')->info('real_estate.deal.complete.start', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
            ]);
            ...
            $booking = $this->db->transaction(function () use ($booking, $correlationId) {
                ...
                $this->cache->tags(['realestate', 'property', "property:{$property->id}", "tenant:{$property->tenant_id}"])->forget("property_availability:{$property->id}");

                $this->log->channel('audit')->info('real_estate.deal.complete.success', [
                    'correlation_id' => $correlationId,
                    'booking_id' => $booking->id,
            ...
            $this->log->channel('audit')->critical('real_estate.deal.complete.error', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
    ...
    public function cancelBooking(int $bookingId, string $reason, ?string $correlationId = null): PropertyBooking
    {
        ...
        try {
            $this->log->channel('audit')->info('real_estate.booking.cancel.start', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
                'reason' => $reason,
            ]);
            ...
            $booking = $this->db->transaction(function () use ($booking, $reason, $correlationId) {
                if ($booking->escrow_amount > 0 && isset($booking->metadata['wallet_id'])) {
                ...
                $this->cache->tags(['realestate', 'property', "property:{$booking->property_id}", "tenant:{$booking->tenant_id}"])->forget("property_availability:{$booking->property_id}");

                $this->log->channel('audit')->info('real_estate.booking.cancel.success', [
                    'correlation_id' => $correlationId,
                    'booking_id' => $booking->id,
                ]);
            ...
            $this->log->channel('audit')->critical('real_estate.booking.cancel.error', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
    ...
    public function initiateVideoCall(int $bookingId, int $initiatorId, ?string $correlationId = null): array
    {
        ...
        try {
            ...
            $this->log->channel('audit')->info('real_estate.video_call.initiated', [
                'correlation_id' => $correlationId,
                'booking_id' => $booking->id,
            ...
            $this->log->channel('audit')->critical('real_estate.video_call.error', [
                'correlation_id' => $correlationId,
                'booking_id' => $bookingId,
                'error' => $e->getMessage(),
            ]);
    ...
    public function getAvailableSlots(int $propertyId, string $startDate, string $endDate, ?string $correlationId = null): array
    {
        ...
        return $this->cache->tags(['realestate', 'property', "property:{$propertyId}", "slots:{$propertyId}:{$startDate}:{$endDate}"])
            ->remember($cacheKey, self::CACHE_TTL_SECONDS, function () use ($propertyId, $startDate, $endDate, $correlationId) {
            ...
            $this->log->channel('audit')->info('real_estate.slots.retrieved', [
                'correlation_id' => $correlationId,
    ...
    private function calculateDynamicPrice(Property $property, string $viewingSlot, bool $isB2B): float
    {
        ...
        $demandMultiplier = $this->predictDemand($property->id, Carbon::parse($viewingSlot));
        ...
        if ($demandMultiplier > 1.3 && Carbon::parse($viewingSlot)->diffInHours(now()) < 24) {
    ...
    {
        $this->log->channel('audit')->info('real_estate.escrow.hold', [
            'correlation_id' => $correlationId,
            'wallet_id' => $walletId,
            'amount' => $amount,
            'booking_id' => $bookingId,
    ...
    {
        $this->log->channel('audit')->info('real_estate.escrow.capture', [
            'correlation_id' => $correlationId,
            'wallet_id' => $walletId,
            'amount' => $amount,
            'booking_id' => $bookingId,
    ...
    {
        $this->log->channel('audit')->info('real_estate.escrow.release', [
            'correlation_id' => $correlationId,
            'wallet_id' => $walletId,
            'amount' => $amount,
            'booking_id' => $bookingId,
    ...
    {
        $this->log->channel('audit')->info('real_estate.payment.transfer', [
            'correlation_id' => $correlationId,
    ...
    private function verifyDocumentsOnBlockchain(PropertyBooking $booking, Property $property): string
    {
        ...
        $this->log->channel('audit')->info('real_estate.blockchain.verify', [
            'booking_id' => $booking->id,
            'document_hash' => $documentHash,
        ]);
    ...
    private function createWebRTCRoom(PropertyBooking $booking): string
    {
        ...
        $this->log->channel('audit')->info('real_estate.webrtc.room.create', [
            'booking_id' => $booking->id,
            'room_id' => $roomId,
        ]);
    ...
    private function generateWebRTCToken(string $roomId, int $userId): string
    {
        ...
        $this->log->channel('audit')->info('real_estate.webrtc.token.generate', [
            'room_id' => $roomId,
            'user_id' => $userId,
        ]);
    ...
    {
        $this->log->channel('audit')->info('real_estate.crm.sync', [
            'booking_id' => $booking->id,
    ...
    {
        $this->log->channel('audit')->info('real_estate.crm.status.update', [
            'booking_id' => $booking->id,
            'status' => $status->value,
        ]);
    }
    ...
    {
        $this->log->channel('audit')->info('real_estate.crm.deal.complete', [
            'booking_id' => $booking->id,
            'property_id' => $property->id,
            'amount' => $booking->amount,
        ]);
    ...
    {
        $this->log->channel('audit')->info('real_estate.crm.cancellation', [
            'booking_id' => $booking->id,
            'reason' => $reason,
        ]);
    }

    private function predictDemand(int $propertyId, Carbon $slot): float
    {
        $hour = $slot->hour;
