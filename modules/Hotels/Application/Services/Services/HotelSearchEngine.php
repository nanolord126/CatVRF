...
final class HotelSearchEngine
{
    ...
    public function search(array $filters): Builder
    {
        ...
        if (isset($filters['has_fishing'])) {
            $query->where('has_fishing', true);
        }
        if (isset($filters['has_zoo'])) {
            $query->where('has_zoo', true);
        }
        if (isset($filters['has_forest_access'])) {
            $query->where('has_forest_access', true);
        }

        // 3a-2. Инфраструктура территории
        if (isset($filters['has_gym'])) {
            $query->where('has_gym', true);
        }
        if (isset($filters['has_pool'])) {
            $query->where('has_pool', true);
        }
        if (isset($filters['has_spa'])) {
            $query->where('has_spa', true);
        }
        if (isset($filters['has_restaurant'])) {
            $query->where('has_restaurant', true);
        }
        if (isset($filters['has_shop'])) {
            $query->where('has_shop', true);
        }
        if (isset($filters['has_flowers_shop'])) {
            $query->where('has_flowers_shop', true);
        }
        ...
        if (isset($filters['price_min']) || isset($filters['price_max']) || isset($filters['capacity']) || isset($filters['sq_meters_min'])) {
            $query->whereHas('rooms', function (Builder $q) use ($filters) {
                ...
                if (isset($filters['sq_meters_min'])) {
                    $q->where('square_meters', '>=', $filters['sq_meters_min']);
                }

                // Фильтры удобств в номере
                if (isset($filters['room_balcony'])) {
                    $q->where('has_balcony', true);
                }
                if (isset($filters['room_kitchen'])) {
                    $q->where('has_kitchen', true);
                }
                if (isset($filters['room_ac'])) {
                    $q->where('has_air_con', true);
                }
