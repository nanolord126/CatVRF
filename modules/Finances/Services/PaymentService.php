...
class PaymentService
{
    ...
    {
        $amount = (float) ($order->total_amount ?? $order->subtotal ?? 0);
        
        // Согласно Канону 2026: Расчет сплитов через CommissionCalculator (Fix 14% / Special Offer)
        ...
        return $this->processSplitPayment([
            ...
            'splits' => [
                $splits['seller']['user_id'] => $splits['seller']['amount'],
                'platform' => $splits['platform']['amount']
            ],
            'metadata' => [
                'order_type' => get_class($order),
                'order_id' => $order->id,
                'rate' => $order->metadata['is_special_offer'] ? 0.10 : 0.14
            ]
        ]);
    }
