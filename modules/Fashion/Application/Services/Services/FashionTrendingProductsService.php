...
final readonly class FashionTrendingProductsService
{
    ...
    public function getTrendingProducts(int $tenantId, int $limit = 20, string $period = '7d'): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($tenantId, $limit, $period) {
            ...
            $products = $this->db->table('fashion_products')
                ...
                ->select(
                    ...
                    $this->db->raw('COALESCE(fashion_product_analytics.views, 0) as views')
                    $this->db->raw('COALESCE(fashion_product_analytics.add_to_cart, 0) as add_to_cart'),
                    $this->db->raw('COALESCE(fashion_product_analytics.purchases, 0) as purchases'),
                    $this->db->raw('COALESCE(fashion_trend_scores.score, 0) as trend_score'),
                    $this->db->raw('COALESCE(fashion_social_mentions.mentions_count, 0) as social_mentions')
                )
    ...
    public function getTrendingProductsByCategory(int $tenantId, int $categoryId, int $limit = 10): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($tenantId, $categoryId, $limit) {
            $products = $this->db->table('fashion_products')
    ...
    public function updateTrendScores(int $tenantId): void
    {
        ...
        $this->cache->tags(["fashion_trending:{$tenantId}"])->flush();

        $this->log->info('Fashion trend scores updated', [
            'tenant_id' => $tenantId,
    ...
    {
        $analytics = $this->db->table('fashion_product_analytics')
