...
final readonly class FashionSocialMediaIntegrationService
{
    ...
    public function getSocialMetrics(int $productId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($productId, $tenantId) {
            $mentions = $this->db->table('fashion_social_mentions')
    ...
    public function getTrendingHashtags(int $tenantId, int $limit = 20): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addHours(6), function () use ($tenantId, $limit) {
            $hashtags = $this->db->table('fashion_trend_keywords')
                ->where('tenant_id', $tenantId)
    ...
    {
        try {
            $this->db->table('fashion_influencer_mentions')->insert([
                'id' => uniqid(),
            ...
            $this->log->info('Influencer mention tracked', [
            ...
            $this->log->error('Failed to track influencer mention', [
                'product_id' => $productId,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        return $this->db->table('fashion_influencers')
    ...
    private function syncToInstagram(object $product, int $tenantId): array
    {
        try {
            ...
            $this->log->info('Product synced to Instagram', [
                'product_id' => $product->id,
                'name' => $product->name,
                'tenant_id' => $tenantId,
            ]);

            $this->db->table('fashion_social_mentions')->updateOrInsert(
                ['fashion_product_id' => $product->id, 'platform' => 'instagram', 'tenant_id' => $tenantId],
                [
                    'post_url' => 'https://instagram.com/p/placeholder',
                    'mentions_count' => $this->db->raw('COALESCE(mentions_count, 0) + 1'),
            ...
            $this->log->error('Failed to sync to Instagram', [
                'product_id' => $product->id,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        try {
            $this->log->info('Product synced to TikTok', [
                'product_id' => $product->id,
                'name' => $product->name,
                'tenant_id' => $tenantId,
            ]);
            ...
            $this->log->error('Failed to sync to TikTok', [
                'product_id' => $product->id,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        try {
            $this->log->info('Product synced to Pinterest', [
                'product_id' => $product->id,
                'name' => $product->name,
                'tenant_id' => $tenantId,
            ]);
            ...
            $this->log->error('Failed to sync to Pinterest', [
                'product_id' => $product->id,
                'error' => $e->getMessage(),
            ]);
