...
final readonly class FashionColorHarmonyService
{
    ...
    public function getRecommendationsFromBeautyHistory(int $userId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($userId, $tenantId) {
            ...
            $this->log->info('Color harmony recommendations generated', [
                'user_id' => $userId,
                'tenant_id' => $tenantId,
    ...
    public function getPhotoSessionOutfitSuggestions(int $userId, int $tenantId, string $occasion = 'general'): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($userId, $tenantId, $occasion) {
            ...
            $this->log->info('Photo session outfit suggestions generated', [
                'user_id' => $userId,
                'tenant_id' => $tenantId,
                'occasion' => $occasion,
    ...
    private function getBeautyHistory(int $userId, int $tenantId): array
    {
        try {
            ...
            $appointments = $this->db->table('beauty_appointments')
            ...
            $this->log->error('Failed to get beauty history', [
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
        ...
        foreach (self::COLOR_HARMONY_RULES as $type => $rules) {
            ...
            foreach ($colors as $color) {
                if (isset($rules[$color])) {
                    ...
                    if (in_array($targetColor, $ruleColors, true)) {
                        return $type;
                    }
                }
            }
    ...
    private function getUserStylePreferences(int $userId, int $tenantId): array
    {
        return $this->db->table('fashion_order_items')
            ...
            ->select(
                ...
                $this->db->raw('COUNT(*) as purchase_count')
            )
    ...
    private function buildOutfitFromTemplate(array $template, array $colorRecs, array $preferences, array $trending, int $tenantId): ?array
    {
        ...
        foreach ($template as $item) {
            ...
            if ($product && ($item['required'] || ! $item['required'])) {
                ...
                throw new \RuntimeException("Required outfit item not found: {$item['type']}");
            }
        }
    ...
    private function selectProductForOutfit(string $type, array $colorRecs, array $preferences, array $trending, array $usedColors, int $tenantId): ?array
    {
        ...
        foreach ($colorRecs as $rec) {
            foreach ($rec['products'] as $product) {
                if ($this->matchesType($product, $type) && ! in_array($product['color'], $usedColors, true)) {
        ...
        foreach ($trending as $trend) {
            if ($this->matchesType($trend, $type) && ! in_array($trend['color'], $usedColors, true)) {
