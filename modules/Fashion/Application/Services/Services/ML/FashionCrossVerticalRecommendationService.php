...
final readonly class FashionCrossVerticalRecommendationService
{
    ...
    public function getBeautyToFashionRecommendations(int $userId, int $tenantId, int $limit = 10): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($userId, $tenantId, $limit) {
            ...
            $this->log->info('Cross-vertical recommendations generated', [
                'user_id' => $userId,
                'tenant_id' => $tenantId,
    ...
    public function getWardrobeUpdateSuggestions(int $userId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($userId, $tenantId) {
            $photoSession = $this->db->table('beauty_appointments')
            ...
            $this->log->info('Wardrobe update suggestions generated', [
                'user_id' => $userId,
                'tenant_id' => $tenantId,
    ...
    {
        try {
            $services = $this->db->table('beauty_appointments')
                ->join('beauty_services', 'beauty_appointments.service_id', '=', 'beauty_services.id')
                ->where('beauty_appointments.user_id', $userId)
                ->where('beauty_appointments.tenant_id', $tenantId)
            ...
            $this->log->error('Failed to get beauty services', [
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $userCategories = $this->db->table('fashion_order_items')
