...
final readonly class FashionSearchService
{
    ...
    public function searchProducts(array $filters, int $tenantId, int $page = 1, int $perPage = 20): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($filters, $tenantId, $page, $perPage) {
    ...
    public function getSearchSuggestions(string $query, int $tenantId, int $limit = 10): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($query, $tenantId, $limit) {
    ...
    public function getPopularSearchTerms(int $tenantId, int $limit = 10): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addHours(6), function () use ($tenantId, $limit) {
            return $this->db->table('fashion_search_logs')
                ->where('tenant_id', $tenantId)
                ->where('created_at', '>=', Carbon::now()->subDays(7))
                ->select('query', $this->db->raw('COUNT(*) as search_count'))
                ->groupBy('query')
    ...
    {
        try {
            $this->db->table('fashion_search_logs')->insert([
                'query' => $query,
                'user_id' => $userId,
                'tenant_id' => $tenantId,
            ...
            $this->log->error('Failed to log search', [
                'query' => $query,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    public function getSearchFilters(int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addHours(24), function () use ($tenantId) {
            return [
                'categories' => $this->db->table('fashion_categories')
                    ->where('tenant_id', $tenantId)
                    ->select('id', 'name')
                    ->orderBy('name')
                    ->get()
                ...
                'brands' => $this->db->table('fashion_products')
                    ...
                    ->get()
                    ->toArray(),
                'sizes' => $this->db->table('fashion_products')
                    ->where('tenant_id', $tenantId)
                    ->select('size')
                    ->distinct()
                    ->orderBy('size')
                    ...
                    ->pluck('size')
                    ->toArray(),
                'colors' => $this->db->table('fashion_products')
                    ->where('tenant_id', $tenantId)
                    ->whereNotNull('color')
                    ->select('color')
                    ->distinct()
                    ...
                    ->toArray(),
                'price_range' => [
                    'min' => $this->db->table('fashion_products')
                        ->where('tenant_id', $tenantId)
                        ->min('price_b2c') ?? 0,
                    'max' => $this->db->table('fashion_products')
                        ->where('tenant_id', $tenantId)
                        ->max('price_b2c') ?? 0,
                ],
            ];
    ...
    public function visualSearch(string $imageUrl, int $tenantId, int $limit = 10): array
    {
        try {
            ...
            $this->log->info('Visual search performed', [
                'image_url' => $imageUrl,
                'tenant_id' => $tenantId,
            ...
            $this->log->error('Visual search failed', [
                'image_url' => $imageUrl,
                'tenant_id' => $tenantId,
                'error' => $e->getMessage(),
            ]);
    ...
    public function getSimilarProducts(int $productId, int $tenantId, int $limit = 6): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($productId, $tenantId, $limit) {
