...
use Modules\Payment\Domain\ValueObjects\GatewayScore;
use Modules\Payment\Domain\ValueObjects\Money;
use Modules\Payment\Domain\ValueObjects\PaymentMethod;
...
final readonly class SmartRoutingService
{
    ...
    public function selectGateway(
        ...
        if ($preferredProvider && $availableGateways->contains($preferredProvider) && $this->isGatewayHealthy($preferredProvider)) {
            ...
            return GatewayScore::create($preferredProvider, 100, 'preferred_provider');
        }
        ...
        return GatewayScore::create($best['provider'], $best['score'], 'smart_selection');
    }
     ...
     */
    private function getAvailableGateways(PaymentMethod $paymentMethod): Collection
    {
        $gatewaysByMethod = [
        ...
        $gateways = collect($gatewaysByMethod[$paymentMethod->value] ?? ['tinkoff']);
    ...
    private function scoreByAmount(string $provider, Money $amount): float
    {
    ...
    private function scoreByPaymentMethod(string $provider, PaymentMethod $paymentMethod): float
    {
        $support = [
        ...
        if (in_array($paymentMethod->value, $support[$provider] ?? [], true)) {
            return 20.0;
        }
    ...
    private function recordMetric(string $provider, string $metric, float $value): void
    {
        ...
        $this->redis->connection()->expire($key, 86400);
    }
