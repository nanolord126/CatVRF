...
final class AnalyticsVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Analytics'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Analytics', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Analytics', ['X-Correlation-ID' => 'test-Analytics']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Analytics');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Analytics/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Analytics');
        }
