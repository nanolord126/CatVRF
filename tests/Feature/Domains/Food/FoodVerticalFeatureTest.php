...
final class FoodVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Food'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Food', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Food', ['X-Correlation-ID' => 'test-Food']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Food');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Food/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Food');
        }
