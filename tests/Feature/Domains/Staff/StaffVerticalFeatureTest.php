...
final class StaffVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Staff'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Staff', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Staff', ['X-Correlation-ID' => 'test-Staff']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Staff');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Staff/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Staff');
        }
