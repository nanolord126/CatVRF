...
final class InventoryVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Inventory'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Inventory', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Inventory', ['X-Correlation-ID' => 'test-Inventory']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Inventory');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Inventory/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Inventory');
        }
