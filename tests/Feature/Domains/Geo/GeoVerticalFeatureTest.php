...
namespace Tests\Feature\Domains\Geo;

use Tests\TestCase;
use Illuminate\Foundation\Testing\RefreshDatabase;

final class GeoVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Geo'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Geo', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Geo', ['X-Correlation-ID' => 'test-Geo']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Geo');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Geo/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Geo');
        }
