...
final class OfficeCateringVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/OfficeCatering'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_OfficeCatering', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_OfficeCatering', ['X-Correlation-ID' => 'test-OfficeCatering']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-OfficeCatering');
    }
    ...
    {
        $aiPath = base_path('app/Domains/OfficeCatering/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали OfficeCatering');
        }
