...
final class HotelsVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Hotels'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Hotels', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Hotels', ['X-Correlation-ID' => 'test-Hotels']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Hotels');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Hotels/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Hotels');
        }
