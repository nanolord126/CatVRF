...
final class AIVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/AI'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_AI', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_AI', ['X-Correlation-ID' => 'test-AI']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-AI');
    }
    ...
    {
        $aiPath = base_path('app/Domains/AI/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали AI');
        }
