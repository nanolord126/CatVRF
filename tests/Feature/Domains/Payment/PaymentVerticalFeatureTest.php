...
final class PaymentVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Payment'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Payment', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Payment', ['X-Correlation-ID' => 'test-Payment']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Payment');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Payment/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Payment');
        }
