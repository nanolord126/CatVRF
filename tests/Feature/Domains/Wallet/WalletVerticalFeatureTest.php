...
namespace Tests\Feature\Domains\Wallet;

use Tests\TestCase;
use Illuminate\Foundation\Testing\RefreshDatabase;

final class WalletVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Wallet'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Wallet', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Wallet', ['X-Correlation-ID' => 'test-Wallet']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Wallet');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Wallet/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Wallet');
        }
