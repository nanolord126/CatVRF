...
final class FurnitureVerticalFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_domain_directory_exists(): void
    {
        self::assertDirectoryExists(base_path('app/Domains/Furniture'));
    }
    ...
    public function test_correlation_id_header_is_supported(): void
    {
        ...
        try {
            \Illuminate\Support\Facades\Route::get('/__health_Furniture', static fn () => response()->json([
                'ok' => true,
        ...
        $response = $this->getJson('/__health_Furniture', ['X-Correlation-ID' => 'test-Furniture']);
        $response->assertStatus(200)->assertJsonPath('correlation_id', 'test-Furniture');
    }
    ...
    {
        $aiPath = base_path('app/Domains/Furniture/Services/AI');
        if (!is_dir($aiPath)) {
            $this->markTestSkipped('AI директория пока отсутствует для вертикали Furniture');
        }
