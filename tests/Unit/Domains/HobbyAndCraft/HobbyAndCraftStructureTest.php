...
/**
 * =================================================================
 *  HobbyAndCraft Domain — STRUCTURAL COMPLIANCE TEST
 *  CANON CatVRF 2026: 9-слойная архитектура.
 * =================================================================
 */
final class HobbyAndCraftStructureTest extends TestCase
{
    private const BASE = __DIR__ . '/../../../../app/Domains/HobbyAndCraft';
    ...
    public function layer1_model_files_present(): void
    {
        ...
        self::assertNotEmpty($models, 'HobbyAndCraft: Models directory must contain files');
    }
    ...
    public function layer2_dto_files_present(): void
    {
        ...
        self::assertNotEmpty($dtos, 'HobbyAndCraft: DTOs directory must contain files');
    }
    ...
    public function layer3_service_files_present(): void
    {
        ...
        self::assertNotEmpty($services, 'HobbyAndCraft: Services directory must contain files');
    }
    ...
    public function layer3_ai_constructor_exists(): void
    {
        self::assertDirectoryExists(
            ...
            'HobbyAndCraft: AI-constructor directory is PRODUCTION MANDATORY',
        );

        $aiFiles = glob(self::BASE . '/Services/AI/*.php');
        self::assertNotEmpty($aiFiles, 'HobbyAndCraft: Must have AI constructor service');
    }
    ...
    public function all_php_files_have_strict_types(): void
    {
        ...
        self::assertEmpty(
            $violations,
            'HobbyAndCraft: strict_types=1 violations: ' . implode(', ', array_slice($violations, 0, 5)),
        );
    }
        ...
        foreach ($files as $file) {
            ...
            foreach ($forbidden as $facade) {
                self::assertStringNotContainsString(
                    $facade,
                    $content,
                    "HobbyAndCraft/Services/{$name}: must not use {$facade}",
                );
            }
        }
    }
        ...
        foreach ($files as $file) {
            ...
            if (str_contains($content, 'class ') && !str_contains($content, 'interface ')) {
                self::assertTrue(
                    ...
                    "HobbyAndCraft/Services/{$name}: must use constructor injection",
                );
            }
        }
    }
    ...
    public function minimum_file_length_60_lines(): void
    {
        ...
        self::assertEmpty(
            $shortFiles,
            'HobbyAndCraft: Files under 60 lines: ' . implode(', ', array_slice($shortFiles, 0, 5)),
        );
    }
}
