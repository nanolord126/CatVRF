    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (RealEstate B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.budget_range" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="более 20 000 000 ₽">более 20 000 000 ₽</option>
          </select>
        </div>
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="commercial_lease">Коммерческая аренда</option>
            <option value="property_management">Управление недвижимостью</option>
            <option value="investment">Инвестиции</option>
            <option value="development">Застройка</option>
          </select>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" :disabled="submitting" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50">
...
import { ref } from 'vue'
import realestateB2bApi from '@/services/realestateB2bApi'
...
const form = ref({
  ...
  requirement: '',
  budget_range: '',
  category: 'commercial_lease' as const,
  source: '',
  notes: ''
})
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await realestateB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
