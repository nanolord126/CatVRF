    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (Logistics B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="freight_contract">Грузовой контракт</option>
            <option value="warehouse">Склад</option>
            <option value="fleet_management">Управление автопарком</option>
            <option value="last_mile">Последняя миля</option>
          </select>
        </div>
...
import { ref } from 'vue'
import logisticsB2bApi from '@/services/logisticsB2bApi'
...
const form = ref({
  ...
  requirement: '',
  budget_range: '',
  category: 'freight_contract' as const,
  source: '',
  notes: ''
})
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await logisticsB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
