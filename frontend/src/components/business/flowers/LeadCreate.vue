    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (Flowers B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="event_contract">Контракт на мероприятия</option>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" :disabled="submitting" class="px-4 py-2 bg-pink-500 text-white rounded-lg hover:bg-pink-600 disabled:opacity-50">
...
import { ref } from 'vue'
import flowersB2bApi from '@/services/flowersB2bApi'
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await flowersB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
