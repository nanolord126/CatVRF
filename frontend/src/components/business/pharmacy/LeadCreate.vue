    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (Pharmacy B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="chain_partnership">Партнерство сетей</option>
            <option value="distribution">Дистрибуция</option>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" :disabled="submitting" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
...
import { ref } from 'vue'
import pharmacyB2bApi from '@/services/pharmacyB2bApi'
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await pharmacyB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
