    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (Office Catering B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="daily_meals">Ежедневное питание</option>
            <option value="event_catering">Кейтеринг мероприятий</option>
            <option value="coffee_service">Кофе-сервис</option>
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await officecateringB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
