    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b">
        <div class="flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-900">Создать лида (Construction B2B)</h2>
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.category" required class="w-full px-3 py-2 border rounded-lg">
            ...
            <option value="materials_supply">Поставка материалов</option>
            <option value="equipment_rental">Аренда оборудования</option>
            <option value="contracting">Подряд</option>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" :disabled="submitting" class="px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-50">
...
import { ref } from 'vue'
import constructionB2bApi from '@/services/constructionB2bApi'
...
const form = ref({
  ...
  requirement: '',
  budget_range: '',
  category: 'materials_supply' as const,
  source: '',
  notes: ''
})
...
const submit = async () => {
  ...
  error.value = ''
  try {
    await constructionB2bApi.createLead(form.value)
    emit('created')
  } catch (err: any) {
