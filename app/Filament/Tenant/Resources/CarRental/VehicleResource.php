...
use Filament\Tables\{Table, Columns\TextColumn, Columns\BadgeColumn, Columns\BooleanColumn, Filters\SelectFilter, Filters\TrashedFilter, Filters\Filter};
...
use App\Filament\Tenant\Resources\CarRental\Pages\CreateVehicle;
use App\Filament\Tenant\Resources\CarRental\Pages\EditVehicle;
use App\Filament\Tenant\Resources\CarRental\Pages\ListVehicles;
...
final class VehicleResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            BadgeColumn::make('car_class')->label('Класс')->color(fn ($state) => match($state) {
                'economy' => 'blue', 'comfort' => 'cyan', 'business' => 'orange', 'premium' => 'yellow'
            }),
            TextColumn::make('daily_price')->label('День (₽)')->money('RUB', divideBy: 100)->sortable(),
            BadgeColumn::make('status')->label('Статус')->color(fn ($state) => match($state) {
                'available' => 'success', 'booked' => 'warning', 'maintenance' => 'info', 'accident' => 'danger'
            }),
            ...
            BulkActionGroup::make([DeleteBulkAction::make(), BulkAction::make('activate')->label('Активировать')->icon('heroicon-m-check-circle')->color('success')->action(function ($records) {
