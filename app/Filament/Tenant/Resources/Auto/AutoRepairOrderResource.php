        ...
        return $table
            ...
            ->actions([
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->after(function () {
                            $this->logger->$this->logger->info('AutoRepairOrder deleted', [
                    ...
                    Tables\Actions\RestoreAction::make()
                        ->after(function () {
                            $this->logger->$this->logger->info('AutoRepairOrder restored', [
                                'resource' => 'AutoRepairOrder',
                                'user_id' => $this->guard->id(),
                                'correlation_id' => Str::uuid(),
                            ]);
                    ...
                    Tables\Actions\Action::make('complete')
                        ...
                        ->action(function (Model $record) {
                            ...
                            $this->logger->$this->logger->info('AutoRepairOrder completed', [
                                'resource' => 'AutoRepairOrder',
                    ...
                    Tables\Actions\Action::make('cancel')
                        ...
                        ->visible(fn (Model $record) => ! in_array($record->status, ['completed', 'cancelled'], true))
                        ->action(function (Model $record) {
                            $record->update(['status' => 'cancelled']);
                            $this->logger->$this->logger->info('AutoRepairOrder cancelled', [
                                'resource' => 'AutoRepairOrder',
                                'resource_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
            ...
            ->bulkActions([
                    Tables\Actions\BulkActionGroup::make([
                        ...
                        ->after(function () {
                            $this->logger->$this->logger->info('AutoRepairOrders bulk deleted', [
                                'resource' => 'AutoRepairOrder',
                                'user_id' => $this->guard->id(),
                                'correlation_id' => Str::uuid(),
                            ]);
                        ...
                        ->action(function (Collection $records) {
                            foreach ($records as $record) {
                                $record->update(['status' => 'completed', 'completed_at' => CarbonImmutable::now()]);
                                $this->logger->$this->logger->info('AutoRepairOrder bulk completed', [
                                    'resource' => 'AutoRepairOrder',
                                    'resource_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                        ...
                        ->action(function (Collection $records) {
                            foreach ($records as $record) {
                                $record->update(['status' => 'cancelled']);
                                $this->logger->$this->logger->info('AutoRepairOrder bulk cancelled', [
                                    'resource' => 'AutoRepairOrder',
                                    'resource_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
