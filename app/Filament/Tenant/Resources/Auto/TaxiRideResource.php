...
use App\Filament\Tenant\Resources\Auto\TaxiRideResource\Pages\CreateTaxiRide;
use App\Filament\Tenant\Resources\Auto\TaxiRideResource\Pages\EditTaxiRide;
use App\Filament\Tenant\Resources\Auto\TaxiRideResource\Pages\ListTaxiRides;
use App\Filament\Tenant\Resources\Auto\TaxiRideResource\Pages\ViewTaxiRide;
...
final class TaxiRideResource extends Resource
{
            ...
            ->actions([
                    ...
                    Tables\Actions\Action::make('complete')
                        ...
                        ->action(function ($record) {
                            $record->update(['status' => 'completed', 'completed_at' => CarbonImmutable::now()]);
                            $this->logger->$this->logger->info('Taxi ride completed', [
                                'ride_id' => $record->id,
                    ...
                    Tables\Actions\Action::make('cancel')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Taxi ride cancelled', [
                                'ride_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    Tables\Actions\BulkActionGroup::make([
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                $this->logger->$this->logger->info('Taxi ride bulk deleted', [
                                    'ride_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                                ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                if (! in_array($record->status, ['completed', 'cancelled'], true)) {
                                    $record->update(['status' => 'completed', 'completed_at' => CarbonImmutable::now()]);
                                    $this->logger->$this->logger->info('Taxi ride bulk completed', [
                                        'ride_id' => $record->id,
                                        'user_id' => $this->guard->id(),
                                        'correlation_id' => $record->correlation_id,
                                    ]);
    ...
    {
        return [
            'index' => ListTaxiRides::route('/'),
            'create' => CreateTaxiRide::route('/create'),
            'view' => ViewTaxiRide::route('/{record}'),
            'edit' => EditTaxiRide::route('/{record}/edit'),
        ];
    }
