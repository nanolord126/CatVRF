...
use Filament\Tables;
use Filament\Tables\{Table, Columns\TextColumn, Columns\BadgeColumn, Filters\Filter, Filters\SelectFilter, Filters\TrashedFilter};
                ...
                Tables\Actions\ActionGroup::make([
                    ...
                    Action::make('confirm')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Grocery order confirmed', [
                                'order_id' => $record->id,
                    ...
                    Action::make('mark_picked')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Grocery order marked as picked', [
                                'order_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    Action::make('mark_in_transit')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Grocery order marked as in transit', [
                                'order_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    Action::make('mark_delivered')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Grocery order marked as delivered', [
                                'order_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    Action::make('cancel')
                        ...
                        ->color('danger')
                        ->label('Отменить')
                        ->visible(fn ($record) => ! in_array($record->status, ['delivered', 'cancelled'], true))
                        ->action(function ($record) {
                            $record->update(['status' => 'cancelled']);
                            $this->logger->$this->logger->info('Grocery order cancelled', [
                                'order_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    BulkActionGroup::make([
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                $this->logger->$this->logger->info('Grocery order bulk deleted', [
                                    'order_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                                ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                if ($record->status === 'pending') {
                                    $record->update(['status' => 'confirmed']);
                                    $this->logger->$this->logger->info('Grocery order bulk confirmed', [
                                        'order_id' => $record->id,
                                        'user_id' => $this->guard->id(),
                                        'correlation_id' => $record->correlation_id,
                                    ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                if (in_array($record->status, ['in_transit', 'picked'], true)) {
                                    $record->update(['status' => 'delivered']);
                                    $this->logger->$this->logger->info('Grocery order bulk marked delivered', [
                                        'order_id' => $record->id,
                                        'user_id' => $this->guard->id(),
                                        'correlation_id' => $record->correlation_id,
                                    ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                if (! in_array($record->status, ['delivered', 'cancelled'], true)) {
                                    $record->update(['status' => 'cancelled']);
                                    $this->logger->$this->logger->info('Grocery order bulk cancelled', [
                                        'order_id' => $record->id,
                                        'user_id' => $this->guard->id(),
                                        'correlation_id' => $record->correlation_id,
                                    ]);
