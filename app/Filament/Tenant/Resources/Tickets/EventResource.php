...
use App\Filament\Tenant\Resources\Tickets\EventResource\Pages\CreateEvent;
use App\Filament\Tenant\Resources\Tickets\EventResource\Pages\EditEvent;
use App\Filament\Tenant\Resources\Tickets\EventResource\Pages\ListEvents;
use App\Filament\Tenant\Resources\Tickets\EventResource\Pages\ViewEvent;
        ...
        return $table->columns([
            ...
            ->filters([
                ...
                Filter::make('upcoming')
                    ->label('Предстоящие')
                    ->query(fn (Builder $query) => $query->where('start_at', '>', CarbonImmutable::now())),
                ...
                ActionGroup::make([
                    ...
                    Action::make('verify')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Event verified', [
                                'event_id' => $record->id,
                    ...
                    Action::make('feature')
                        ...
                        ->action(function ($record) {
                            ...
                            $this->logger->$this->logger->info('Event featured', [
                                'event_id' => $record->id,
                                'user_id' => $this->guard->id(),
                                'correlation_id' => $record->correlation_id,
                            ]);
                    ...
                    BulkActionGroup::make([
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                $this->logger->$this->logger->info('Event bulk deleted', [
                                    'event_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                                ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                ...
                                $this->logger->$this->logger->info('Event activated', [
                                    'event_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                                ]);
                        ...
                        ->action(function ($records) {
                            $records->each(function ($record) {
                                $record->update(['is_verified' => true]);
                                $this->logger->$this->logger->info('Event bulk verified', [
                                    'event_id' => $record->id,
                                    'user_id' => $this->guard->id(),
                                    'correlation_id' => $record->correlation_id,
                                ]);
