...
use Filament\Forms\Components\{Section, TextInput, Select, RichEditor, FileUpload, Toggle, TagsInput, Hidden, Grid};
...
{
    protected static ?string $model = \App\Domains\Stationery\Models\StationeryProduct::class;
        ...
        return $form->schema([
            Section::make('Основная информация')
                ...
                ->schema([
                    ...
                    FileUpload::make('main_photo')->label('Фото')->image()->directory('stationery')->columnSpan(1),
                    FileUpload::make('photos')->label('Галерея')->image()->multiple()->directory('stationery')->columnSpan(1),
                ])->columns(4),
            ...
            Section::make('Категория и характеристики')
                ...
                ->schema([
                    ...
                    Select::make('material')
                        ->label('Материал')
                        ->options(['wood' => 'Дерево', 'plastic' => 'Пластик', 'metal' => 'Металл', 'paper' => 'Бумага'])
                        ->columnSpan(2),

                    TagsInput::make('features')->label('Свойства')->columnSpan('full'),

                    Select::make('color')->label('Цвет')->options(['black' => 'Чёрный', 'white' => 'Белый', 'red' => 'Красный', 'blue' => 'Синий', 'multicolor' => 'Разноцветный'])->columnSpan(2),

                    TextInput::make('quantity_per_pack')->label('В упаковке')->numeric()->columnSpan(1),

                    Select::make('pack_size')->label('Размер упаковки')->options(['small' => 'Маленькая', 'medium' => 'Средняя', 'large' => 'Большая'])->columnSpan(1),
                ])->columns(4),
            ...
            Section::make('Применение')
                ...
                ->schema([
                    Select::make('use_case')->label('Применение')->options(['office' => 'Офис', 'school' => 'Школа', 'home' => 'Дом', 'creative' => 'Творчество'])->columnSpan(2),
    ...
    public static function table(Tables\Table $table): Tables\Table
    {
        ...
        ->filters([
            SelectFilter::make('category')->label('Категория')->options(['paper' => 'Бумага', 'pens' => 'Ручки'])->multiple(),
    ...
    {
        return [
            'index' => \App\Filament\Tenant\Resources\Stationery\StationeryProductResource\Pages\ListStationeryProducts::route('/'),
            'create' => \App\Filament\Tenant\Resources\Stationery\StationeryProductResource\Pages\CreateStationeryProduct::route('/create'),
            'edit' => \App\Filament\Tenant\Resources\Stationery\StationeryProductResource\Pages\EditStationeryProduct::route('/{record}/edit'),
        ];
    }
