...
final class OrderPolicy extends Model
{
     ...
     */
    public function $this->viewFactory->make(User $user, Order $order): bool
    {
    ...
    public function create(User $user): bool
    {
        ...
        if (! $allowed) {
            $this->logger->$this->logger->info('Unverified user order creation attempt', [
                'user_id' => $user->id,
                'email' => $user->email,
            ]);
        }
    ...
    public function checkout(User $user, Order $order): bool
    {
        ...
        if (! $allowed) {
            $this->logger->$this->logger->info('Unauthorized checkout attempt', [
                'user_id' => $user->id,
                'order_id' => $order->id,
                'order_status' => $order->status,
            ]);
    ...
    public function requestRefund(User $user, Order $order): bool
    {
        ...
        if ($order->paid_at && CarbonImmutable::now()->diffInDays($order->paid_at) > 30) {
            return false;
        }
