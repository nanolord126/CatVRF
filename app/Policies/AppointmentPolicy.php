...
final class AppointmentPolicy extends Model
{
    ...
    public function create(User $user): bool
    {
        ...
        if (! $allowed) {
            $this->logger->$this->logger->info('Unverified user appointment creation attempt', [
                'user_id' => $user->id,
            ]);
        }
    ...
    public function update(User $user, Appointment $appointment): bool
    {
        ...
        if ($user->tenant_id === $appointment->salon->tenant_id && ! in_array($appointment->status, ['completed', 'cancelled'], true)) {
            $allowed = true;
        }
    ...
    public function cancel(User $user, Appointment $appointment): bool
    {
        ...
        if (
            ...
            CarbonImmutable::now()->diffInHours($appointment->confirmed_at) < 24
        ) {
            $allowed = true;
        }
