...
final class PaymentPolicy extends Model
{
     ...
     */
    public function $this->viewFactory->make(User $user, PaymentTransaction $payment): bool
    {
    ...
    public function create(User $user): bool
    {
        ...
        if (! $allowed) {
            $this->logger->$this->logger->info('Unverified user payment creation attempt', [
                'user_id' => $user->id,
                'email' => $user->email,
            ]);
        }
    ...
    public function refund(User $user, PaymentTransaction $payment): bool
    {
        $allowed = (
            ...
            CarbonImmutable::now()->diffInDays($payment->captured_at) <= (int) $this->config->get('payments.refund.max_refund_days') &&
            $user->hasRole(['business', 'admin'])
        );
        ...
        if (! $allowed) {
            $this->logger->warning('Unauthorized refund attempt', [
                ...
                'payment_id' => $payment->id,
                'payment_status' => $payment->status,
                'days_since_capture' => $payment->captured_at ? CarbonImmutable::now()->diffInDays($payment->captured_at) : null,
            ]);
        }
