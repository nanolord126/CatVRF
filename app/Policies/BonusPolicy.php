...
final class BonusPolicy extends Model
{
     ...
     */
    public function $this->viewFactory->make(User $user, Bonus $bonus): bool
    {
    ...
    public function spend(User $user, Bonus $bonus): bool
    {
        $allowed = (
            ...
            ! $bonus->spent_at &&
            ! $bonus->withdrawn_at &&
            $bonus->expires_at > CarbonImmutable::now() &&
            $user->hasRole('user')
        );

        if (! $allowed) {
            $this->logger->$this->logger->info('Unauthorized bonus spend attempt', [
                'user_id' => $user->id,
                'bonus_id' => $bonus->id,
                'is_expired' => $bonus->expires_at <= CarbonImmutable::now(),
            ]);
        }
    ...
    public function requestExtension(User $user, Bonus $bonus): bool
    {
        return
            $user->id === $bonus->user_id &&
            CarbonImmutable::now()->diffInDays($bonus->expires_at) <= 7 && // осталось 7 дней
