 ...
 *
 * Управление доступом к бонусам и вознаграждениям.
 * Разделяет права между физлицами (тратят) и бизнесом (выводят).
 */
final class BonusPolicy
{
    ...
    public function view(User $user, Bonus $bonus): bool
    {
        ...
        if (!$allowed) {
            $this->log->warning('Unauthorized bonus view attempt', [
                'user_id' => $user->id,
                'bonus_id' => $bonus->id,
            ]);
        }
    ...
    public function spend(User $user, Bonus $bonus): bool
    {
        ...
        if (!$allowed) {
            $this->log->info('Unauthorized bonus spend attempt', [
                'user_id' => $user->id,
                'bonus_id' => $bonus->id,
    ...
    public function withdraw(User $user, Bonus $bonus): bool
    {
        ...
        if (!$allowed) {
            $this->log->warning('Unauthorized bonus withdrawal attempt', [
                'user_id' => $user->id,
                'bonus_id' => $bonus->id,
    ...
    public function revoke(User $user, Bonus $bonus): bool
    {
        ...
        if (!$allowed) {
            $this->log->warning('Unauthorized bonus revocation attempt', [
                'user_id' => $user->id,
                'bonus_id' => $bonus->id,
            ]);
        }
