 ...
 *
 * Управление доступом к товарам и услугам.
 * Разделяет права продавца (может менять только свои) и администратора.
 */
final class ProductPolicy
{
    ...
    public function create(User $user): bool
    {
        ...
        if (!$allowed) {
            $this->log->info('Unauthorized product creation attempt', [
                'user_id' => $user->id,
            ]);
        }
    ...
    public function update(User $user, Product $product): bool
    {
        ...
        if (!$allowed) {
            $this->log->warning('Unauthorized product update attempt', [
                'user_id' => $user->id,
