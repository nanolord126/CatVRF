...
/**
 * Beauty Salon Authorization Policy
 * CANON 2026 - Production Ready
 *
 * Управление доступом к салонам красоты.
 * Разделяет права владельца и администратора.
 */
final class BeautyPolicy
{
    ...
    public function create(User $user): bool
    {
        ...
        if (!$allowed) {
            $this->log->info('Unauthorized salon creation attempt', [
                'user_id' => $user->id,
    ...
    public function update(User $user, BeautySalon $salon): bool
    {
        ...
        if (!$allowed) {
            $this->log->warning('Unauthorized salon update attempt', [
                'user_id' => $user->id,
                'user_tenant_id' => $user->tenant_id,
