...
{
    public function __construct(private readonly MdlpService $mdlpService,
    ...
    public function index()
    {
        ...
        return $this->viewFactory->make('account.integrations.index', compact('integrations'));
    }
    ...
    public function test(Request $request, string $type): JsonResponse
    {
        ...
        try {
            $this->logger->channel('audit')->$this->logger->info('Testing compliance integration', [
                'type' => $type,
                'inn' => $inn,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function connect(Request $request, string $type): JsonResponse
    {
        ...
        return $this->db->transaction(function () use ($request, $type, $correlationId) {
            try {
                ...
                $integration->save();
                $this->logger->channel('audit')->$this->logger->info('Compliance integration connected', [
                    'id' => $integration->id,
                    'type' => $type,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function disconnect(string $type): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Compliance integration disconnected', [
                'type' => $type,
                'correlation_id' => $correlationId,
            ]);
    ...
    private function performApiTest(string $type, string $inn, string $token): array
    {
        switch ($type) {
            ...
            case 'mercury':
                ...
                if ($token === 'test_mercury') {
                    return ['success' => true];
                }
                $mercury = $this->mercuryService /* TODO: inject via constructor DI */ /* TODO: inject via DI */;
            ...
            case 'mdlp':
                ...
                $mdlp = $this->mdlpService /* TODO: inject via constructor DI */ /* TODO: inject via DI */;
