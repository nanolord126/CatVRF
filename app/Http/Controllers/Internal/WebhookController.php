...
final class WebhookController extends Controller
{
    ...
    public function handle(Request $request, string $provider): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Webhook received', [
                'provider' => $provider,
            ...
            $this->db->transaction(function () use ($payment, $paymentInfo, $provider, $correlationId) {
                ...
                $payment->update([
                    ...
                    'metadata' => array_merge($payment->metadata ?? [], [
                        'webhook_received_at' => CarbonImmutable::now()->toIso8601String(),
                        'provider' => $provider,
                    ]),
                ]);
            ...
            $this->logger->channel('audit')->$this->logger->info('Webhook processed successfully', [
                'provider' => $provider,
    ...
    private function extractTinkoffInfo(array $data): array
    {
        ...
        return [
            ...
            'authorized_at' => isset($data['AuthDateTime']) ? CarbonImmutable::now() : null,
            'captured_at' => $data['Status'] === 'CONFIRMED' ? CarbonImmutable::now() : null,
        ];
    }
    ...
    private function extractSberInfo(array $data): array
    {
        ...
        return [
            ...
            'authorized_at' => in_array($data['orderStatus'], ['1', '2'], true) ? CarbonImmutable::now() : null,
            'captured_at' => $data['orderStatus'] === '2' ? CarbonImmutable::now() : null,
        ];
    }
    ...
    private function extractSbpInfo(array $data): array
    {
        ...
        return [
            ...
            'status' => $statusMap[$data['status']] ?? 'pending',
            'authorized_at' => isset($data['created_at']) ? CarbonImmutable::now() : null,
            'captured_at' => $data['status'] === 'COMPLETED' ? CarbonImmutable::now() : null,
        ];
    }
}
