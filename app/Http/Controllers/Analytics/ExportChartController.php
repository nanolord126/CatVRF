...
final class ExportChartController extends Controller
{
    ...
    public function exportPng(Request $request): Response
    {
        try {
            ...
            $filename = $validated['filename'] ?? 'chart-'.CarbonImmutable::now()->format('Y-m-d-H-i-s').'.png';
            $this->logger->channel('audit')->$this->logger->info('Chart PNG exported', [
                'correlation_id' => $correlationId,
                'filename' => $filename,
    ...
    public function exportPdf(Request $request): Response
    {
        try {
            ...
            $pdfData = [
                ...
                'generated_at' => CarbonImmutable::now()->format('d.m.Y H:i:s'),
                'correlation_id' => $correlationId,
            ...
            $filename = Str::slug($filename).'-'.CarbonImmutable::now()->format('Y-m-d-H-i-s').'.pdf';
            $this->logger->channel('audit')->$this->logger->info('Chart PDF exported', [
                'correlation_id' => $correlationId,
                'filename' => $filename,
                'title' => $pdfData['title'],
                'tenant_id' => $tenant->id,
    ...
    public function exportPngBrowsershot(Request $request): Response
    {
        try {
            ...
            //     ->screenshot();
            $this->logger->channel('audit')->$this->logger->info('Browsershot PNG export attempted', [
                'correlation_id' => $correlationId,
                'width' => $width,
                'height' => $height,
            ]);
     ...
     */
    public function quickExport(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
            ...
            $correlationId = $request->header('X-Correlation-ID') ?? Str::uuid();
            $tenant = $this->guard->user()->currentTenant;
            $filename = 'analytics/'.$tenant->id.'/'.CarbonImmutable::now()->format('Y-m-d').'/'
            ...
            $this->logger->channel('audit')->$this->logger->info('Quick export completed', [
                'correlation_id' => $correlationId,
