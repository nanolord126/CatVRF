...
/**
 * Контроллер для экспорта графиков в PNG/PDF
 * 
 * Принимает данные графика (Canvas image + metadata)
 * Генерирует файлы для скачивания
 */
final class ExportChartController extends Controller
{
    ...
    public function exportPng(Request $request): Response
    {
        try {
            ...
            if (!preg_match('/^data:image\/png;base64,/', $validated['chart_image'])) {
                ...
                return response('Invalid image format', 400);
            }
            ...
            if ($imageData === false) {
                throw new \Exception('Failed to decode PNG image');
            }
    ...
    public function exportPngBrowsershot(Request $request): Response
    {
        try {
            ...
            return response('Browsershot PNG export requires Chrome installation. Use basic PNG export instead.', 501)
                ->header('X-Correlation-ID', $correlationId);
        } catch (\Exception $e) {
            ...
            return response('Browsershot export failed: ' . $e->getMessage(), 500);
        }
    }
    /**
