...
final class EventPlanningController extends Controller
{
    public function __construct(private readonly EventDispatcher $eventDispatcher,
        ...
        private readonly EventPlanningService $eventService,
    ...
    public function store(CreateEventRequest $request): JsonResponse
    {
        ...
        try {
            $this->logger->channel('audit')->$this->logger->info('API-Request: Store Event starting', [
    ...
    public function index(Request $request): JsonResponse
    {
        $correlationId = $request->header('X-Correlation-ID', (string) Str::uuid());
        try {
            $events = $this->eventDispatcher->where('client_id', $request->user()?->id)
                ->with(['vendors'])
