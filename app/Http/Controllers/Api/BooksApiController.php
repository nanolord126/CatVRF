...
final class BooksApiController extends Controller
{
    ...
    public function searchByGenre(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Books API search triggered', [
                'cid' => $correlationId,
                'genre' => $genreSlug,
                'count' => $books->count(),
            ]);
    ...
    public function consultAi(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('AI Book Consultant invoked', [
                'cid' => $correlationId,
                'user_id' => $user->id,
                'mood' => $validated['mood'],
            ]);
    ...
    public function submitCorporateOrder(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $bookIds = new Collection($validated['items'])->pluck('book_id')->toArray();
            $quantities = new Collection($validated['items'])->pluck('quantity')->toArray();
