...
use App\Domains\PersonalDevelopment\Services\AIGrowthConstructor;
...
use Throwable;
/**
 * PersonalDevelopmentApiController — Production Ready 2026
 * 
 * Layer 9: API Entry Point
 * Предоставляет эндпоинты для доступа к программам, сессиям и AI-конструктору.
 * 
 * @version 1.0.0
 * @author CatVRF
 */
final class PersonalDevelopmentApiController extends Controller
{
    ...
    public function enroll(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::channel('audit')->error('PD API Enrollment Error', [
                'error' => $e->getMessage(),
    ...
    public function generateAiRoadmap(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::channel('audit')->error('PD AI Roadmap Error', [
                'error' => $e->getMessage(),
                'correlation_id' => $correlationId,
            ]);
            return response()->json([
