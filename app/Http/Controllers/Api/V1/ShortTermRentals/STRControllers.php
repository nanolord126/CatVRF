...
final class PropertyController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Properties list requested', [
    ...
    public function show(Property $property, Request $request): JsonResponse
    {
        try {
            ...
            $property->load([
                ...
                'bookings' => function ($q) {
                    $q->where('status', '!=', 'cancelled')
                        ->where('check_out_date', '>=', CarbonImmutable::now()->toDateString())
            ...
            $this->logger->channel('audit')->$this->logger->info('Property details viewed', [
    ...
    public function update(UpdatePropertyRequest $request, Property $property): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Property updated', [
                'property_id' => $property->id,
                'user_id' => $this->guard->id(),
...
final class BookingController extends Controller
{
    ...
    public function store(CreateBookingRequest $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($request, $correlationId) {
                ...
                if ($fraudScore > 0.85) {
                    throw new HttpException(403, 'Booking blocked by fraud detection');
                }
            ...
            $this->logger->channel('audit')->$this->logger->info('Booking created', [
    ...
    public function show(PropertyBooking $booking, Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Booking details viewed', [
    ...
    public function cancel(PropertyBooking $booking, Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Booking cancelled', [
                'booking_id' => $booking->id,
                'user_id' => $this->guard->id(),
                'correlation_id' => $correlationId,
            ]);
...
final class PayoutController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Payouts list requested', [
    ...
    public function requestPayout(Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Payout requested', [
                'tenant_id' => $this->guard->user()->current_tenant_id,
