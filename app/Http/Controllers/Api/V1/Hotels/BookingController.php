    ...
    public function store(CreateBookingRequest $request): JsonResponse
    {
        ...
        try {
            return $this->db->transaction(function () use ($request, $correlationId, $tenantId) {
                ...
                // 5. Логирование
                $this->logger->channel('audit')->$this->logger->info('Hotel booking created', [
                    'correlation_id' => $correlationId,
    ...
    public function checkIn(Booking $booking, CreateBookingRequest $request): JsonResponse
    {
        ...
        try {
            return $this->db->transaction(function () use ($booking, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Hotel check-in completed', [
                    'correlation_id' => $correlationId,
                    'booking_id' => $booking->id,
                ]);
        ...
        try {
            return $this->db->transaction(function () use ($booking, $correlationId) {
                ...
                if ($request->has('early_checkout') && $request->boolean('early_checkout')) {
                    $daysUsed = CarbonImmutable::now()->diffInDays($booking->checked_in_at);
                    ...
                    $this->logger->channel('audit')->$this->logger->info('Early checkout processed', [
                        'correlation_id' => $correlationId,
                        'booking_id' => $booking->id,
    ...
    public function cancel(Booking $booking, CreateBookingRequest $request): JsonResponse
    {
        ...
        try {
            return $this->db->transaction(function () use ($booking, $correlationId) {
                ...
                $daysUntilCheckIn = CarbonImmutable::now()->diffInDays($booking->check_in_date);
