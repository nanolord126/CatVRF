...
final class PaymentController extends Controller
{
     ...
     * )
     */
    public function init(PaymentInitRequest $request): ResponseFactory
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('PaymentController::init called', [
            ...
            return $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Payment initiated V1', [
     ...
     */
    public function show(string $paymentId): ResponseFactory
    {
        $correlationId = (string) \Illuminate\Support\Str::uuid()->toString();
        try {
            $payment = PaymentTransaction::where('uuid', $paymentId)
                ->where('tenant_id', (int) tenant('id'))
                ->firstOrFail();
            $this->logger->channel('audit')->$this->logger->info('Payment retrieved', [
                'payment_id' => $payment->id,
                'correlation_id' => $correlationId,
            ]);
