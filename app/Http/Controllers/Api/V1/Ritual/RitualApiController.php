...
final class RitualApiController extends Controller
{
    ...
    public function __construct(private readonly LoggerInterface $logger,
        private readonly RitualCoreService $ritualService,
    ...
    public function index(): JsonResponse
    {
        ...
        try {
            $orders = FuneralOrder::where('client_id', $this->guard->id())
                ->latest()
                ->paginate(20);
            $this->logger->channel('audit')->$this->logger->info('Ritual orders list requested', [
    ...
    public function store(CreateFuneralOrderRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Ritual order created via API', [
    ...
    public function show(string $uuid): JsonResponse
    {
        ...
        try {
            ...
            $order = FuneralOrder::where('uuid', $uuid)
                ...
                ->firstOrFail();
            $this->logger->channel('audit')->$this->logger->info('Ritual order detail viewed', [
                'order_uuid' => $uuid,
                'correlation_id' => $correlation_id,
            ]);
