...
final class AuthController extends Controller
{
    ...
    public function store(TokenCreateRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $token = $user->createToken(
                ...
                $request->abilities ?? ['*'],
                CarbonImmutable::now()->addDays((int) config('SANCTUM_EXPIRATION_DAYS', 365))
            );
            $this->logger->channel('audit')->$this->logger->info('Token created', [
                'user_id' => $user->id,
    ...
    public function refresh(TokenRefreshRequest $request): JsonResponse
    {
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Token refreshed', [
                'user_id' => $user->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function destroy(int $tokenId): JsonResponse
    {
        ...
        try {
            ...
            $this->db->table('personal_access_tokens')
                ...
                ->update(['revoked' => true]);
            $this->logger->channel('audit')->$this->logger->info('Token revoked', [
                'user_id' => $user->id,
                'token_id' => $tokenId,
                'correlation_id' => $correlationId,
            ]);
