...
final class FashionB2CController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $products = FashionProduct::query()
                ...
                ->paginate(20);
            $this->logger->channel('audit')->$this->logger->info('B2C products browsed', [
            ...
            $this->logger->channel('audit')->error('B2C index error', ['trace' => $e->getTraceAsString(), 'correlation_id' => $correlationId]);

            return $this->response->json(['error' => 'Internal Server Error', 'correlation_id' => $correlationId], 500);
        }
    }
    ...
    public function reserve(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if (! $success) {
                return $this->response->json(['error' => 'Insufficient stock', 'correlation_id' => $correlationId], 422);
            }
            ...
            $this->logger->channel('audit')->error('Reservation failed', ['msg' => $e->getMessage(), 'correlation_id' => $correlationId]);

            return $this->response->json(['error' => 'Reservation failed', 'correlation_id' => $correlationId], 400);
        }
    }
    ...
    public function aiOutfit(Request $request): JsonResponse
    {
        ...
        if (! $request->hasFile('photo')) {
            return $this->response->json(['error' => 'Photo required', 'correlation_id' => $correlationId], 400);
        }
        try {
            ...
            return $this->response->json(['error' => 'AI processing failed', 'correlation_id' => $correlationId], 500);
        }
    }
}
