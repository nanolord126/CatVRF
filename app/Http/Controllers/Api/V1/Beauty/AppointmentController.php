    ...
    public function store(CreateAppointmentRequest $request): JsonResponse
    {
        ...
        try {
            return $this->db->transaction(function () use ($request, $correlationId, $tenantId) {
                ...
                // 4. Логирование
                $this->logger->channel('audit')->$this->logger->info('Beauty appointment created', [
                    'correlation_id' => $correlationId,
    ...
    public function confirm(
        ...
        try {
            return $this->db->transaction(function () use ($appointment, $request, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Beauty appointment confirmed', [
                    'correlation_id' => $correlationId,
                    'appointment_id' => $appointment->id,
                    'user_id' => $this->guard->id(),
                ]);
    ...
    public function cancel(
        ...
        try {
            return $this->db->transaction(function () use ($appointment, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Beauty appointment cancelled', [
                    'correlation_id' => $correlationId,
                    'appointment_id' => $appointment->id,
                ]);
