...
final readonly class FraudDetectionController extends Controller
{
    public function __construct(
        ...
        private readonly EducationFraudDetectionService $fraudService,
    ) {}

    public function detectCheating(int $enrollmentId, Request $request): JsonResponse
    {
        $request->validate([
        ...
        if ($detection->isCheating) {
            $this->events->dispatch(new FraudDetectedEvent(
        ...
        return new JsonResponse($detection->toArray())
            ->header('X-Correlation-ID', $correlationId);
    }

    public function detectReviewFraud(int $reviewId, Request $request): JsonResponse
    {
        $request->validate([
            'user_id' => ['required', 'integer', 'exists:users,id'],
