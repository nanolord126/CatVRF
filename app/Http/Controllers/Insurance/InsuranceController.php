...
final class InsuranceController extends Controller
{
    ...
    public function storePolicy(Request $request): JsonResponse
    {
        ...
        // 1. Audit Log Request
        $this->logger->channel('audit')->$this->logger->info('[InsuranceController] storePolicy request starting', [
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('[InsuranceController] storePolicy completed successfully', [
                'correlation_id' => $correlationId,
    ...
    {
        $correlationId = $request->header('X-Correlation-ID') ?? (string) Str::uuid();
        $this->logger->channel('audit')->$this->logger->info('[InsuranceController] storeClaim request starting', [
            'correlation_id' => $correlationId,
            'user_id' => $request->user()->id,
        ]);
        try {
    ...
    public function estimate(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $type = InsuranceType::findOrFail($validated['type_id']);
