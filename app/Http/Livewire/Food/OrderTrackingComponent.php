...
final class OrderTrackingComponent extends Component
{
        ...
        try {
            ...
            $this->timelineEvents = [
                [
                    'status' => 'pending',
                    'title' => 'Заказ получен',
                    'completed' => in_array($this->order->status, ['pending', 'cooking', 'ready', 'delivered'], true),
                    'time' => $this->order->created_at->format('H:i'),
                ],
                [
                    'status' => 'cooking',
                    'title' => 'Готовится',
                    'completed' => in_array($this->order->status, ['cooking', 'ready', 'delivered'], true),
                    'time' => $this->order->cooking_started_at?->format('H:i') ?? '--',
                ],
                [
                    'status' => 'ready',
                    'title' => 'Готово к подаче',
                    'completed' => in_array($this->order->status, ['ready', 'delivered'], true),
    ...
    public function cancelOrder(): void
    {
        if ($this->order && $this->order->status === 'pending') {
            try {
                ...
                $this->logger->$this->logger->info('Order cancelled', [
    ...
    {
        return $this->viewFactory->make('livewire.food.order-tracking', [
