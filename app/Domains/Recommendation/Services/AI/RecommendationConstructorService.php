...
namespace App\Domains\Recommendation\Services\AI;
...
final readonly class RecommendationConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'recommendation_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:Recommendation:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('Recommendation', $fullProfile, $userId);

            $this->logger->$this->logger->info('Recommendation AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Персонализированные рекомендации. Подбери товары и услуги по профилю пользователя.',
            ];
        });
    }
}
