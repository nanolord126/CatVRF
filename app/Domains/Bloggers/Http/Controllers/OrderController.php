...
use App\Domains\Bloggers\Http\Requests\CreateOrderRequest;
use App\Domains\Bloggers\Http\Requests\ConfirmPaymentRequest;
...
final class OrderController
{
    ...
    public function confirmPayment(ConfirmPaymentRequest $request, int $orderId): JsonResponse
    {
        try {
            ...
            $order = \App\Domains\Bloggers\Models\StreamOrder::findOrFail($orderId);
    ...
    {
        try {
            $order = \App\Domains\Bloggers\Models\StreamOrder::with('product', 'stream')
                ->findOrFail($orderId);
    ...
    public function getUserOrders(Request $request): JsonResponse
    {
        try {
            ...
            $orders = \App\Domains\Bloggers\Models\StreamOrder::where('user_id', $userId)
                ->where('tenant_id', tenant()->id)
                ->with('product', 'stream')
