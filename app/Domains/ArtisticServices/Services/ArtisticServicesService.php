...
namespace App\Domains\ArtisticServices\Services;
use App\Domains\ArtisticServices\Models\Artist;
use App\Domains\ArtisticServices\Models\ArtisticProject;
...
return $this->db->transaction(function()use($artistId,$projectType,$artisticHours,$dueDate,$correlationId){$a=Artist::findOrFail($artistId);$total=(int)($a->price_kopecks_per_hour*$artisticHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'artistic','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$p=ArtisticProject::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'artist_id'=>$artistId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','project_type'=>$projectType,'artistic_hours'=>$artisticHours,'due_date'=>$dueDate,'tags'=>['artistic'=>true]]);$this->log->channel('audit')->info('Artistic project created',['project_id'=>$p->id,'correlation_id'=>$correlationId]);return $p;});
}
public function completeProject(int $projectId,string $correlationId=""):ArtisticProject{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($projectId,$correlationId){$p=ArtisticProject::findOrFail($projectId);if($p->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$p->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$p->payout_kopecks,'art_payout',['correlation_id'=>$correlationId,'project_id'=>$p->id]);$this->log->channel('audit')->info('Artistic project completed',['project_id'=>$p->id]);return $p;});}
public function cancelProject(int $projectId,string $correlationId=""):ArtisticProject{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($projectId,$correlationId){$p=ArtisticProject::findOrFail($projectId);if($p->status==='completed')throw new \RuntimeException("Cannot cancel",400);$p->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($p->payment_status==='completed')$this->wallet->credit(tenant()->id,$p->total_kopecks,'art_refund',['correlation_id'=>$correlationId,'project_id'=>$p->id]);$this->log->channel('audit')->info('Artistic project cancelled',['project_id'=>$p->id]);return $p;});}
