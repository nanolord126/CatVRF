...
final readonly class PromoCampaignConstructorService
{
    ...
    public function analyzeAndRecommend(array $promoData, int $userId): array
    {
        ...
        // 3. Разбор ответа AI
        $promo_profile = $this->parseAnalysis($analysisText);

        // 4. Персонализация по вкусам
        $promo_profile['taste_enrichment'] = $tasteProfile->toArray();
        ...
        $this->saveToUserProfile($userId, 'promo', $promo_profile, $correlationId);
