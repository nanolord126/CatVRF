...
final readonly class AIWeddingPlannerConstructor
{
    ...
    {
        $this->logger->$this->logger->info('AIConstructor: Generating wedding plan', [
            'budget' => $budget,
            'style' => $style,
            'guests' => $guestCount,
    ...
    private function assessWeddingRisks(string $style, int $guests): array
    {
        $risks = [];
        if ($guests > 100) {
            $risks[] = 'High logistical risk for transport and catering';
        }
        if ($style === 'boho' || $style === 'rustic') {
            $risks[] = 'Outdoor event: Weather dependency (need Tent fallback)';
        }
