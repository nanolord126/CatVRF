...
namespace App\Domains\MusicAndInstruments\Services\AI;
...
final readonly class MusicConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'musicandinstruments_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:MusicAndInstruments:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('MusicAndInstruments', $fullProfile, $userId);

            $this->logger->$this->logger->info('MusicAndInstruments AI constructor used', [
                'user_id'        => $userId,
                'correlation_id' => $correlationId,
                'vertical'       => 'MusicAndInstruments',
            ]);
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Подбор музыкальных инструментов. Определи уровень игры и музыкальный стиль.',
            ];
        });
    }
}
