...
final readonly class ToyDomainService
{
    ...
    {
        $this->logger->$this->logger->info('Toy Upsert Initiated', [
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('Toy Upsert Successful', [
                'cid' => $dto->correlationId,
                'toy_uuid' => $toy->uuid,
                'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
            ]);
    ...
    {
        $this->logger->$this->logger->info('B2B Toy Order Processed', ['cid' => $dto->correlationId]);
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('B2B Toy Order Completed', [
                'cid' => $dto->correlationId,
    ...
    public function submitReview(int $toyId, int $userId, int $rating, string $comment, string $cid): ToyReview
    {
        return $this->db->transaction(function () use ($toyId, $userId, $rating, $comment, $cid) {
            ...
            $this->logger->$this->logger->info('Toy Review Added', [
                'cid' => $cid,
                'toy_id' => $toyId,
                'rating' => $rating,
                'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
