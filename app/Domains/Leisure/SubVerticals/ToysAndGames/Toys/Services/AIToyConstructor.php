...
final readonly class AIToyConstructor
{
    ...
    {
        $this->logger->$this->logger->info('AI Toy & Game Constructor Invoked', [
            'user_id' => $dto->userId,
        ...
        $scoredToys = $toys->map(function (Toy $toy) use ($dto) {
            ...
            foreach ($dto->interests as $interest) {
                ...
                if (new Collection($toy->tags)->contains(fn ($tag) => str_contains(strtolower($tag), $interest))) {
                    $score += 10;
                }
            ...
            if ($toy->safety_certification) {
                $score += 2;
            }

            // Filter by budget
            $price = $dto->b2bMode ? $toy->price_b2b : $toy->price_b2c;
            if ($price > $dto->budgetLimit) {
                $score = -100;
        ...
        $this->logger->$this->logger->info('AI Toy Construction Complete', [
            'cid' => $roadmap['cid'],
