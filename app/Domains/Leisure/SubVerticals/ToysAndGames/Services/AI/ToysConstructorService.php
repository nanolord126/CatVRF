...
namespace App\Domains\Leisure\SubVerticals\ToysAndGames\Services\AI;
...
final readonly class ToysConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        return $this->cache->tags(['toys', 'ai', 'constructor'])->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $this->logger->$this->logger->info('ToysAndGames AI constructor used', [
                'user_id'        => $userId,
