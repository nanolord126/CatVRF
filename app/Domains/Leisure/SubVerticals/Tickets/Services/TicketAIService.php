...
namespace App\Domains\Leisure\SubVerticals\Tickets\Services;
...
final readonly class TicketAIService
{
    ...
    public function suggestEventsForUser(int $userId, array $context = []): Collection
    {
        ...
        return $this->cache->remember($cacheKey, 3600, function () use ($userId, $context, $correlationId) {
            $this->logger->$this->logger->info('AI event suggestion initiated', [
                'user_id' => $userId,
            ...
            $candidates = $this->eventDispatcher->active()->with(['venue', 'ticketTypes'])->get();
            ...
            $scored = $candidates->map(function ($event) use ($tasteProfile, $context) {
                ...
                $score = in_array($event->category, $tasteProfile['categories'] ?? [], true) ? 0.45 : 0.1;
            ...
            $this->logger->$this->logger->info('AI event scoring completed', [
    ...
    {
        $event = $this->eventDispatcher->findOrFail($eventId);
        $correlationId = $event->correlation_id ?? (string) Str::uuid();

        $this->logger->$this->logger->info('Event demand prediction requested', [
            'event_id' => $eventId,
            'correlation_id' => $correlationId,
        ]);
        ...
        $features = [
            ...
            'days_until_start' => $event->start_at->diffInDays(CarbonImmutable::now()),
    ...
    {
        $this->logger->$this->logger->info('AI SeatMap design requested', $requirements);
    ...
    private function calculateSalesRate(Event $event): float
    {
        ...
        $daysOpen = $event->created_at->diffInDays(CarbonImmutable::now()) ?: 1;
