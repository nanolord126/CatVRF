...
namespace App\Domains\Leisure\SubVerticals\Tickets\Services\AI;
...
final readonly class TicketsConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'tickets_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:Tickets:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('Tickets', $fullProfile, $userId);

            $this->logger->$this->logger->info('Tickets AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Подбор билетов. Определи интересы, даты и бюджет пользователя.',
            ];
        });
    }
}
