...
namespace App\Domains\Leisure\SubVerticals\Tickets\Policies;

final class B2BTicketPolicy
{
    ...
    public function viewStorefront(User $user, B2BTicketStorefront $storefront): Response
    {
    ...
    public function updateStorefront(User $user, B2BTicketStorefront $storefront): Response
    {
        return $user->tenant_id === $storefront->tenant_id || $user->is_admin
            ? $this->response->allow()
            : $this->response->deny('Нет доступа');
    ...
    public function viewOrder(User $user, B2BTicketOrder $order): Response
    {
    ...
    public function approveOrder(User $user, B2BTicketOrder $order): Response
    {
    ...
    public function rejectOrder(User $user, B2BTicketOrder $order): Response
    {
        return ($user->tenant_id === $order->tenant_id || $user->is_admin) && $order->status === 'pending'
            ? $this->response->allow()
