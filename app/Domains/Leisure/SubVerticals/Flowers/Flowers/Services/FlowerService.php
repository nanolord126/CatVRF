...
final readonly class FlowerService
{
    ...
    public function createOrder(array $data): FlowerOrder
    {
        ...
        return $this->db->transaction(function () use ($data, $correlationId, $isB2B, $tenantId): FlowerOrder {
            ...
            $this->logger->$this->logger->info('Flower Order Created', [
                'order_id' => $order->id,
                'mode' => $isB2B ? 'B2B' : 'B2C',
            ...
            $this->eventDispatcher->dispatch(new FlowerOrderCreated($order, $correlationId));

            // Запуск реалтайм-трекинга доставки цветов
    ...
    public function updateStatus(int $orderId, string $status, string $correlationId = ''): void
    {
        ...
        $this->logger->$this->logger->info('Flower Order Status Updated', [
            'order_id' => $orderId,
            'from' => $oldStatus,
            'to' => $status,
            'correlation_id' => $correlationId,
