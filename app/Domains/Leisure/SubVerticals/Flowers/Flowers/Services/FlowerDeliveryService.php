...
final readonly class FlowerDeliveryService
{
    ...
    public function assignDelivery(
        ...
        return $this->db->transaction(function () use ($orderId, $courierName, $courierPhone, $correlationId): FlowerDelivery {
            ...
            $this->logger->$this->logger->info('Flower delivery assigned', [
    ...
    public function updateDeliveryStatus(
        ...
        return $this->db->transaction(function () use ($deliveryId, $status, $location, $correlationId): FlowerDelivery {
            ...
            if ($status === 'in_transit') {
                $data['picked_up_at'] = CarbonImmutable::now();
            } elseif ($status === 'delivered') {
                $data['delivered_at'] = CarbonImmutable::now();
            }

            $delivery->update($data);

            if ($status === 'delivered') {
                FlowerDeliveryCompleted::$this->bus->dispatch($delivery, $correlationId);
            }

            $this->logger->$this->logger->info('Flower delivery status updated', [
                'delivery_id' => $delivery->id,
                'status' => $status,
                'correlation_id' => $correlationId,
            ]);
