...
final class FlowerShopController extends Controller
{
    ...
    public function show(int $id): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('Flower shop viewed', [
                'shop_id' => $shop->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $shop = $this->db->transaction(function () use ($validated, $correlationId, $tenantId, $userId) {
                ...
                $this->logger->$this->logger->info('Flower shop created', [
                    'shop_id' => $shop->id,
                    'user_id' => $userId,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function update(int $id, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $shop = $this->db->transaction(function () use ($shop, $validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('Flower shop updated', [
                    'shop_id' => $shop->id,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function verify(int $id): JsonResponse
    {
        ...
        try {
            $shop = $this->db->transaction(function () use ($id, $correlationId) {
                ...
                $this->logger->$this->logger->info('Flower shop verified', [
                    'shop_id' => $shop->id,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function adminDestroy(int $id): JsonResponse
    {
        ...
        try {
            $this->db->transaction(function () use ($id, $correlationId) {
                ...
                $this->logger->$this->logger->info('Flower shop deleted', [
                    'shop_id' => $shop->id,
                    'correlation_id' => $correlationId,
                ]);
            });
