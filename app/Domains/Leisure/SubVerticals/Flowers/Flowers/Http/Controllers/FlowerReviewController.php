...
final class FlowerReviewController extends Controller
{
    ...
    public function store(int $orderId, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $review = $this->db->transaction(function () use ($order, $validated, $correlationId, $request) {
                ...
                $this->logger->$this->logger->info('Flower review created', [
                    'review_id' => $review->id,
                    'order_id' => $order->id,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function update(int $id, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $review = $this->db->transaction(function () use ($review, $validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('Flower review updated', [
                    'review_id' => $review->id,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function destroy(int $id, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                ...
                $this->logger->$this->logger->info('Flower review deleted', [
                    'review_id' => $review->id,
                    'correlation_id' => $correlationId,
                ]);
            });
