...
use App\Domains\Flowers\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $flowersOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $data = $request->all();
        $data['vertical'] = 'flowers';

        $validation = $this->flowersOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Flowers order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->flowersOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->flowersOrderService->getDeliveryEstimate($address);
