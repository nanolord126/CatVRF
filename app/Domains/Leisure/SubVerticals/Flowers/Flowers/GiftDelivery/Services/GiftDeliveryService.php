...
namespace App\Domains\Leisure\SubVerticals\Flowers\GiftDelivery\Services;
...
final readonly class GiftDeliveryService
{
    ...
    public function createOrder(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Gift order created', [
                'order_id'       => $order->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function completeOrder(int $orderId, string $correlationId = ''): GiftOrder
    {
        ...
        return $this->db->transaction(function () use ($orderId, $correlationId): GiftOrder {
            ...
            $this->logger->$this->logger->info('Gift order completed', [
                'order_id'       => $order->id,
    ...
    public function cancelOrder(int $orderId, string $correlationId = ''): GiftOrder
    {
        ...
        return $this->db->transaction(function () use ($orderId, $correlationId): GiftOrder {
            ...
            $this->logger->$this->logger->info('Gift order cancelled', [
                'order_id'       => $order->id,
                'refunded'       => $wasPaid,
                'correlation_id' => $correlationId,
            ]);
     ...
     * @return Collection<int, GiftOrder>
     */
    public function getUserOrders(int $senderId, int $limit = 10): Collection
    {
