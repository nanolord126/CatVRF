...
namespace App\Domains\BooksAndLiterature\Services\AI;
...
final readonly class BooksConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'booksandliterature_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:BooksAndLiterature:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('BooksAndLiterature', $fullProfile, $userId);

            $this->logger->$this->logger->info('BooksAndLiterature AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Анализ читательских предпочтений. Подбери книги по жанру, стилю и интересам.',
            ];
        });
    }
}
