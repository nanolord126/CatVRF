...
final readonly class BooksDomainService
{
    ...
    public function upsertBook(BookSaveDto $dto): Book
    {
        ...
        // 1. Audit Entry
        $this->logger->$this->logger->info('Book Upsert Initiated', [
            'sku' => $dto->isbn,
            'cid' => $correlationId,
            'title' => $dto->title,
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('Book Upsert Completed', ['id' => $book->id, 'cid' => $correlationId]);
    ...
    public function createCorporateOrder(VolumeOrderDto $dto): BookOrder
    {
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('Corporate Order Created', [
                'order_id' => $order->id,
                'type' => $dto->type,
                'total' => $totalAmount,
                'cid' => $correlationId,
    ...
    public function submitReview(BookReviewDto $dto): BookReview
    {
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('Book Review Submitted', ['id' => $review->id, 'cid' => $correlationId]);
