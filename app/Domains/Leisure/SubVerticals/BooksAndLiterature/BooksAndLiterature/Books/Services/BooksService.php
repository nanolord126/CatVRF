...
final readonly class BooksService
{
    ...
    public function createOrder(array $items, string $correlationId = ''): BookOrder
    {
        ...
        if ($this->rateLimiter->tooManyAttempts('books:order:'.$this->guard->id(), 20)) {
        ...
        return $this->db->transaction(function () use ($items, $correlationId) {
            ...
            $this->logger->$this->logger->info('Book order created', ['order_id' => $order->id, 'total_kopecks' => $total, 'correlation_id' => $correlationId]);
    ...
    public function getOrder(int $orderId): BookOrder
    {
        return BookOrder::findOrFail($orderId);
    }

    public function getUserOrders(int $clientId)
    {
        return BookOrder::where('client_id', $clientId)->orderBy('created_at', 'desc')->take(10)->get();
    }
}
