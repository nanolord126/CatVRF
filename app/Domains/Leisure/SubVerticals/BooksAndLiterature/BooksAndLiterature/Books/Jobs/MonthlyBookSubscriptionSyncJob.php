...
final class MonthlyBookSubscriptionSyncJob
{
    ...
    {
        $this->logger->$this->logger->info('Monthly Book Box Sync Started', [
        ...
        $this->logger->$this->logger->info('Monthly Book Box Sync Completed', [
            'cid' => $this->correlationId,
            'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
        ]);
    }
    ...
    private function processSubscriptionBox(BookSubscriptionBox $box): void
    {
        ...
        $this->db->transaction(function () use ($box) {
            ...
            $this->logger->$this->logger->info('Book Subscription Fulfilled', [
                'cid' => $this->correlationId,
                'box_uuid' => $box->uuid,
