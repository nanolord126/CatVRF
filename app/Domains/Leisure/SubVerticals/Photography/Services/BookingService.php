...
final readonly class BookingService
{
    ...
    public function createBooking(
        ...
        return $this->db->transaction(function () use ($clientId, $sessionId, $startsAt, $photographerId, $studioId, $correlationId) {
            ...
            $this->logger->$this->logger->info('Photography booking validation (Correlation: '.$correlationId.')', [
                'client_id' => $clientId,
            ...
            if ($photographerId) {
                $isBusy = Booking::where('photographer_id', $photographerId)
                    ...
                    ->exists();
                if ($isBusy) {
                    throw new \RuntimeException('Выбранный фотограф занят в это время.');
                }
            }
            ...
            $this->logger->$this->logger->info('Photography booking successfully stored (UUID: '.$booking->uuid.')', [
                'correlation_id' => $correlationId,
    ...
    public function reschedule(int $bookingId, string $newStartsAt, ?string $correlationId = null): bool
    {
        ...
        return $this->db->transaction(function () use ($bookingId, $newStartsAt, $correlationId) {
            ...
            $newStarts = Carbon::parse($newStartsAt);
            ...
            $this->logger->$this->logger->info('Photography booking rescheduling audit completed', [
