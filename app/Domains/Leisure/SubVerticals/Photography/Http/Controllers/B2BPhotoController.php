...
final class B2BPhotoController extends Controller
{
    ...
    public function updateStorefront(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Photography B2B: Storefront updated', [
                'storefront_id' => $id,
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function updateOrderStatus(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Photography B2B: Order status updated', [
                'order_id' => $id,
                'status' => $status,
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function rejectOrder(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Photography B2B: Order rejected', [
                'order_id' => $id,
                'reason' => $reason,
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function verifyInn(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($id) {
                ...
                $this->logger->$this->logger->info('Photography B2B: INN verified', [
                    'storefront_id' => $id,
                    'inn' => $storefront->inn,
                    'correlation_id' => Str::uuid(),
                ]);
