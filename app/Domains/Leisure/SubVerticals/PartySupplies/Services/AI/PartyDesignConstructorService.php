...
namespace App\Domains\Leisure\SubVerticals\PartySupplies\Services\AI;
...
/**
 * Тема праздника + декор + атрибутика + программа + смета
 * Вертикаль: party
 * Тип: party_theme_design
 *
 ...
 */
final readonly class PartyDesignConstructorService
{
     ...
     */
    public function analyzeAndRecommend(array $partyData, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'ai_constructor_party', amount: 0, correlationId: $correlationId ?? '');

        // Кэширование результата
        $cacheKey = "ai_party:party_theme_design:$userId:".md5(json_encode(func_get_args()));
        ...
        // 1. AI — анализ данных
        $inputData = array_merge($this->getInputData($partyData, $userId), ['party_profile' => true]);
        ...
        try {
            $response = $this->openai->chat([
                ['role' => 'system', 'content' => 'Разработка тематики и дизайна праздника с подбором атрибутики. Определи: тип праздника, количество гостей, возраст, бюджет, тематику. Рекомендуй декор, угощения, развлечения, атрибутику.'],
            ...
            throw new \RuntimeException('Failed to get party design. Please try again later.');
        }
        ...
        // 3. Разбор ответа AI
        $party_profile = $this->parseAnalysis($analysisText);

        // 4. Персонализация по вкусам
        $party_profile['taste_enrichment'] = $tasteProfile->toArray();
        ...
        $this->saveToUserProfile($userId, 'party', $party_profile, $correlationId);

        $result = [
            'success'        => true,
            'party_profile' => $party_profile,
            'recommendations' => $recommendations,
            'ar_link'        => url('party/design-preview/'.$userId),
            'correlation_id' => $correlationId,
        ];
        ...
        $this->logger->$this->logger->info('PartyDesignConstructorService used', [
            'user_id'        => $userId,
            'vertical'       => 'party',
            'type'           => 'party_theme_design',
            'correlation_id' => $correlationId,
        ]);
    ...
    {
        $patterns = [
            '/\b[A-ZА-Я][a-zа-я]+\s+[A-ZА-Я][a-zа-я]+\b/' => '[ИМЯ]',
            '/\b\d{11}\b/' => '[ТЕЛЕФОН]',
    ...
    private function getInputData(array $partyData, int $userId): array
    {
        return [
            'party_type' => $partyData['party_type'] ?? null,
            'guests_count' => $partyData['guests_count'] ?? null,
            'age_group' => $partyData['age_group'] ?? null,
