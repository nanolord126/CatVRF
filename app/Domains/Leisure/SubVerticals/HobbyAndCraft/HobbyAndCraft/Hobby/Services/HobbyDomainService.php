...
namespace App\Domains\Leisure\SubVerticals\HobbyAndCraft\Hobby\Services;
...
final readonly class HobbyDomainService
{
    ...
    public function upsertProduct(HobbyProductSaveDto $dto): HobbyProduct
    {
        ...
        $this->logger->$this->logger->info('Hobby Product Upsert Started', [
            'sku' => $dto->sku,
            'cid' => $cid,
        ...
        return $this->db->transaction(function () use ($dto, $cid) {
            ...
            $this->logger->$this->logger->info('Hobby Product Upsert Success', [
                'id' => $product->id,
                'cid' => $cid,
                'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
            ]);
     ...
     */
    public function createB2BOrder(VolumeOrderDto $dto): Model
    {
        $this->logger->$this->logger->info('B2B Craft Material Procurement Initiated', [
            'user_id' => $dto->userId,
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('B2B Wholesale Order Created', [
                'order_id' => $order,
                'total' => $orderAmount,
                'sku' => $product->sku,
                'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
