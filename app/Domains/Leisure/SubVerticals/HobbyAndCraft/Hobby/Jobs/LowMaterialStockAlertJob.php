...
namespace App\Domains\Leisure\SubVerticals\HobbyAndCraft\Hobby\Jobs;
...
final class LowMaterialStockAlertJob
{
    ...
    public function handle(): void
    {
        ...
        $this->logger->$this->logger->info('Low Stock Audit Started (Hobby)', [
            'cid' => $cid,
        ...
        try {
            ...
            if ($criticalMaterials->isEmpty()) {
                $this->logger->$this->logger->info('Hobby Inventory Check: All materials in stock.', [
                    'cid' => $cid,
                    'correlation_id' => $this->request->header('X-Correlation-ID', $this->correlationId ?? ''),
                ]);
            ...
            $this->logger->$this->logger->info('Low Stock Audit Completed (Hobby)', [
    ...
    {
        // Actual logic: $this->bus->dispatch(new SendHobbyStockNotification($store, $materials))
        $skus = $materials->pluck('sku')->toArray();

        $this->logger->$this->logger->info("Email notification queued for Store: {$store->name} (SKUs: ".implode(',', $skus).')');
    }
}
