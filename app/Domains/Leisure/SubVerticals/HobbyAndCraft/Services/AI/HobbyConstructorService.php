...
namespace App\Domains\Leisure\SubVerticals\HobbyAndCraft\Services\AI;
...
final readonly class HobbyConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'hobbyandcraft_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:HobbyAndCraft:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('HobbyAndCraft', $fullProfile, $userId);

            $this->logger->$this->logger->info('HobbyAndCraft AI constructor used', [
                'user_id'        => $userId,
                'correlation_id' => $correlationId,
                'vertical'       => 'HobbyAndCraft',
            ]);
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Подбор хобби-материалов. Определи уровень мастерства и подбери инструменты и материалы.',
            ];
        });
    }
}
