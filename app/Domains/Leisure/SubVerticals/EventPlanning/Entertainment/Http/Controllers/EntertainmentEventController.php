...
namespace App\Domains\Leisure\SubVerticals\EventPlanning\Entertainment\Http\Controllers;

use Psr\Log\LoggerInterface;
use App\Http\Controllers\Controller;
use App\Domains\EventPlanning\Entertainment\Models\EventReview;
...
final class EntertainmentEventController extends Controller
{
    ...
    public function update(int $id): JsonResponse
    {
        ...
        try {
            ...
            $this->db->transaction(function () use ($event, $correlationId) {
                ...
                $this->logger->$this->logger->info('Event updated', ['event_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function addSchedule(int $eventId): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($eventId, $correlationId) {
                ...
                $this->logger->$this->logger->info('Event schedule created', ['event_id' => $eventId, 'correlation_id' => $correlationId]);
            });
    ...
    public function updateSchedule(int $scheduleId): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($schedule, $correlationId) {
                ...
                $this->logger->$this->logger->info('Schedule updated', ['schedule_id' => $schedule->id, 'correlation_id' => $correlationId]);
            });
    ...
    public function cancelSchedule(int $scheduleId): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($schedule, $correlationId) {
                ...
                $this->logger->$this->logger->info('Schedule cancelled', ['schedule_id' => $scheduleId, 'correlation_id' => $correlationId]);
            });
    ...
    {
        try {
            $reviews = EventReview::where('entertainment_event_id', $id)
                ->with('reviewer')
                ->paginate(20);
    ...
    public function addReview(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($id, $correlationId) {
                ...
                $this->logger->$this->logger->info('Event review created', ['event_id' => $id, 'correlation_id' => $correlationId]);
            });
