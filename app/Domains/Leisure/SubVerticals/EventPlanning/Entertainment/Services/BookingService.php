...
namespace App\Domains\Leisure\SubVerticals\EventPlanning\Entertainment\Services;
...
final readonly class BookingService
{
    ...
    public function initiateBooking(Event $event, array $seats, ?int $userId, string $type = 'b2c'): Booking
    {
        ...
        $this->logger->$this->logger->info('Initiating entertainment booking', [
        ...
        return $this->db->transaction(function () use ($event, $seats, $userId, $type, $correlationId) {
            ...
            $eventRefresh = $this->eventDispatcher->where('id', $event->id)->lockForUpdate()->first();
            ...
            $this->logger->$this->logger->info('Booking created (Hold state)', [
    ...
    public function completeBooking(Booking $booking): bool
    {
        ...
        $this->logger->$this->logger->info('Completing entertainment booking', [
            'booking_uuid' => $booking->uuid,
            'correlation_id' => $correlationId,
        ]);
    ...
    public function cancelBooking(Booking $booking, string $reason = ''): void
    {
        ...
        $this->db->transaction(function () use ($booking, $correlationId) {
            ...
            // 1. Release capacity
            $event = $this->eventDispatcher->where('id', $lockingBooking->event_id)->lockForUpdate()->first();
            if ($event) {
            ...
            $this->logger->$this->logger->info('Booking cancelled and capacity released', [
                'booking_uuid' => $lockingBooking->uuid,
                'correlation_id' => $correlationId,
            ]);
        });
