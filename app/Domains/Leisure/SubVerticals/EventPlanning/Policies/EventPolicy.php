...
namespace App\Domains\Leisure\SubVerticals\EventPlanning\Policies;
...
final class EventPolicy
{
    ...
    {
        $this->logger->$this->logger->info('Security: UI Access - ViewAny Events', [
            'user_id' => $user->id,
            'tenant_id' => tenant()->id,
    ...
    {
        $this->logger->$this->logger->info('Security: UI Access - Create Event', [
            'user_id' => $user->id,
            'tenant_id' => tenant()->id,
            'correlation_id' => $this->request->header('X-Correlation-ID', $this->correlationId ?? ''),
        ]);
