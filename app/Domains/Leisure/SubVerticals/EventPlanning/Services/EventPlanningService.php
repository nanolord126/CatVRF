...
namespace App\Domains\Leisure\SubVerticals\EventPlanning\Services;
...
final readonly class EventPlanningService
{
    public function __construct(private readonly EventDispatcher $eventDispatcher,
        private readonly EventAIService $aiService,
    ...
    {
        $this->logger->$this->logger->info('EventPlanningService: Creating event project', [
        ...
        return $this->db->transaction(function () use ($inputData, $correlationId) {
            ...
            $event = $this->eventDispatcher->create([
            ...
            $this->logger->$this->logger->info("EventPlanningService: Event project #{$event->uuid} created successfully", [
                'correlation_id' => $correlationId,
                'event_id' => $event->id,
            ]);
    ...
    {
        $this->logger->$this->logger->info("EventPlanningService: Contracting vendor for event #{$event->uuid}", [
            'correlation_id' => $correlationId,
        ...
        return $this->db->transaction(function () use ($event, $vendorData, $correlationId) {
            ...
            if ($ev->agreed_price_kopecks > 0) {
                ...
                $this->walletService->hold($event->client_id, $depositAmount, BalanceTransactionType::HOLD, $correlationId, null, null, null);
    ...
    public function finishEvent(Event $event, string $correlationId): bool
    {
        ...
        $this->logger->$this->logger->info("EventPlanningService: Completing event #{$event->uuid}", [
            'correlation_id' => $correlationId,
        ]);

        return $this->db->transaction(function () use ($event) {
    ...
    public function cancelEvent(Event $event, string $reason, string $correlationId): bool
    {
        ...
        return $this->db->transaction(function () use ($event, $correlationId) {
            ...
            if ($event->cancellation_fee_kopecks > 0) {
                $this->walletService->debit($event->client_id, $event->cancellation_fee_kopecks, BalanceTransactionType::REFUND, $correlationId, null, null, null);
            }
