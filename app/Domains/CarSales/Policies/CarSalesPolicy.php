...
namespace App\Domains\CarSales\Policies;

use App\Models\User;
use App\Domains\CarSales\Models\CarDealerStorefront;
use App\Domains\CarSales\Models\CarSaleOrder;
...
final class B2BAutoPolicy
{
	...
	public function viewStorefront(User $user, B2BAutoStorefront $storefront): Response
	{
	...
	public function updateStorefront(User $user, B2BAutoStorefront $storefront): Response
	{
		return $user->tenant_id === $storefront->tenant_id || $user->is_admin
			? $this->response->allow()
			: $this->response->deny('Нет доступа');
	...
	public function viewOrder(User $user, B2BAutoOrder $order): Response
	{
	...
	public function approveOrder(User $user, B2BAutoOrder $order): Response
	{
	...
	public function rejectOrder(User $user, B2BAutoOrder $order): Response
	{
		return ($user->tenant_id === $order->tenant_id || $user->is_admin) && $order->status === 'pending'
			? $this->response->allow()
