...
final readonly class ConstructionMaterialService
{
    ...
    public function orderMaterial(
        ...
        try {
            ...
            return $this->db->transaction(function () use ($materialId, $quantity, $deliveryAddress, $correlationId, $userId, $tenantId) {
                ...
                // Log audit
                $this->logger->$this->logger->info('Construction material order created', [
                    'correlation_id' => $correlationId,
                    'order_id' => $order->id,
                    'material_id' => $materialId,
                    'quantity' => $quantity,
    ...
    public function deliverOrder(MaterialOrder $order, ?string $trackingNumber = null): void
    {
        ...
        try {
            $order->update([
                ...
                'delivery_date' => CarbonImmutable::now(),
            ]);

            $this->logger->$this->logger->info('Material order delivered', [
                'correlation_id' => $correlationId,
                'order_id' => $order->id,
            ]);
        } catch (\Throwable $e) {
    ...
    public function cancelOrder(MaterialOrder $order): void
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('Material order cancelled', [
                'correlation_id' => $correlationId,
                'order_id' => $order->id,
            ]);
        } catch (\Throwable $e) {
