...
namespace App\Domains\SportsEquipment\Services;
...
/**
 * Главный сервис вертикали SportsEquipment.
 *
 * Координирует спортивный инвентарь (fitness_equipment, sports_gear, outdoor_sports).
 * Является под-вертикалью HealthAndSports.
 */
final readonly class SportsEquipmentService
{
    ...
    /**
     * Подготовить чекаут для спортивного инвентаря.
     *
    ...
    /**
     * Создать заказ спортивного инвентаря.
     *
    ...
    public function createOrder(array $orderData): array
    {
        ...
        $this->logger->info('SportsEquipment order creation started', [
            'correlation_id' => $correlationId,
        ...
        return $this->db->transaction(function () use ($orderData, $correlationId) {
            ...
            $deliveryCalculation = $this->geoAdapter->calculateDeliveryForOrder([
                ...
                'heavy_cargo' => in_array($orderData['sub_vertical'] ?? '', ['fitness_equipment']),
            ]);

            $orderId = $this->db->table('sports_equipment_orders')->insertGetId([
            ...
            $paymentPreparation = $this->paymentAdapter->preparePayment([
                ...
                'vertical' => 'sports_equipment',
                'sub_vertical' => $orderData['sub_vertical'] ?? null,
                'payable_type' => 'sports_equipment_order',
                'payable_id' => $orderId,
            ]);
