...
namespace App\Domains\CustomerRetention\Services;
use App\Domains\CustomerRetention\Models\RetentionSpecialist;
use App\Domains\CustomerRetention\Models\RetentionProgram;
 ...
 * @version 1.0.0
 */
class CustomerRetentionService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){
    /**
...
return $this->db->transaction(function()use($specialistId,$programType,$hoursSpent,$dueDate,$correlationId){$s=RetentionSpecialist::findOrFail($specialistId);$total=(int)($s->price_kopecks_per_hour*$hoursSpent);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'retention','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$p=RetentionProgram::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'specialist_id'=>$specialistId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','program_type'=>$programType,'hours_spent'=>$hoursSpent,'due_date'=>$dueDate,'tags'=>['retention'=>true]]);$this->log->channel('audit')->info('Retention program created',['program_id'=>$p->id,'correlation_id'=>$correlationId]);return $p;});
}
public function completeProgram(int $programId,string $correlationId=""):RetentionProgram{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($programId,$correlationId){$p=RetentionProgram::findOrFail($programId);if($p->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$p->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$p->payout_kopecks,'retention_payout',['correlation_id'=>$correlationId,'program_id'=>$p->id]);$this->log->channel('audit')->info('Retention program completed',['program_id'=>$p->id]);return $p;});}
public function cancelProgram(int $programId,string $correlationId=""):RetentionProgram{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($programId,$correlationId){$p=RetentionProgram::findOrFail($programId);if($p->status==='completed')throw new \RuntimeException("Cannot cancel",400);$p->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($p->payment_status==='completed')$this->wallet->credit(tenant()->id,$p->total_kopecks,'retention_refund',['correlation_id'=>$correlationId,'program_id'=>$p->id]);$this->log->channel('audit')->info('Retention program cancelled',['program_id'=>$p->id]);return $p;});}
