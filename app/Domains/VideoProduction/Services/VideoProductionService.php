...
final class VideoProductionService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){}
...
return DB::transaction(function()use($producerId,$projectType,$productionHours,$dueDate,$correlationId){$p=VideoProducer::findOrFail($producerId);$total=(int)($p->price_kopecks_per_hour*$productionHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'video_project','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$v=VideoProject::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'producer_id'=>$producerId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','project_type'=>$projectType,'production_hours'=>$productionHours,'due_date'=>$dueDate,'tags'=>['video'=>true]]);Log::channel('audit')->info('Video project created',['project_id'=>$v->id,'correlation_id'=>$correlationId]);return $v;});
}
public function completeProject(int $projectId,string $correlationId=""):VideoProject{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($projectId,$correlationId){$v=VideoProject::findOrFail($projectId);if($v->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$v->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$v->payout_kopecks,'video_payout',['correlation_id'=>$correlationId,'project_id'=>$v->id]);Log::channel('audit')->info('Video project completed',['project_id'=>$v->id]);return $v;});}
