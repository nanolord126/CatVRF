...
final readonly class VideoProductionService
{
    ...
    public function completeProject(int $projectId,string $correlationId=""):VideoProject{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($projectId,$correlationId){$v=VideoProject::findOrFail($projectId);if($v->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$v->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$v->payout_kopecks,\App\Domains\Wallet\Enums\BalanceTransactionType::PAYOUT,$correlationId, null, null, ['correlation_id'=>$correlationId,'project_id'=>$v->id]);return $v;});}
    public function cancelProject(int $projectId,string $correlationId=""):VideoProject{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($projectId,$correlationId){$v=VideoProject::findOrFail($projectId);if($v->status==='completed')throw new \RuntimeException("Cannot cancel",400);$v->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($v->payment_status==='completed')$this->wallet->credit(tenant()->id,$v->total_kopecks,\App\Domains\Wallet\Enums\BalanceTransactionType::REFUND,$correlationId, null, null, ['correlation_id'=>$correlationId,'project_id'=>$v->id]);return $v;});}
