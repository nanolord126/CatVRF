 ...
 * @package App\Domains\Fitness\Jobs
 */
final class ProcessGymJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function __construct(
        ...
        $this->onQueue('fitness');
    }

    public function handle(AuditService $audit): void
    {
        $model = Gym::findOrFail($this->modelId);

        $this->logger->info('ProcessGymJob processed', [
            'model_id' => $model->id,
        ...
        $audit->log(
            action: 'fitness_job_processed',
            subjectType: Gym::class,
            subjectId: $model->id,
    ...
    {
        $this->logger->error('ProcessGymJob failed', [
