...
final readonly class OrderService
{
    ...
    public function getUserOrders(int $userId): Collection
    {
        ...
        $this->logger->$this->logger->info('Service method called in FashionRetail', ['correlation_id' => $correlationId]);
    ...
    public function calculateTotal(array $items): float
    {
        ...
        return new Collection($items)->sum(function ($item) {
    ...
    public function updateStatus(int $orderId, string $status, string $correlationId): void
    {
        ...
        $this->db->transaction(function () use ($orderId, $status, $correlationId) {
            ...
            $this->logger->$this->logger->info('FashionRetail order status updated', [
                'order_id' => $orderId,
                'status' => $status,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function cancelOrder(int $orderId, string $correlationId): void
    {
        ...
        $this->db->transaction(function () use ($orderId, $correlationId) {
            ...
            if (in_array($order->status, ['pending', 'confirmed'], true)) {
                ...
                $this->logger->$this->logger->info('FashionRetail order cancelled', [
                    'order_id' => $orderId,
                    'correlation_id' => $correlationId,
                ]);
            }
