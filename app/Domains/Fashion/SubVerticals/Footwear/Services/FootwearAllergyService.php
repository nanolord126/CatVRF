...
/**
 * FootwearAllergyService — Context-aware allergy checking for Footwear vertical.
 *
 * Healthcare compliance (152-ФЗ, ФЗ-323). Only shows relevant allergies
 * for footwear products (e.g., no food allergies for shoes).
 *
 * @version 2026.1
 */
final class FootwearAllergyService
{
    private const CACHE_TTL_DAYS = 7;

    /**
     * Relevant allergen types for footwear products.
     */
    private const FOOTWEAR_RELEVANT_ALLERGENS = [
    ...
    /**
     * Check if a footwear product is safe for user with given allergies.
     *
     * @param array $materials List of material names used in footwear
    ...
    /**
     * Get relevant allergen types for footwear context.
     */
    public function getRelevantAllergenTypes(): array
    {
        return self::FOOTWEAR_RELEVANT_ALLERGENS;
    }

    /**
     * Filter user allergies to only footwear-relevant ones.
     */
    private function filterRelevantAllergies(array $userAllergies): array
    {
        return array_intersect($userAllergies, self::FOOTWEAR_RELEVANT_ALLERGENS);
    }
    ...
    {
        $cacheKey = 'footwear:safe_materials:' . md5(json_encode($userAllergies));
    ...
    {
        $cacheKey = 'footwear:unsafe_materials:' . md5(json_encode($userAllergies));
        ...
        return Cache::remember($cacheKey, now()->addDays(self::CACHE_TTL_DAYS), function () use ($userAllergies) {
            ...
            foreach ($materials as $material) {
                if (!$this->checkMaterialSafety($material->name, $userAllergies)) {
                    ...
                    $relevantAllergenTypes = array_intersect($allergenTypes, self::FOOTWEAR_RELEVANT_ALLERGENS);
    ...
    public function logAllergyConflict(
        ...
        Log::warning('Footwear allergy conflict detected', [
            'user_id' => $userId,
    ...
    public function getAllergyWarningMessage(array $materials, array $userAllergies): ?string
    {
        ...
        return match ($severity) {
            MaterialAllergy::SEVERITY_MILD => "Эта обувь содержит материалы, которые могут вызвать легкую аллергическую реакцию.",
            MaterialAllergy::SEVERITY_MODERATE => "Внимание: эта обувь содержит {$conflictCount} материалов, которые могут вызвать аллергическую реакцию.",
            MaterialAllergy::SEVERITY_SEVERE => "Осторожно: эта обувь содержит материалы, которые могут вызвать тяжелую аллергическую реакцию.",
            MaterialAllergy::SEVERITY_LIFE_THREATENING => "ОПАСНОСТЬ: эта обувь содержит материалы, которые могут вызвать угрожающую жизни аллергическую реакцию.",
            default => null,
        };
    }
}
