...
final readonly class FashionMannequinSizeAlgorithmService
{
    ...
    public function calculateIdealSize(int $userId, int $tenantId, int $productId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($userId, $tenantId, $productId) {
            ...
            $this->log->info('Mannequin size calculation completed', [
                'user_id' => $userId,
    ...
    public function updateSizeAccuracy(int $userId, int $tenantId, int $productId, string $actualSize, string $feedback): bool
    {
        try {
            ...
            $this->db->table('fashion_size_feedback')->insert([
                'id' => uniqid(),
                'user_id' => $userId,
                'tenant_id' => $tenantId,
                'product_id' => $productId,
            ...
            $this->cache->tags(["fashion_mannequin_size:{$tenantId}:{$userId}"])->flush();

            $this->log->info('Size accuracy feedback recorded', [
                'user_id' => $userId,
                'product_id' => $productId,
                'feedback' => $feedback,
            ]);
            ...
            $this->log->error('Failed to record size feedback', [
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $feedback = $this->db->table('fashion_size_feedback')
            ...
            ->select('feedback', $this->db->raw('COUNT(*) as count'))
            ->groupBy('feedback')
            ->get()
            ->keyBy('feedback')
            ->toArray();
    ...
    public function trainSizeModel(int $tenantId): void
    {
        try {
            ...
            $feedbackData = $this->db->table('fashion_size_feedback')
                ->join('fashion_products', 'fashion_size_feedback.product_id', '=', 'fashion_products.id')
            ...
            $this->cache->tags(["fashion_mannequin_size:{$tenantId}"])->flush();

            $this->log->info('Size model training completed', [
                'tenant_id' => $tenantId,
                'feedback_records' => count($feedbackData),
            ]);
        } catch (\Exception $e) {
            $this->log->error('Failed to train size model', [
                'tenant_id' => $tenantId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    {
        $measurements = $this->db->table('fashion_user_size_profiles')
    ...
    private function estimateFromOrderHistory(int $userId, int $tenantId): array
    {
        ...
        $successfulOrders = $this->db->table('fashion_order_items')
    ...
    private function getAlternativeSizes(array $bestSize, array $sizeChart, array $tolerance): array
    {
        ...
        $alternatives = [];
        $currentIndex = array_search($bestSize['size'], array_column($sizeChart, 'size'), true);
    ...
    private function updateBrandConversionFactors(array $feedbackData, int $tenantId): void
    {
        ...
        foreach ($byBrand as $brandId => $stats) {
            ...
            $this->db->table('fashion_brand_adjustments')->updateOrInsert(
