...
final readonly class FashionReviewAggregationService
{
    ...
    public function aggregateProductReviews(int $productId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($productId, $tenantId) {
            $reviews = $this->db->table('fashion_reviews')
    ...
    {
        return $this->db->table('fashion_reviews')
            ->where('fashion_product_id', $productId)
            ->where('tenant_id', $tenantId)
            ->where('is_approved', true)
    ...
    {
        try {
            $this->db->table('fashion_review_helpfuls')->insert([
                'review_id' => $reviewId,
                'user_id' => $userId,
                'tenant_id' => $tenantId,
            ...
            // Clear cache
            $this->cache->tags(["fashion_reviews:{$tenantId}"])->flush();

            return true;
        } catch (\Exception $e) {
            $this->log->error('Failed to mark review as helpful', [
                'review_id' => $reviewId,
                'error' => $e->getMessage(),
            ]);
    ...
    public function moderateReview(int $reviewId, bool $approve, int $tenantId): bool
    {
        try {
            ...
            $this->log->info('Review moderated', [
                'review_id' => $reviewId,
                'approved' => $approve,
            ]);

            $this->logAction('fashion_review_moderated', 'FashionReview', $reviewId, [
                'approved' => $approve,
            ], null, $tenantId);

            return true;
        } catch (\Exception $e) {
            $this->log->error('Failed to moderate review', [
                'review_id' => $reviewId,
                'error' => $e->getMessage(),
            ]);
    ...
    public function getStoreReviewInsights(int $storeId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addHours(6), function () use ($storeId, $tenantId) {
            $reviews = $this->db->table('fashion_reviews')
