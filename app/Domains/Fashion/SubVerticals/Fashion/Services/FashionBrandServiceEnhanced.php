...
final readonly class FashionBrandServiceEnhanced
{
    ...
    public function getBrandAnalytics(int $brandId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, Carbon::now()->addSeconds(self::CACHE_TTL), function () use ($brandId, $tenantId) {
            $brand = $this->db->table('fashion_brands')
                ->where('id', $brandId)
            ...
            // Get product count
            $productCount = $this->db->table('fashion_products')
                ->where('brand_id', $brandId)
                ->where('tenant_id', $tenantId)
                ->count();

            // Get total sales
            $totalSales = $this->db->table('fashion_order_items')
                ...
                ->sum($this->db->raw('fashion_order_items.quantity * fashion_order_items.price'));

            // Get average rating
            $avgRating = $this->db->table('fashion_products')
                ->where('brand_id', $brandId)
                ->where('tenant_id', $tenantId)
                ->avg('rating') ?? 0;

            // Get top performing products
            $topProducts = $this->db->table('fashion_products')
                ->where('brand_id', $brandId)
                ->where('tenant_id', $tenantId)
    ...
    {
        return $this->db->table('fashion_brand_fit_profiles')
            ->where('brand_id', $brandId)
            ->where('tenant_id', $tenantId)
            ->get()
            ->toArray();
    ...
    {
        try {
            $this->db->table('fashion_brand_fit_profiles')->updateOrInsert(
            ...
            $this->log->info('Brand fit profile updated', [
                'brand_id' => $brandId,
                'tenant_id' => $tenantId,
            ]);
            ...
            $this->log->error('Failed to update brand fit profile', [
                'brand_id' => $brandId,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $score = $this->db->table('fashion_sustainability_scores')
            ->where('brand_id', $brandId)
            ->where('tenant_id', $tenantId)
            ->first();
    ...
    {
        $brands = $this->db->table('fashion_brands')
            ->whereIn('id', $brandIds)
            ->where('tenant_id', $tenantId)
            ->get()
            ->keyBy('id');
    ...
    public function getBrandRecommendations(int $userId, int $tenantId, int $limit = 5): array
    {
        ...
        $purchasedBrandIds = $this->db->table('fashion_order_items')
            ->join('fashion_orders', 'fashion_order_items.fashion_order_id', '=', 'fashion_orders.id')
            ->join('fashion_products', 'fashion_order_items.fashion_product_id', '=', 'fashion_products.id')
        ...
        $similarBrands = $this->db->table('fashion_products')
            ->whereIn('brand_id', $purchasedBrandIds)
            ->select('category_id', $this->db->raw('AVG(price_b2c) as avg_price'))
            ->groupBy('category_id')
            ->get();
        ...
        foreach ($similarBrands as $category) {
            $brands = $this->db->table('fashion_products')
                ...
                ->select('fashion_brands.id', 'fashion_brands.name', $this->db->raw('COUNT(*) as product_count'))
    ...
    private function getSalesTrend(int $brandId, int $tenantId): array
    {
        ...
        for ($i = 29; $i >= 0; $i--) {
            ...
            $sales = $this->db->table('fashion_order_items')
                ->join('fashion_orders', 'fashion_order_items.fashion_order_id', '=', 'fashion_orders.id')
                ->join('fashion_products', 'fashion_order_items.fashion_product_id', '=', 'fashion_products.id')
                ->where('fashion_products.brand_id', $brandId)
                ->where('fashion_orders.tenant_id', $tenantId)
