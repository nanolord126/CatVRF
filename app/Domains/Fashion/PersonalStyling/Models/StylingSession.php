...
/**
 * StylingSession — CatVRF 2026 Component.
 *
 ...
 *
 * @see https://catvrf.ru/docs/stylingsession
 */
...
final class StylingSession extends Model
{
    ...
    protected $table = 'styling_sessions';

    protected $fillable = ['uuid', 'tenant_id', 'stylist_id', 'client_id', 'correlation_id', 'status', 'total_kopecks', 'payout_kopecks', 'payment_status', 'style_type', 'session_hours', 'session_date', 'tags'];

    protected $casts = ['total_kopecks' => 'integer', 'payout_kopecks' => 'integer', 'session_hours' => 'integer', 'session_date' => 'datetime', 'tags' => 'json'];

    protected static function booted()
    {
        self::addGlobalScope('tenant', fn ($q) => $q->where('styling_sessions.tenant_id', tenant()->id));
    }
