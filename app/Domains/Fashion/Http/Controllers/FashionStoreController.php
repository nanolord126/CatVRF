...
final class FashionStoreController extends Controller
{
        ...
        try {
            $this->db->transaction(function () use ($correlationId) {
                FashionStore::create([
                    ...
                    'categories' => new Collection($request->input('categories', [])),
                    'is_active' => true,
                    'correlation_id' => $correlationId,
                ]);

                $this->logger->$this->logger->info('Fashion store created', [
                    'owner_id' => $request->user()?->id,
                    'name' => $request->input('name'),
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function update(int $id): JsonResponse
    {
        ...
        try {
            ...
            $this->db->transaction(function () use ($store, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion store updated', ['store_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function delete(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($store, $correlationId) {
                $store->delete();
                $this->logger->$this->logger->info('Fashion store deleted', ['store_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function verify(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($store, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion store verified', ['store_id' => $id, 'correlation_id' => $correlationId]);
            });
