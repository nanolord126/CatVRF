...
final class FashionReviewController extends Controller
{
    ...
    public function store(): JsonResponse
    {
        ...
        try {
            $this->db->transaction(function () use ($correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion review submitted', [
                    'product_id' => $request->input('product_id'),
                    'reviewer_id' => $request->user()?->id,
                    'correlation_id' => $correlationId,
                ]);
    ...
    public function update(int $id): JsonResponse
    {
        ...
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion review updated', ['review_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function delete(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                $review->delete();
                $this->logger->$this->logger->info('Fashion review deleted', ['review_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function markHelpful(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion review marked helpful', ['review_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function approve(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fashion review approved', ['review_id' => $id, 'correlation_id' => $correlationId]);
            });
    ...
    public function reject(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($review, $correlationId) {
                $review->delete();
                $this->logger->$this->logger->info('Fashion review rejected', ['review_id' => $id, 'correlation_id' => $correlationId]);
            });
