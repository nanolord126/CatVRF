    ...
    public function createStorefront(Request $request): JsonResponse
    {
        ...
        try {
            ...
            return $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('B2B Fashion storefront created', [
            ...
            $this->logger->error('Fashion storefront creation failed', [
                'error' => $e->getMessage(),
                'user_id' => $request->user()?->id,
    ...
    public function createOrder(Request $request): JsonResponse
    {
        ...
        try {
            ...
            return $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('B2B Fashion order created', [
                    'storefront_id' => $storefront->id,
                    'correlation_id' => $correlationId,
            ...
            $this->logger->error('Fashion order creation failed', [
                'error' => $e->getMessage(),
                'correlation_id' => $request->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
            ]);
    ...
    public function approveOrder(int $id): JsonResponse
    {
        try {
            return $this->db->transaction(function () use ($id) {
                ...
                $this->logger->$this->logger->info('Fashion order approved', [
                    'order_id' => $id,
                    'user_id' => $request->user()?->id,
                    'correlation_id' => $request->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
                ]);
    ...
    public function rejectOrder(int $id, Request $request): JsonResponse
    {
        try {
            ...
            return $this->db->transaction(function () use ($id, $validated) {
                ...
                $this->logger->$this->logger->info('Fashion order rejected', [
                    'order_id' => $id,
    ...
    public function verifyInn(int $id): JsonResponse
    {
        try {
            return $this->db->transaction(function () use ($id) {
                ...
                $this->logger->$this->logger->info('Fashion storefront verified', [
                    'storefront_id' => $id,
