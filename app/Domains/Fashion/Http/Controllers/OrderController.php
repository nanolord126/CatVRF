...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $fashionOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->fashionOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Fashion order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->fashionOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->fashionOrderService->getDeliveryEstimate($address);
