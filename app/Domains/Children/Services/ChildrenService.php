...
namespace App\Domains\Children\Services;
...
/**
 * Главный сервис вертикали Children.
 *
 * Координирует товары для детей (игрушки, одежда, детское питание)
 * с учётом возрастных ограничений и безопасности.
 */
final readonly class ChildrenService
{
    ...
    /**
     * Подготовить чекаут с проверкой возрастных ограничений.
     *
     * @param  array{items: array, seller_address: string, buyer_address: string, user_age?: int}  $cartData
     * @return array{delivery_cost: int, eta: int, age_restriction_warning: bool}
     */
    public function prepareCheckout(array $cartData): array
    {
        $correlationId = (string) Str::uuid();

        $this->logger->info('Children checkout preparation started', [
            'correlation_id' => $correlationId,
        ]);

        // Проверка возрастных ограничений для товаров
        $hasAgeRestrictedItems = $this->checkAgeRestrictedItems($cartData['items'] ?? []);
        $ageRestrictionWarning = $hasAgeRestrictedItems && ($cartData['user_age'] ?? 18) < 18;
    ...
    /**
     * Создать заказ с проверками безопасности.
     *
    ...
    public function createOrder(array $orderData): array
    {
        ...
        $this->logger->info('Children order creation started', [
            'correlation_id' => $correlationId,
        ...
        return $this->db->transaction(function () use ($orderData, $correlationId) {
            ...
            // Создание заказа
            $orderId = $this->db->table('children_orders')->insertGetId([
    ...
    /**
     * Проверить наличие товаров с возрастными ограничениями.
     */
    private function checkAgeRestrictedItems(array $items): bool
    {
        foreach ($items as $item) {
            if (($item['age_restriction'] ?? 0) >= 18) {
                return true;
            }
        }
        return false;
