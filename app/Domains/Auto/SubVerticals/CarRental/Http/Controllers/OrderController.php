...
use App\Domains\CarRental\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $carRentalOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $data = $request->all();
        $data['vertical'] = 'car_rental';

        $validation = $this->carRentalOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('CarRental order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->carRentalOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->carRentalOrderService->getDeliveryEstimate($address);
