    ...
    public function storefronts(): JsonResponse
    	...
    	public function createStorefront(Request $request): JsonResponse
    	{
    		try {
    			...
    			$this->db->transaction(function () use ($validated, $correlationId) {
    				...
    				$this->logger->info('Auto B2B: Storefront created', [
    					'inn' => $validated['inn'],
    					'correlation_id' => $correlationId,
    				]);
    			});
    	...
    	public function createOrder(Request $request): JsonResponse
    	{
    		try {
    			...
    			$this->db->transaction(function () use ($validated, $correlationId) {
    				...
    				$this->logger->info('Auto B2B: Order created', ['correlation_id' => $correlationId]);
    			});
    	...
    	public function approveOrder(int $id): JsonResponse
    	{
    		try {
    			...
    			$this->db->transaction(function () use ($order) {
    				...
    				$this->logger->info('Auto B2B: Order approved', ['order_id' => $order->id]);
    			});
       ...
       $validated = $request->all();
    			$this->db->transaction(function () use ($order, $validated) {
    				...
    				$this->logger->info('Auto B2B: Order rejected', ['order_id' => $order->id]);
    			});
    	...
    	public function verifyInn(int $id): JsonResponse
    	{
    		try {
    			...
    			$this->db->transaction(function () use ($id) {
    				...
    				$this->logger->info('Auto B2B: INN verified', ['storefront_id' => $id]);
    			});
