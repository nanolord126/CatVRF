...
final class CarSalesController extends Controller
{
    ...
    public function storefronts(): JsonResponse
    {
        try {
            ...
            $this->logger->error('Auto B2B: Storefronts failed', ['error' => $e->getMessage()]);
    ...
    public function createStorefront(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('Auto B2B: Storefront created', [
                    'inn' => $validated['inn'],
                    'correlation_id' => $correlationId,
                ]);
            });
    ...
    public function createOrder(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('Auto B2B: Order created', ['correlation_id' => $correlationId]);
            });
    ...
    {
        try {
            $orders = B2BAutoOrder::where('tenant_id', $request->user()->tenant_id)->latest()->paginate(20);
    ...
    public function approveOrder(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order) {
                ...
                $this->logger->$this->logger->info('Auto B2B: Order approved', ['order_id' => $order->id]);
            });
    ...
    public function rejectOrder(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order, $validated) {
                ...
                $this->logger->$this->logger->info('Auto B2B: Order rejected', ['order_id' => $order->id]);
            });
    ...
    public function verifyInn(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($id) {
                ...
                $this->logger->$this->logger->info('Auto B2B: INN verified', ['storefront_id' => $id]);
            });
