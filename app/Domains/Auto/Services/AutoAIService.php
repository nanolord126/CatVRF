...
final readonly class AutoAIService
{
     ...
     */
    public function estimateRepairFromPhoto(UploadedFile $photo, Vehicle $vehicle, string $correlationId): array
    {
        $this->logger->$this->logger->info('AI Repair Vision Started', [
        ...
        $this->logger->$this->logger->info('AI Repair Vision Finished', [
     ...
     */
    public function recommendVehicleForUser(int $userId, string $correlationId): Collection
    {
        $user = User::findOrFail($userId);
        $taste = $user->taste_profile ?? [];

        $this->logger->$this->logger->info('AI Vehicle Recommendation Started', [
            'user_id' => $userId,
            'correlation_id' => $correlationId,
        ]);
