...
final readonly class WebhookService
{
    ...
    public function trigger(int $tenantId, string $eventType, array $payload, string $correlationId): void
    {
        ...
        foreach ($webhooks as $webhook) {
            $this->bus->dispatch(function () use ($webhook, $eventType, $payload, $correlationId) {
                $this->deliver($webhook, $eventType, $payload, $correlationId);
            })->onQueue('webhooks');

            $this->eventDispatcher->dispatch(new WebhookTriggered($webhook, $eventType, $correlationId));
        }
    }
     ...
     */
    public function getWebhooks(int $tenantId): Collection
    {
     ...
     */
    public function getDeliveries(int $tenantId, ?int $webhookId = null): Collection
    {
    ...
    private function deliver(Webhook $webhook, string $eventType, array $payload, string $correlationId): void
    {
        ...
        try {
            ...
            $response = $this->http->timeout($webhook->timeout)
            ...
            $this->logger->channel('webhooks')->$this->logger->info('Webhook delivered', [
                'webhook_id' => $webhook->id,
            ...
            $delivery->update([
                ...
                'failed_at' => \Carbon\CarbonImmutable::now(),
                'retry_count' => $delivery->retry_count + 1,
                'next_retry_at' => \Carbon\CarbonImmutable::now()->addMinutes(pow(2, $delivery->retry_count)),
            ]);
