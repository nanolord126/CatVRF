...
use App\Models\User;
use App\Domains\Freelance\Models\B2BFreelanceStorefront;
use App\Domains\Freelance\Models\B2BFreelanceOrder;
...
final class B2BFreelancePolicy
{
    ...
    public function viewStorefront(User $user, B2BFreelanceStorefront $storefront): Response
    {
    ...
    public function updateStorefront(User $user, B2BFreelanceStorefront $storefront): Response
    {
        return $user->tenant_id === $storefront->tenant_id || $user->is_admin
            ? $this->response->allow()
            : $this->response->deny('Нет доступа');
    ...
    public function viewOrder(User $user, B2BFreelanceOrder $order): Response
    {
    ...
    public function approveOrder(User $user, B2BFreelanceOrder $order): Response
    {
    ...
    public function rejectOrder(User $user, B2BFreelanceOrder $order): Response
    {
        return ($user->tenant_id === $order->tenant_id || $user->is_admin) && $order->status === 'pending'
            ? $this->response->allow()
