...
final class ReviewController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            return $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('Freelance review submitted', [
                    'review_id' => $review->id,
                    'contract_id' => ($validated['contract_id'] ?? null),
                    'overall_rating' => ($validated['overall_rating'] ?? null),
                    'correlation_id' => $correlationId,
    ...
    public function markHelpful(int $id): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Review marked as helpful', [
                'review_id' => $id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function markUnhelpful(int $id): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Review marked as unhelpful', [
                'review_id' => $id,
                'correlation_id' => $correlationId,
            ]);
