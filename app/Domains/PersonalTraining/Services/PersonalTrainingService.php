...
namespace App\Domains\PersonalTraining\Services;
use App\Domains\PersonalTraining\Models\PersonalTrainer;
use App\Domains\PersonalTraining\Models\TrainingSession;
...
return $this->db->transaction(function()use($trainerId,$workoutType,$sessionHours,$sessionDate,$correlationId){$t=PersonalTrainer::findOrFail($trainerId);$total=(int)($t->price_kopecks_per_hour*$sessionHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'personal_training','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$s=Training$this->session->create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'trainer_id'=>$trainerId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','workout_type'=>$workoutType,'session_hours'=>$sessionHours,'session_date'=>$sessionDate,'tags'=>['training'=>true]]);$this->log->channel('audit')->info('Training session created',['session_id'=>$s->id,'correlation_id'=>$correlationId]);return $s;});
}
