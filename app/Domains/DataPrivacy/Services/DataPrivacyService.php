 ...
 * @version 1.0.0
 */
class DataPrivacyService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){
    /**
...
return $this->db->transaction(function()use($consultantId,$auditType,$hoursSpent,$dueDate,$correlationId){$c=PrivacyConsultant::findOrFail($consultantId);$total=(int)($c->price_kopecks_per_hour*$hoursSpent);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'privacy','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$a=PrivacyAudit::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'consultant_id'=>$consultantId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','audit_type'=>$auditType,'hours_spent'=>$hoursSpent,'due_date'=>$dueDate,'tags'=>['privacy'=>true]]);$this->log->channel('audit')->info('Privacy audit created',['audit_id'=>$a->id,'correlation_id'=>$correlationId]);return $a;});
}
public function completeAudit(int $auditId,string $correlationId=""):PrivacyAudit{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($auditId,$correlationId){$a=PrivacyAudit::findOrFail($auditId);if($a->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$a->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$a->payout_kopecks,'privacy_payout',['correlation_id'=>$correlationId,'audit_id'=>$a->id]);$this->log->channel('audit')->info('Privacy audit completed',['audit_id'=>$a->id]);return $a;});}
public function cancelAudit(int $auditId,string $correlationId=""):PrivacyAudit{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($auditId,$correlationId){$a=PrivacyAudit::findOrFail($auditId);if($a->status==='completed')throw new \RuntimeException("Cannot cancel",400);$a->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($a->payment_status==='completed')$this->wallet->credit(tenant()->id,$a->total_kopecks,'privacy_refund',['correlation_id'=>$correlationId,'audit_id'=>$a->id]);$this->log->channel('audit')->info('Privacy audit cancelled',['audit_id'=>$a->id]);return $a;});}
