...
namespace App\Domains\Supermarket\SubVerticals\MeatShops\Services;
...
final readonly class MeatService
{
    ...
    public function createOrder(int $supplierId, array $items, string $correlationId = ''): MeatOrder
    {
        ...
        return $this->db->transaction(function () use ($supplierId, $items, $correlationId) {
            ...
            $this->logger->$this->logger->info('Meat: order sent to butcher', ['order_id' => $order->id, 'total' => $totalPrice]);
    ...
    public function readyForDelivery(int $orderId, int $finalWeightGrams, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($order, $finalWeightGrams) {
            ...
            $this->logger->$this->logger->info('Meat: order weighed and ready', ['order_id' => $order->id, 'weight' => $finalWeightGrams]);
        });
    }
