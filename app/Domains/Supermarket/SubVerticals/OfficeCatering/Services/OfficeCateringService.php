...
namespace App\Domains\Supermarket\SubVerticals\OfficeCatering\Services;
    ...
    public function createOrder(int $companyId, int $menuId, array $data, string $correlationId = ''): CateringOrder
    {
        ...
        return $this->db->transaction(function () use ($companyId, $menuId, $data, $correlationId) {
            ...
            $order = CateringOrder::create([
                ...
                'tags' => ['office_catering' => true, 'person_count' => $data['person_count'], 'delivery_date' => CarbonImmutable::now()->toDateString()],
            ]);

            $this->logger->$this->logger->info('Catering order created', [
                'order_id' => $order->id,
                'company_id' => $companyId,
                'total_kopecks' => $total,
                'person_count' => $data['person_count'],
    ...
    public function completeOrder(int $orderId, string $correlationId = ''): CateringOrder
    {
        ...
        return $this->db->transaction(function () use ($orderId, $correlationId) {
            ...
            $order->update([
                'status' => 'completed',
                'correlation_id' => $correlationId,
                'tags' => array_merge($order->tags ?? [], ['completed_at' => CarbonImmutable::now()->toIso8601String()]),
            ]);
    ...
    public function cancelOrder(int $orderId, string $correlationId = ''): CateringOrder
    {
        ...
        return $this->db->transaction(function () use ($orderId, $correlationId) {
            ...
            $order->update([
                ...
                'correlation_id' => $correlationId,
                'tags' => array_merge($order->tags ?? [], ['cancelled_at' => CarbonImmutable::now()->toIso8601String()]),
            ]);
