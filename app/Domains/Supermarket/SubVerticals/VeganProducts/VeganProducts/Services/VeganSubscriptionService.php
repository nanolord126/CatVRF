...
namespace App\Domains\Supermarket\SubVerticals\VeganProducts\Services;
...
final readonly class VeganSubscriptionService
{
    ...
    public function subscribe(int $userId, int $boxId, string $planType, ?string $correlationId = null): void
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: New Vegan Subscription Request', [
            'user' => $userId,
            'box' => $boxId,
            'plan' => $planType,
        ...
        $this->db->transaction(function () use ($userId, $box, $planType, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Vegan Subscription CREATED', [
                'user' => $userId,
                'box' => $box->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function renewBatch(string $correlationId = ''): int
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Renewal Batch START', ['correlation_id' => $correlationId]);

        $activeSubs = $this->db->table('vegan_subscriptions')
            ->where('status', 'active')
            ->whereDate('next_delivery_at', '<=', CarbonImmutable::now())
            ->get();
    ...
    private function renewSingle($sub, string $correlationId): void
    {
        $this->db->transaction(function () use ($sub, $correlationId) {
            ...
            $this->db->table('vegan_subscriptions')
                ->where('id', $sub->id)
                ->update([
                    'last_delivery_at' => CarbonImmutable::now(),
