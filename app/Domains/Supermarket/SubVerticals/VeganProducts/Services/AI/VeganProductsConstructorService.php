...
namespace App\Domains\Supermarket\SubVerticals\VeganProducts\Services\AI;
...
final readonly class VeganProductsConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        return $this->cache->tags(['vegan', 'ai', 'constructor'])->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $this->logger->$this->logger->info('VeganProducts AI constructor used', [
                'user_id'        => $userId,
