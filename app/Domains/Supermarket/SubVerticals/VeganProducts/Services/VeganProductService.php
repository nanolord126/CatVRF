...
final readonly class VeganProductService
{
    ...
    public function createProduct(VeganProductCreateDto $dto): VeganProduct
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Creating vegan product', [
            'sku' => $dto->sku,
            'name' => $dto->name,
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Product created successfully', [
                'id' => $product->id,
                'correlation_id' => $correlationId,
            ]);
     ...
     */
    public function processOrder(VeganOrderProcessDto $dto): Order
    {
        $correlationId = $dto->correlationId ?: (string) Str::uuid();

        $this->logger->$this->logger->info('LAYER-3: Processing vegan order', [
            'user' => $dto->userId,
    ...
    public function adjustStock(int $productId, int $delta, string $reason, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($productId, $delta, $reason, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Stock Adjustment', [
                'sku' => $product->sku,
                'delta' => $delta,
                'reason' => $reason,
                'correlation_id' => $correlationId,
