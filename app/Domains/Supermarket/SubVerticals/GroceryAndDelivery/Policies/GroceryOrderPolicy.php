...
namespace App\Domains\Supermarket\SubVerticals\GroceryAndDelivery\Policies;
...
final class GroceryOrderPolicy
{
     ...
     */
    public function $this->viewFactory->make(User $user, GroceryOrder $order): bool
    {
        $canView = $user->id === $order->user_id || $user->isTenantAdmin();

        $this->logger->$this->logger->info('GroceryOrderPolicy::view checked', [
            'user_id' => $user->id,
            'order_id' => $order->id,
            'can_view' => $canView,
    ...
    public function update(User $user, GroceryOrder $order): bool
    {
        ...
        $this->logger->$this->logger->info('GroceryOrderPolicy::update checked', [
            'user_id' => $user->id,
            'order_id' => $order->id,
            'status' => $order->status,
    ...
    public function cancel(User $user, GroceryOrder $order): bool
    {
        ...
        $canCancel = $user->id === $order->user_id
            && in_array($order->status, ['pending', 'confirmed', 'in_transit'], true);
        ...
        $this->logger->$this->logger->info('GroceryOrderPolicy::cancel checked', [
            'user_id' => $user->id,
            'order_id' => $order->id,
            'status' => $order->status,
    ...
    public function delete(User $user, GroceryOrder $order): bool
    {
        ...
        $this->logger->$this->logger->info('GroceryOrderPolicy::delete checked', [
            'user_id' => $user->id,
            'order_id' => $order->id,
    ...
    public function confirm(User $user, GroceryOrder $order): bool
    {
        ...
        $this->logger->$this->logger->info('GroceryOrderPolicy::confirm checked', [
            'user_id' => $user->id,
            'order_id' => $order->id,
