...
use App\Events\FoodOrderPlacedEvent;
...
final readonly class FoodOrderingService
{
    ...
    public function placeOrder(CreateFoodOrderDto $dto): FoodOrder
    {
        ...
        return $this->db->transaction(function () use ($dto): FoodOrder {
            ...
            $this->eventDispatcher->dispatch(new FoodOrderPlacedEvent($order));
            ...
            $this->log->channel('audit')->$this->logger->info('Food order successfully placed', [
                'order_id' => $order->id,
                'total_price' => $totalPrice,
                'correlation_id' => $dto->correlationId,
            ]);
