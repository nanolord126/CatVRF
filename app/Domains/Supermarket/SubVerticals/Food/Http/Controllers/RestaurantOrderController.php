...
final class RestaurantOrderController extends Controller
{
    ...
    public function cancel(RestaurantOrder $order): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Order cancelled', ['order_id' => $order->id]);
    ...
    {
        try {
            $order->update(['status' => 'ready', 'ready_at' => CarbonImmutable::now()]);
    ...
    {
        try {
            $order->update(['status' => 'delivered', 'completed_at' => CarbonImmutable::now()]);
            $this->eventDispatcher->dispatch(new OrderCompleted($order, Str::uuid()->toString()));
