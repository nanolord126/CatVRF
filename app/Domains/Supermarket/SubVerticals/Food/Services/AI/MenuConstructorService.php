...
namespace App\Domains\Supermarket\SubVerticals\Food\Services\AI;
...
final readonly class MenuConstructorService
{
    ...
    public function generateMenuByPreferences(
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($ingredients, $diet, $minCalories, $maxCalories, $userId, $correlationId) {
            return $this->db->transaction(function () use ($ingredients, $diet, $minCalories, $maxCalories, $userId, $correlationId) {
                ...
                $readyDishes = new Collection($this->recommendation->getForUser(
                    userId: $userId,
                    vertical: 'food_ready',
                    context: $fullProfile
                ))->toArray();
                ...
                $homeIngredients = new Collection($this->recommendation->getForUser(
                    userId: $userId,
                    vertical: 'food_home',
                    context: $fullProfile
                ))->toArray();
                ...
                $this->logger->$this->logger->info('Food AI constructor completed', [
                    'user_id'        => $userId,
                    'diet'           => $diet,
    ...
    private function generateRecipes(array $profile, string $correlationId): array
    {
        ...
        $this->logger->$this->logger->info('Food recipe generation AI called', [
            'diet'           => $profile['diet'],
            'correlation_id' => $correlationId,
        ]);
