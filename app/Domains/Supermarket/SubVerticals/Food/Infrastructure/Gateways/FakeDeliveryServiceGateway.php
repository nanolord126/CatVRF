...
namespace App\Domains\Supermarket\SubVerticals\Food\Infrastructure\Gateways;
...
final readonly class FakeDeliveryServiceGateway
{
    ...
    {
        $this->logger->$this->logger->info('Scheduling delivery for order.', [
        ...
        $this->logger->$this->logger->info('Delivery scheduled successfully.', [
            'order_id' => $orderId->toString(),
    ...
    {
        $this->logger->$this->logger->info('Fetching delivery status.', [
            'delivery_id' => $deliveryId->toString(),
            'correlation_id' => $correlationId?->toString(),
        ]);
