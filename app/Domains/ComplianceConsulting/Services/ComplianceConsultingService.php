...
final class ComplianceConsultingService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){}
...
return DB::transaction(function()use($consultantId,$auditType,$auditHours,$dueDate,$correlationId){$c=ComplianceConsultant::findOrFail($consultantId);$total=(int)($c->price_kopecks_per_hour*$auditHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'compliance','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$a=ComplianceAudit::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'consultant_id'=>$consultantId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','audit_type'=>$auditType,'audit_hours'=>$auditHours,'due_date'=>$dueDate,'tags'=>['compliance'=>true]]);Log::channel('audit')->info('Compliance audit created',['audit_id'=>$a->id,'correlation_id'=>$correlationId]);return $a;});
}
public function completeAudit(int $auditId,string $correlationId=""):ComplianceAudit{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($auditId,$correlationId){$a=ComplianceAudit::findOrFail($auditId);if($a->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$a->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$a->payout_kopecks,'comp_payout',['correlation_id'=>$correlationId,'audit_id'=>$a->id]);Log::channel('audit')->info('Compliance audit completed',['audit_id'=>$a->id]);return $a;});}
public function cancelAudit(int $auditId,string $correlationId=""):ComplianceAudit{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($auditId,$correlationId){$a=ComplianceAudit::findOrFail($auditId);if($a->status==='completed')throw new \RuntimeException("Cannot cancel",400);$a->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($a->payment_status==='completed')$this->wallet->credit(tenant()->id,$a->total_kopecks,'comp_refund',['correlation_id'=>$correlationId,'audit_id'=>$a->id]);Log::channel('audit')->info('Compliance audit cancelled',['audit_id'=>$a->id]);return $a;});}
