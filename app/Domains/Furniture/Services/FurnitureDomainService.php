...
final readonly class FurnitureDomainService
{
    ...
    public function saveProduct(FurnitureProductDto $dto): FurnitureProduct
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Saving Furniture Product', [
            'sku' => $dto->sku,
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Product Saved Successfully', [
                'id' => $product->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function createCustomOrder(FurnitureCustomOrderDto $dto): FurnitureCustomOrder
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Initiating Custom Interior Order', [
            'user_id' => $dto->userId,
            'correlation_id' => $correlationId,
        ]);
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Custom Order Created', [
                'order_id' => $order->id,
                'correlation_id' => $correlationId,
            ]);
