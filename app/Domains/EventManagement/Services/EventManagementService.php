...
namespace App\Domains\EventManagement\Services;
use App\Domains\EventManagement\Models\EventCoordinator;
use App\Domains\EventManagement\Models\EventCoordination;
 ...
 * @version 1.0.0
 */
class EventManagementService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){
    /**
...
return $this->db->transaction(function()use($coordinatorId,$eventType,$coordinationHours,$eventDate,$correlationId){$c=EventCoordinator::findOrFail($coordinatorId);$total=(int)($c->price_kopecks_per_hour*$coordinationHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'event_mgmt','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$coord=EventCoordination::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'coordinator_id'=>$coordinatorId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','event_type'=>$eventType,'coordination_hours'=>$coordinationHours,'event_date'=>$eventDate,'tags'=>['event'=>true]]);$this->log->channel('audit')->info('Event coordination created',['coordination_id'=>$coord->id,'correlation_id'=>$correlationId]);return $coord;});
}
public function completeCoordination(int $coordinationId,string $correlationId=""):EventCoordination{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($coordinationId,$correlationId){$c=EventCoordination::findOrFail($coordinationId);if($c->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$c->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$c->payout_kopecks,'event_payout',['correlation_id'=>$correlationId,'coordination_id'=>$c->id]);$this->log->channel('audit')->info('Event coordination completed',['coordination_id'=>$c->id]);return $c;});}
public function cancelCoordination(int $coordinationId,string $correlationId=""):EventCoordination{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($coordinationId,$correlationId){$c=EventCoordination::findOrFail($coordinationId);if($c->status==='completed')throw new \RuntimeException("Cannot cancel",400);$c->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($c->payment_status==='completed')$this->wallet->credit(tenant()->id,$c->total_kopecks,'event_refund',['correlation_id'=>$correlationId,'coordination_id'=>$c->id]);$this->log->channel('audit')->info('Event coordination cancelled',['coordination_id'=>$c->id]);return $c;});}
