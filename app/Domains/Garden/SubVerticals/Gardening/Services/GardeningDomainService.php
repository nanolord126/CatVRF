...
namespace App\Domains\Garden\SubVerticals\Gardening\Services;
...
final readonly class GardeningDomainService
{
    ...
    public function saveProduct(ProductSaveDto $dto, int $tenantId): GardenProduct
    {
        ...
        return $this->db->transaction(function () use ($dto, $tenantId, $correlationId) {
            ...
            $this->logger->$this->logger->info('Gardening product persisted', [
                'sku' => $product->sku,
                'cid' => $correlationId,
                'tenant' => $tenantId,
    ...
    public function updateSubscriptionBox(SubscriptionBoxDto $dto, int $tenantId): GardenSubscriptionBox
    {
        ...
        return $this->db->transaction(function () use ($dto, $tenantId, $correlationId) {
            ...
            $this->logger->$this->logger->info('Gardening subscription box updated', [
                'box_id' => $box->id,
                'cid' => $correlationId,
                'tenant' => $tenantId,
                'correlation_id' => $this->request?->header('X-Correlation-ID', \Illuminate\Support\Str::uuid()->toString()),
    ...
    public function isPlantInSeason(GardenPlant $plant): bool
    {
        ...
        if (isset($calendar['active_months']) && is_array($calendar['active_months'])) {
            return in_array($month, $calendar['active_months'], true);
        }
