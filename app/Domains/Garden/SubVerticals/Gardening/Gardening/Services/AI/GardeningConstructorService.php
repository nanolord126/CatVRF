...
namespace App\Domains\Garden\SubVerticals\Gardening\Services\AI;
...
final readonly class GardeningConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'gardening_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:Gardening:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('Gardening', $fullProfile, $userId);

            $this->logger->$this->logger->info('Gardening AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Планировщик сада. Определи тип участка, климат и предпочтения.',
            ];
        });
    }
}
