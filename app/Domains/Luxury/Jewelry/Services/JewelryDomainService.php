...
final readonly class JewelryDomainService
{
    ...
    public function saveProduct(JewelryProductDto $dto): JewelryProduct
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Saving Jewelry Product', [
            'sku' => $dto->sku,
            'name' => $dto->name,
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Jewelry Product Saved Successfully', [
                'id' => $product->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function createCustomOrder(JewelryCustomOrderDto $dto): JewelryCustomOrder
    {
        ...
        $this->logger->$this->logger->info('LAYER-3: Creating Custom Jewelry Order', [
            'store_id' => $dto->storeId,
            'user_id' => $dto->userId,
            'correlation_id' => $correlationId,
        ]);
        ...
        return $this->db->transaction(function () use ($dto, $correlationId) {
            ...
            $this->logger->$this->logger->info('LAYER-3: Custom Jewelry Order Created Successfully', [
                'id' => $order->id,
                'uuid' => $order->uuid,
                'correlation_id' => $correlationId,
            ]);
