...
final readonly class AIHealthConstructorService
{
    ...
    public function matchSymptomToService(string $symptoms, int $tenantId, ?string $correlationId = null): array
    {
        ...
        try {
            ...
            if (Str::contains(mb_strtolower($symptoms), ['зуб', 'десна'])) {
                $suggestedSpecialization = 'Dentist';
            }
            if (Str::contains(mb_strtolower($symptoms), ['глаз', 'зрение'])) {
                $suggestedSpecialization = 'Ophthalmologist';
            }
            if (Str::contains(mb_strtolower($symptoms), ['сердце', 'давление'])) {
            ...
            $this->logger->$this->logger->info('AI Health Constructor matched symptoms', [
                'symptoms' => $symptoms,
     ...
     */
    public function analyzeVisionCondition(UploadedFile $photo, ?string $correlationId = null): array
    {
        $correlationId = $correlationId ?? (string) Str::uuid();

        // КРИТИЧНО: Логируем факт анализа фото - это конфиденциальные данные!
        $this->logger->$this->logger->info('Vision AI Analysis requested for patient photo', [
