...
final class ServiceJobController extends Controller
{
    ...
    public function create(): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('HomeService job created', [
            ...
            SendJobReminderJob::$this->bus->dispatch($job->id, $correlationId);
    ...
    public function myJobs(): JsonResponse
    {
        try {
            $jobs = ServiceJob::where('client_id', $request->user()?->id)
                ->orWhere('contractor_id', Contractor::where('user_id', $request->user()?->id)->value('id'))
    ...
    public function accept(int $id): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('HomeService job accepted', [
                'correlation_id' => $correlationId,
                'job_id'         => $job->id,
                'user_id'        => $request->user()?->id,
            ]);
    ...
    public function start(int $id): JsonResponse
    {
        try {
            ...
            $job->update(['status' => 'in_progress', 'started_at' => CarbonImmutable::now(), 'correlation_id' => $correlationId]);

            $this->logger->$this->logger->info('HomeService job started', [
                'correlation_id' => $correlationId,
                'job_id'         => $job->id,
                'user_id'        => $request->user()?->id,
            ]);
