...
final class B2BHomeServiceController
{
    public function storefronts(): JsonResponse
    {
        try {
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Failed to fetch storefronts', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function createStorefront(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->log->channel('audit')->info('HomeServices B2B: Storefront created', [
                    'inn' => $validated['inn'],
                    'correlation_id' => $correlationId,
                ]);
            });
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Storefront creation failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function createOrder(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId, $commission) {
                ...
                $this->log->channel('audit')->info('HomeServices B2B: Order created', [
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Order creation failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function myB2BOrders(): JsonResponse
    {
        try {
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Failed to fetch orders', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function approveOrder(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order, $correlationId) {
                ...
                $this->log->channel('audit')->info('HomeServices B2B: Order approved', [
                    'order_id' => $order->id,
                    'correlation_id' => $correlationId,
                ]);
            });
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Order approval failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function rejectOrder(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order, $correlationId, $reason) {
                ...
                $this->log->channel('audit')->info('HomeServices B2B: Order rejected', [
                    'order_id' => $order->id,
                    'reason' => $reason,
                    'correlation_id' => $correlationId,
                ]);
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Order rejection failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function verifyInn(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($storefront, $correlationId) {
                ...
                $this->log->channel('audit')->info('HomeServices B2B: Storefront verified', [
            ...
            $this->log->channel('audit')->error('HomeServices B2B: Verification failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
