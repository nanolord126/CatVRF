...
final class B2BHomeServiceController extends Controller
{
    ...
    public function storefronts(): JsonResponse
    {
        try {
            ...
            $this->logger->error('HomeServices B2B: Failed to fetch storefronts', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function createStorefront(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId) {
                ...
                $this->logger->$this->logger->info('HomeServices B2B: Storefront created', [
                    'inn' => $validated['inn'],
                    'correlation_id' => $correlationId,
                ]);
            });
            ...
            $this->logger->error('HomeServices B2B: Storefront creation failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function createOrder(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($validated, $correlationId, $commission) {
                ...
                $this->logger->$this->logger->info('HomeServices B2B: Order created', [
            ...
            $this->logger->error('HomeServices B2B: Order creation failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    {
        try {
            $orders = B2BHomeServiceOrder::where('tenant_id', $request->user()->tenant_id)
                ->latest()
                ->paginate(20);
            ...
            $this->logger->error('HomeServices B2B: Failed to fetch orders', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function approveOrder(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order, $correlationId) {
                ...
                $this->logger->$this->logger->info('HomeServices B2B: Order approved', [
                    'order_id' => $order->id,
                    'correlation_id' => $correlationId,
                ]);
            });
            ...
            $this->logger->error('HomeServices B2B: Order approval failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function rejectOrder(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($order, $correlationId, $reason) {
                ...
                $this->logger->$this->logger->info('HomeServices B2B: Order rejected', [
                    'order_id' => $order->id,
                    'reason' => $reason,
                    'correlation_id' => $correlationId,
                ]);
            ...
            $this->logger->error('HomeServices B2B: Order rejection failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
    ...
    public function verifyInn(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($storefront, $correlationId) {
                ...
                $this->logger->$this->logger->info('HomeServices B2B: Storefront verified', [
            ...
            $this->logger->error('HomeServices B2B: Verification failed', [
                'error' => $e->getMessage(),
                'correlation_id' => Str::uuid(),
            ]);
