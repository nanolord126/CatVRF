...
final readonly class ConstructionService
{
    ...
    public function createProject(array $data, int $tenantId): ConstructionProject
    {
        ...
        return $this->db->transaction(function () use ($data, $tenantId) {
            // Check if address is in delivery zone for materials
            $isInZone = true;
            ...
            $project = ConstructionProject::create([
                ...
                'delivery_zone_verified' => $isInZone,
                'correlation_id' => $this->correlationId,
            ]);

            $this->logger->$this->logger->info('Construction project created', [
    ...
    {
        return $this->db->transaction(function () use ($materialId, $usage) {
            ...
            $this->logger->$this->logger->info('Material deducted from project', [
