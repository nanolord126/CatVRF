...
namespace App\Domains\OfficeCatering\Services\AI;
    ...
    public function analyzeAndRecommend(array $cateringData, int $userId): array
    {
        ...
        $analysis = $this->openai->chat()->create([
            ...
            'messages' => [
                ...
                ['role' => 'user', 'content' => json_encode(array_merge($this->getInputData($cateringData, $userId), ['catering_profile' => true]))],
            ],
            'max_tokens' => 1024,
        ]);
        ...
        $this->logger->info('OfficeCateringConstructorService used', [
            'user_id'        => $userId,
            'vertical'       => 'catering',
            'type'           => 'corporate_menu',
            'correlation_id' => $correlationId,
