...
final readonly class RealEstateDesignConstructorService
{
    ...
    public function analyzePropertyAndRecommend(array $data, int $userId): array
    {
        ...
        try {
            $this->log->channel('audit')->info('real_estate.ai.constructor.start', [
            ...
            $result = $this->cache->remember($cacheKey, self::CACHE_TTL_SECONDS, function () use ($data, $userId, $correlationId) {
                ...
                $this->db->transaction(function () use ($userId, $analysis, $recommendations, $correlationId) {
            ...
            $this->log->channel('audit')->info('real_estate.ai.constructor.success', [
                'correlation_id' => $correlationId,
                'user_id' => $userId,
            ]);
            ...
            $this->log->channel('audit')->critical('real_estate.ai.constructor.error', [
                'correlation_id' => $correlationId,
                'user_id' => $userId,
                'error' => $e->getMessage(),
    ...
    {
        $climateFactor = in_array($city, ['Москва', 'Санкт-Петербург', 'Новосибирск'], true) ? 'cold' : 'moderate';
    ...
    {
        return [
            '3d_model_url' => 'https://3d.catvrf.ru/models/'.Str::random(16),
            'ar_ready' => true,
            'vr_ready' => true,
            'render_quality' => 'high',
    ...
    private function calculateRenovationCost(array $analysis): array
    {
        ...
        return [
            ...
            'breakdown' => [
                'demolition' => (int)round($totalCost * 0.05),
    ...
    private function estimateRenovationTimeline(array $analysis): array
    {
        ...
        return [
            ...
            'phases' => [
                ...
                'demolition' => (int)ceil($totalWeeks * 0.1),
