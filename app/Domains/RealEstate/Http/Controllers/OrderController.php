...
use App\Domains\RealEstate\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $realEstateOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $data = $request->all();
        $data['vertical'] = 'real_estate';

        $validation = $this->realEstateOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('RealEstate order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->realEstateOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->realEstateOrderService->getDeliveryEstimate($address);
