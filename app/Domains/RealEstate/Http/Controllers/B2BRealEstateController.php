...
final class B2BRealEstateController extends Controller
{
    ...
    public function createStorefront(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(fn () => B2BRealEstateStorefront::create(['uuid' => Str::uuid(), 'tenant_id' => $request->user()->tenant_id] + $v + ['correlation_id' => $cId]) && $this->logger->$this->logger->info('RealEstate B2B: Storefront created', ['inn' => $v['inn'], 'correlation_id' => $cId]));
    ...
    public function createOrder(Request $request): JsonResponse
    {
        try {
            ...
            $cId = Str::uuid()->toString();
            $this->db->transaction(fn () => B2BRealEstateOrder::create(['uuid' => Str::uuid(), 'tenant_id' => $request->user()->tenant_id, 'order_number' => 'B2B-'.Str::random(8), 'commission_amount' => (int) ($v['total_amount'] * 0.14), 'status' => 'pending'] + $v + ['correlation_id' => $cId]) && $this->logger->$this->logger->info('RealEstate B2B: Order created', ['correlation_id' => $cId]));
