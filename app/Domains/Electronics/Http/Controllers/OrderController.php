...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $electronicsOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->electronicsOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Electronics order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->electronicsOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->electronicsOrderService->getDeliveryEstimate($address);
