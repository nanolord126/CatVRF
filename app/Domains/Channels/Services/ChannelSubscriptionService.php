...
use App\Domains\Channels\Models\ChannelSubscriber;
...
final class ChannelSubscriptionService
{
    ...
    public function subscribe(
        ...
        return DB::transaction(function () use ($userId, $channel, $visibilityPreference, $correlationId): ChannelSubscriber {
            ...
            event(new \App\Domains\Channels\Events\ChannelSubscribed($channel, $userId, $correlationId));
    ...
    public function getPersonalFeed(int $userId, string $audience = 'all', int $perPage = 15)
    {
        ...
        if ($subscribedChannelIds->isEmpty()) {
            return \App\Domains\Channels\Models\Post::withoutGlobalScopes()->whereRaw('1=0')->paginate($perPage);
        }

        return \App\Domains\Channels\Models\Post::withoutGlobalScopes()
