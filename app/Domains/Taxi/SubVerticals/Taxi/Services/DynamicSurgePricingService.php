...
final readonly class DynamicSurgePricingService
{
    ...
    public function triggerSurge(int $zoneId, float $multiplier, string $reason, string $correlationId): void
    {
        ...
        $this->db->transaction(function () use ($zone, $multiplier, $reason, $correlationId) {
            $zone->update([
            ...
            $this->log->channel('audit')->info('Surge triggered', [
                'correlation_id' => $correlationId,
                'zone_id' => $zone->id,
                'multiplier' => $multiplier,
            ]);
    ...
    public function decaySurge(int $zoneId, string $correlationId): void
    {
        ...
        $this->db->transaction(function () use ($zone, $correlationId) {
            $newMultiplier = max(
    ...
    {
        $nearbyEvents = $this->db->table('events')
    ...
    private function getWeatherData(float $latitude, float $longitude): array
    {
        ...
        $this->redis->setex($cacheKey, 1800, json_encode($weather));
