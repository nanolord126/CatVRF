...
final readonly class DriverMatchingService
{
    ...
    public function findBestDrivers(
        ...
        $this->redis->setex($cacheKey, self::CACHE_TTL, json_encode($topDrivers));
    ...
    public function assignDriver(int $rideId, int $driverId, string $correlationId): void
    {
        $this->db->transaction(function () use ($rideId, $driverId, $correlationId) {
            ...
            $this->log->channel('audit')->info('Driver assigned to ride', [
                'correlation_id' => $correlationId,
                'ride_id' => $rideId,
                'driver_id' => $driverId,
            ]);
    ...
    {
        $this->db->transaction(function () use ($driverId, $correlationId) {
            $this->db->table('taxi_drivers')
                ->where('id', $driverId)
                ->update([
                    'status' => 'available',
    ...
    {
        $previousRides = $this->db->table('taxi_rides')
