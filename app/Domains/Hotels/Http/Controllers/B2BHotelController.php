...
final class B2BHotelController
{
	...
	public function createStorefront(Request $request): JsonResponse
	{
		try {
			...
			$this->db->transaction(fn() => B2BHotelStorefront::create(['uuid' => Str::uuid(), 'tenant_id' => auth()->user()->tenant_id] + $v + ['correlation_id' => $cId]) && $this->log->channel('audit')->info('Hotel B2B: Storefront created', ['inn' => $v['inn'], 'correlation_id' => $cId]));
	...
	public function createOrder(Request $request): JsonResponse
	{
		try {
			...
			$cId = Str::uuid()->toString();
			$this->db->transaction(fn() => B2BHotelOrder::create(['uuid' => Str::uuid(), 'tenant_id' => auth()->user()->tenant_id, 'order_number' => 'B2B-' . Str::random(8), 'commission_amount' => (int)($v['total_amount'] * 0.14), 'status' => 'pending'] + $v + ['correlation_id' => $cId]));
