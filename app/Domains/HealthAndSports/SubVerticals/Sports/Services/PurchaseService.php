...
final readonly class PurchaseService
{
    ...
    public function createPurchase(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Creating purchase', [
                'studio_id' => $studioId,
                'member_id' => $memberId,
                'item_type' => $itemType,
            ...
            $purchase = $this->db->transaction(function () use (
                ...
                PurchaseCreated::$this->bus->dispatch($purchase, $correlationId);

                return $purchase;
            });

            $this->logger->$this->logger->info('Purchase created successfully', [
    ...
    public function confirmPayment(Purchase $purchase, string $transactionId, ?string $correlationId = null): void
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('Confirming purchase payment', [
                'purchase_id' => $purchase->id,
            ...
            $this->logger->$this->logger->info('Purchase payment confirmed', [
                'purchase_id' => $purchase->id,
                'correlation_id' => $correlationId,
            ]);
        } catch (Throwable $e) {
    ...
    public function refundPurchase(Purchase $purchase, string $reason = '', ?string $correlationId = null): void
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('Refunding purchase', [
                'purchase_id' => $purchase->id,
                'reason' => $reason,
                'correlation_id' => $correlationId,
            ]);
            ...
            $this->db->transaction(function () use ($purchase, $reason, $correlationId) {
                ...
                PurchaseRefunded::$this->bus->dispatch($purchase, $reason, $correlationId);
            });

            $this->logger->$this->logger->info('Purchase refunded', [
                'purchase_id' => $purchase->id,
                'correlation_id' => $correlationId,
            ]);
        } catch (Throwable $e) {
