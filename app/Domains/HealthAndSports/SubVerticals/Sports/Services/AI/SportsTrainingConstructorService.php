...
/**
 * Персональный план тренировок + подбор экипировки + расписание
 * Вертикаль: sports
 * Тип: training_plan
 *
 ...
 */
final readonly class SportsTrainingConstructorService
{
     ...
     */
    public function analyzeAndRecommend(array $athleteData, int $userId, string $sport = 'general'): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'ai_constructor_sports', amount: 0, correlationId: $correlationId ?? '');

        // Кэширование результата
        $cacheKey = "ai_sports:training_plan:$userId:".md5(json_encode(func_get_args()));
        ...
        // 1. AI — анализ данных
        $inputData = array_merge($this->getInputData($athleteData, $userId), ['training_profile' => true]);
        ...
        try {
            $response = $this->openai->chat([
                ['role' => 'system', 'content' => 'Анализ физической формы и целей для составления тренировочного плана. Определи: уровень подготовки, вид спорта, цели, ограничения. Рекомендуй план тренировок, экипировку, инвентарь.'],
            ...
            throw new \RuntimeException('Failed to get training plan. Please try again later.');
        }
        ...
        // 3. Разбор ответа AI
        $training_profile = $this->parseAnalysis($analysisText);

        // 4. Персонализация по вкусам
        $training_profile['taste_enrichment'] = $tasteProfile->toArray();
        ...
        $this->saveToUserProfile($userId, 'sports', $training_profile, $correlationId);

        $result = [
            'success'        => true,
            'training_profile' => $training_profile,
            'recommendations' => $recommendations,
            'ar_link'        => url('sports/training-preview/'.$userId),
            'correlation_id' => $correlationId,
        ];
        ...
        $this->logger->$this->logger->info('SportsTrainingConstructorService used', [
            'user_id'        => $userId,
            'vertical'       => 'sports',
            'type'           => 'training_plan',
            'correlation_id' => $correlationId,
    ...
    {
        $patterns = [
            '/\b[A-ZА-Я][a-zа-я]+\s+[A-ZА-Я][a-zа-я]+\b/' => '[АТЛЕТ]',
            '/\b\d{11}\b/' => '[ТЕЛЕФОН]',
    ...
    private function getInputData(array $athleteData, int $userId): array
    {
        return [
            'fitness_level' => $athleteData['fitness_level'] ?? null,
            'goals' => $athleteData['goals'] ?? [],
            'limitations' => $athleteData['limitations'] ?? [],
            'sport' => $athleteData['sport'] ?? null,
        ];
    }
}
