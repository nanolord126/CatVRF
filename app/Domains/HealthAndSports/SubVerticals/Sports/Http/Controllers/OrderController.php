...
use App\Domains\Sports\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $sportsOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->sportsOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Sports order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->sportsOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->sportsOrderService->getDeliveryEstimate($address);
