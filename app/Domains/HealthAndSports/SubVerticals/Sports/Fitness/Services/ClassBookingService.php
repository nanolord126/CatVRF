    ...
    public function bookFitnessClass(
        ...
        try {
            ...
            $bookingId = $this->db->transaction(function () use ($classId, $userId, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fitness class booked', [
        ...
        try {
            ...
            $this->db->transaction(function () use ($bookingId, $correlationId) {
                $this->db->table('class_bookings')
                    ->where('id', $bookingId)
                    ->update(['status' => 'cancelled', 'cancelled_at' => CarbonImmutable::now()]);

                $this->logger->$this->logger->info('Fitness class booking cancelled', [
                    'booking_id' => $bookingId,
                    'correlation_id' => $correlationId,
                ]);
            });
