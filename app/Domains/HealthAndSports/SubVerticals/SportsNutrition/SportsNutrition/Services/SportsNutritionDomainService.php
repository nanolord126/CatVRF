...
namespace App\Domains\HealthAndSports\SubVerticals\SportsNutrition\Services;
...
final readonly class SportsNutritionDomainService
{
    ...
    {
        $cid = $cid ?? (string) Str::uuid();

        $this->logger->$this->logger->info('Supplement product update requested', [
            'cid' => $cid,
            'sku' => $dto->sku,
        ...
        return $this->db->transaction(function () use ($dto, $cid) {
            ...
            if ($expiry->isPast() || $expiry->diffInDays(CarbonImmutable::now()) < 30) {
            ...
            $this->logger->$this->logger->info('Supplement product saved successfully.', [
                'cid' => $cid,
                'pid' => $product->id,
                'correlation_id' => $this->request?->header('X-Correlation-ID', Str::uuid()->toString()),
            ]);
    ...
    public function useConsumable(int $consumableId, float $kgValue, string $reason, ?string $cid = null): void
    {
        ...
        $this->db->transaction(function () use ($consumableId, $kgValue, $reason, $cid) {
            ...
            $this->logger->$this->logger->info('Supplement raw material deducted', [
                'cid' => $cid,
                'item' => $consumableId,
                'kg' => $kgValue,
                'before' => $currentStock,
