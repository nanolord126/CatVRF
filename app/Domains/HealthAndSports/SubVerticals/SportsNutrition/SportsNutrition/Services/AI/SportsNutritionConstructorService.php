...
namespace App\Domains\HealthAndSports\SubVerticals\SportsNutrition\Services\AI;
...
final readonly class SportsNutritionConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'sportsnutrition_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:SportsNutrition:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('SportsNutrition', $fullProfile, $userId);

            $this->logger->$this->logger->info('SportsNutrition AI constructor used', [
                'user_id'        => $userId,
                'correlation_id' => $correlationId,
                'vertical'       => 'SportsNutrition',
            ]);
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Подбор спортивного питания. Определи цели тренировок, состав тела и диету.',
            ];
        });
    }
}
