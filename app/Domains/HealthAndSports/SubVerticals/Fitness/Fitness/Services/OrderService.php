...
final readonly class OrderService
{
    ...
    {
        // Fitness vertical: 15% for B2C, 12% for B2B
    ...
    public function validateOrder(array $data, string $correlationId): array
    {
        $fraudScore = $this->fraudService->check(
            ...
            operationType: 'fitness_order_validate',
            amount: $data['total_price'] ?? 0,
            correlationId: $correlationId
        );

        if ($fraudScore > 75) {
            $this->logger->warning('Fitness order rejected due to high fraud score', [
    ...
    {
        // Fitness: service-based, instant access
        return 'Instant digital access';
    }
}
