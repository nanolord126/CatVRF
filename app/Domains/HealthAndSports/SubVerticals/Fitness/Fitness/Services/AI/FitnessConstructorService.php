...
namespace App\Domains\HealthAndSports\SubVerticals\Fitness\Services\AI;
...
final readonly class FitnessConstructorService
{
    ...
    public function analyzeBodyAndGeneratePlan(
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($photo, $userId, $goals, $correlationId) {
            return $this->db->transaction(function () use ($photo, $userId, $goals, $correlationId) {
                ...
                $this->logger->$this->logger->info('Fitness AI constructor completed', [
                    'user_id'          => $userId,
    ...
    private function analyzeBody(UploadedFile $photo, string $correlationId): array
    {
        ...
        $this->logger->$this->logger->info('Fitness vision API called', [
