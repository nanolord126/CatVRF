...
namespace App\Domains\HealthAndSports\SubVerticals\Fitness\Services;
...
final readonly class FitnessService
{
    ...
    public function purchaseMembership(
        ...
        return $this->db->transaction(function () use ($userId, $gymId, $type, $durationDays, $priceKopecks, $isB2B, $correlationId) {
            ...
            $membership = Membership::create([
                ...
                'expires_at'        => CarbonImmutable::now()->addDays($durationDays),
                'is_active'         => true,
                'correlation_id'    => $correlationId,
                'tags'              => ['b2b' => $isB2B],
            ]);

            $this->logger->$this->logger->info('Fitness membership purchased', [
                'user_id'        => $userId,
                'gym_id'         => $gymId,
                'type'           => $type,
                'is_b2b'         => $isB2B,
    ...
    public function bookSession(
        ...
        return $this->db->transaction(function () use ($userId, $trainerId, $gymId, $scheduledAt, $durationMinutes, $correlationId) {
            ...
            $this->logger->$this->logger->info('Fitness session booked', [
                'user_id'        => $userId,
                'trainer_id'     => $trainerId,
                'gym_id'         => $gymId,
                'scheduled_at'   => $scheduledAt,
     ...
     */
    public function listGyms(array $filters = []): Collection
    {
     ...
     */
    public function listTrainers(int $gymId): Collection
    {
