...
namespace App\Domains\MarketExpansion\Services;
use App\Domains\MarketExpansion\Models\ExpansionStrategist;
use App\Domains\MarketExpansion\Models\ExpansionStrategy;
...
return $this->db->transaction(function()use($strategistId,$strategyType,$hoursSpent,$dueDate,$correlationId){$s=ExpansionStrategist::findOrFail($strategistId);$total=(int)($s->price_kopecks_per_hour*$hoursSpent);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'expand','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$st=ExpansionStrategy::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'strategist_id'=>$strategistId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','strategy_type'=>$strategyType,'hours_spent'=>$hoursSpent,'due_date'=>$dueDate,'tags'=>['expand'=>true]]);$this->log->channel('audit')->info('Expansion strategy created',['strategy_id'=>$st->id,'correlation_id'=>$correlationId]);return $st;});
}
public function completeStrategy(int $strategyId,string $correlationId=""):ExpansionStrategy{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($strategyId,$correlationId){$st=ExpansionStrategy::findOrFail($strategyId);if($st->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$st->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$st->payout_kopecks,'expand_payout',['correlation_id'=>$correlationId,'strategy_id'=>$st->id]);$this->log->channel('audit')->info('Expansion strategy completed',['strategy_id'=>$st->id]);return $st;});}
public function cancelStrategy(int $strategyId,string $correlationId=""):ExpansionStrategy{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($strategyId,$correlationId){$st=ExpansionStrategy::findOrFail($strategyId);if($st->status==='completed')throw new \RuntimeException("Cannot cancel",400);$st->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($st->payment_status==='completed')$this->wallet->credit(tenant()->id,$st->total_kopecks,'expand_refund',['correlation_id'=>$correlationId,'strategy_id'=>$st->id]);$this->log->channel('audit')->info('Expansion strategy cancelled',['strategy_id'=>$st->id]);return $st;});}
