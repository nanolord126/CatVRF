...
namespace App\Domains\PetServices\Policies;

use App\Models\User;
use App\Domains\PetServices\Models\B2BPetStorefront;
use App\Domains\PetServices\Models\B2BPetOrder;
...
final class B2BPetPolicy
{
    ...
    public function viewStorefront(User $user, B2BPetStorefront $storefront): Response
    {
    ...
    public function updateStorefront(User $user, B2BPetStorefront $storefront): Response
    {
        return $user->tenant_id === $storefront->tenant_id || $user->is_admin
            ? Response::allow()
            : Response::deny('Нет доступа');
    ...
    public function viewOrder(User $user, B2BPetOrder $order): Response
    {
    ...
    public function approveOrder(User $user, B2BPetOrder $order): Response
    {
    ...
    public function rejectOrder(User $user, B2BPetOrder $order): Response
    {
        return ($user->tenant_id === $order->tenant_id || $user->is_admin) && $order->status === 'pending'
            ? Response::allow()
