...
final class HotelManagementService{public function __construct(private readonly FraudControlService $fraud,private readonly WalletService $wallet){}
...
return DB::transaction(function()use($hotelId,$roomType,$checkIn,$checkOut,$correlationId){$h=Hotel::findOrFail($hotelId);$nights=(int)((\strtotime($checkOut)-\strtotime($checkIn))/86400);$total=(int)($h->price_kopecks_per_night*$nights);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'hotel_booking','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$b=HotelBooking::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'hotel_id'=>$hotelId,'guest_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','room_type'=>$roomType,'check_in'=>$checkIn,'check_out'=>$checkOut,'nights_count'=>$nights,'tags'=>['hotel'=>true]]);Log::channel('audit')->info('Hotel booking created',['booking_id'=>$b->id,'correlation_id'=>$correlationId]);return $b;});
}
public function completeBooking(int $bookingId,string $correlationId=""):HotelBooking{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($bookingId,$correlationId){$b=HotelBooking::findOrFail($bookingId);if($b->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$b->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$b->payout_kopecks,'hotel_payout',['correlation_id'=>$correlationId,'booking_id'=>$b->id]);Log::channel('audit')->info('Hotel booking completed',['booking_id'=>$b->id]);return $b;});}
public function cancelBooking(int $bookingId,string $correlationId=""):HotelBooking{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($bookingId,$correlationId){$b=HotelBooking::findOrFail($bookingId);if($b->status==='completed')throw new \RuntimeException("Cannot cancel",400);$b->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($b->payment_status==='completed')$this->wallet->credit(tenant()->id,$b->total_kopecks,'hotel_refund',['correlation_id'=>$correlationId,'booking_id'=>$b->id]);Log::channel('audit')->info('Hotel booking cancelled',['booking_id'=>$b->id]);return $b;});}
