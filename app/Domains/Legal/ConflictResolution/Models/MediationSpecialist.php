...
final class MediationSpecialist extends Model
{
    ...
    use HasUuids,SoftDeletes,TenantScoped;protected $table='mediation_specialists';protected $fillable=['uuid','tenant_id','user_id','correlation_id','name','specialties','price_kopecks_per_hour','rating','is_verified','tags'];protected $casts=['specialties'=>'json','price_kopecks_per_hour'=>'integer','rating'=>'float','is_verified'=>'boolean','tags'=>'json'];protected static function booted_disabled(){static::addGlobalScope('tenant',fn($q)=>$q->where('mediation_specialists.tenant_id',tenant()->id));}
