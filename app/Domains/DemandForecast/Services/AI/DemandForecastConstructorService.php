...
namespace App\Domains\DemandForecast\Services\AI;
...
final readonly class DemandForecastConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'demandforecast_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:DemandForecast:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('DemandForecast', $fullProfile, $userId);

            $this->logger->$this->logger->info('DemandForecast AI constructor used', [
                'user_id'        => $userId,
                'correlation_id' => $correlationId,
                'vertical'       => 'DemandForecast',
            ]);
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Прогнозирование спроса. Определи тренды и дай рекомендации по запасам.',
            ];
        });
    }
}
