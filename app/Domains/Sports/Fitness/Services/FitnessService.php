...
final readonly class FitnessService
{
    ...
    public function completeMembership(int $membershipId,string $correlationId=""):FitnessMembership{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($membershipId,$correlationId){$m=FitnessMembership::findOrFail($membershipId);if($m->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$m->update(['status'=>'active','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$m->payout_kopecks,\App\Domains\Wallet\Enums\BalanceTransactionType::PAYOUT,$correlationId, null, null, ['correlation_id'=>$correlationId,'membership_id'=>$m->id]);return $m;});}
    public function cancelMembership(int $membershipId,string $correlationId=""):FitnessMembership{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($membershipId,$correlationId){$m=FitnessMembership::findOrFail($membershipId);if($m->status==='active')throw new \RuntimeException("Cannot cancel",400);$m->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($m->payment_status==='completed')$this->wallet->credit(tenant()->id,$m->total_kopecks,\App\Domains\Wallet\Enums\BalanceTransactionType::REFUND,$correlationId, null, null, ['correlation_id'=>$correlationId,'membership_id'=>$m->id]);return $m;});}
