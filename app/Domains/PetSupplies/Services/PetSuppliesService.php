...
namespace App\Domains\PetSupplies\Services;
...
/**
 * Главный сервис вертикали PetSupplies.
 *
 * Координирует товары для животных (pet_food, pet_accessories, pet_care).
 */
final readonly class PetSuppliesService
{
    ...
    /**
     * Подготовить чекаут для товаров для животных.
     *
     * @param  array{items: array, seller_address: string, buyer_address: string, sub_vertical?: string}  $cartData
     * @return array{delivery_cost: int, eta: int, pet_recommendation: array}
     */
    public function prepareCheckout(array $cartData): array
    {
        $correlationId = (string) Str::uuid();

        $this->logger->info('PetSupplies checkout preparation started', [
        ...
        $petRecommendation = $this->generatePetRecommendations($cartData['items'] ?? []);

        return [
            'delivery_cost' => $deliveryCalculation['cost'],
            'eta' => $deliveryCalculation['eta'],
            'distance' => $deliveryCalculation['distance'],
            'pet_recommendation' => $petRecommendation,
            'correlation_id' => $correlationId,
        ];
    }

    /**
     * Создать заказ товаров для животных.
     *
    ...
    public function createOrder(array $orderData): array
    {
        ...
        $this->logger->info('PetSupplies order creation started', [
            'correlation_id' => $correlationId,
        ...
        return $this->db->transaction(function () use ($orderData, $correlationId) {
            ...
            // Создание заказа
            $orderId = $this->db->table('pet_supplies_orders')->insertGetId([
            ...
            $paymentPreparation = $this->paymentAdapter->preparePayment([
                ...
                'vertical' => 'pet_supplies',
                'sub_vertical' => $orderData['sub_vertical'] ?? null,
                'payable_type' => 'pet_supplies_order',
                'payable_id' => $orderId,
            ]);
    ...
    /**
     * Сгенерировать рекомендации на основе покупок.
     */
    private function generatePetRecommendations(array $items): array
    {
        // Упрощённо - рекомендуем аксессуары если покупаем еду
        $hasFood = false;
        ...
        if ($hasFood) {
            return [
                'recommendation' => 'Рекомендуем добавить игрушки или лежанку для вашего питомца',
                'items' => ['pet_toys', 'pet_bedding'],
            ];
        }
