...
namespace App\Domains\Art\UXDesign\Services;
use App\Domains\Art\UXDesign\Models\UXDesigner;
use App\Domains\Art\UXDesign\Models\UXDesignProject;
...
return DB::transaction(function()use($designerId,$designType,$designHours,$dueDate,$correlationId){$d=UXDesigner::findOrFail($designerId);$total=(int)($d->price_kopecks_per_hour*$designHours);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'ux_design','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$p=UXDesignProject::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'designer_id'=>$designerId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','design_type'=>$designType,'design_hours'=>$designHours,'due_date'=>$dueDate,'tags'=>['ux'=>true]]);Log::channel('audit')->info('UX design project created',['project_id'=>$p->id,'correlation_id'=>$correlationId]);return $p;});
}
public function completeProject(int $projectId,string $correlationId=""):UXDesignProject{$correlationId=$correlationId?:(string)Str::uuid();return DB::transaction(function()use($projectId,$correlationId){$p=UXDesignProject::findOrFail($projectId);if($p->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$p->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$p->payout_kopecks,'ux_payout',['correlation_id'=>$correlationId,'project_id'=>$p->id]);Log::channel('audit')->info('UX design project completed',['project_id'=>$p->id]);return $p;});}
