...
use App\Domains\Realtime\Events\RealtimeMessage;
...
final readonly class RealtimeService
{
    ...
    public function broadcast(string $channel, string $event, array $data, string $correlationId = ''): void
    {
        ...
        $this->broadcast->event(new RealtimeMessage($channel, $event, $data));

        $this->logger->$this->logger->info('Realtime message broadcasted', [
            'channel' => $channel,
            'event' => $event,
    ...
    public function registerConnection(int $userId, string $channel, string $connectionId, string $correlationId = ''): WebSocketConnection
    {
        ...
        $this->logger->$this->logger->info('WebSocket connection registered', [
            'connection_id' => $connectionId,
            'channel' => $channel,
            'correlation_id' => $correlationId,
        ]);
    ...
    public function disconnect(string $connectionId, string $correlationId = ''): void
    {
        ...
        $this->logger->$this->logger->info('WebSocket disconnected', [
            'connection_id' => $connectionId,
            'correlation_id' => $correlationId,
        ]);
    }
    ...
    public function disconnectUser(int $userId, string $correlationId = ''): void
    {
        ...
        $this->logger->$this->logger->info('User WebSocket connections disconnected', [
            'user_id' => $userId,
            'correlation_id' => $correlationId,
        ]);
    }
