...
/**
 * CloudMigration — CatVRF 2026 Component.
 *
 ...
 *
 * @see https://catvrf.ru/docs/cloudmigration
 */
...
final class CloudMigration extends Model
{
    ...
    protected $table = 'cloud_migrations';

    protected $fillable = ['uuid', 'tenant_id', 'architect_id', 'client_id', 'correlation_id', 'status', 'total_kopecks', 'payout_kopecks', 'payment_status', 'migration_type', 'hours_spent', 'due_date', 'tags'];

    protected $casts = ['total_kopecks' => 'integer', 'payout_kopecks' => 'integer', 'hours_spent' => 'integer', 'due_date' => 'datetime', 'tags' => 'json'];

    protected static function booted()
    {
        self::addGlobalScope('tenant', fn ($q) => $q->where('cloud_migrations.tenant_id', tenant()->id));
    }
