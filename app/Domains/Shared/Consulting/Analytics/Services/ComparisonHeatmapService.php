...
final readonly class ComparisonHeatmapService
{
    ...
    public function compareGeoTimeSeries(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Geo comparison generated', [
                'correlation_id' => $this->correlationId,
                'tenant_id' => $tenantId,
                'metric' => $metric,
    ...
    public function compareClickTimeSeries(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Click comparison generated', [
                'correlation_id' => $this->correlationId,
                'tenant_id' => $tenantId,
                'page_url' => $pageUrl,
                'delta_percent' => $comparisonResult['percent_delta'],
    ...
    public function invalidateCache(int $tenantId, string $vertical = '*'): void
    {
        ...
        $this->logger->$this->logger->info('Comparison cache invalidated', [
            'correlation_id' => $this->correlationId,
            'tenant_id' => $tenantId,
            'vertical' => $vertical,
        ]);
    ...
    private function calculateGeoComparison(
        ...
        $period1Map = new Collection($period1Data)->keyBy('geo_hash')->toArray();
        $period2Map = new Collection($period2Data)->keyBy('geo_hash')->toArray();
    ...
    private function calculateClickComparison(
        ...
        $period1Map = new Collection($period1Data)->keyBy(fn ($item) => "{$item['x']}-{$item['y']}")->toArray();
        $period2Map = new Collection($period2Data)->keyBy(fn ($item) => "{$item['x']}-{$item['y']}")->toArray();
