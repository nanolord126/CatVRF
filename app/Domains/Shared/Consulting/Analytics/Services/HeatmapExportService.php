...
final class HeatmapExportService
{
     ...
     */
    private readonly string $storagePath = 'heatmap-exports';

    /**
     * @var int Export file TTL in seconds (24 hours)
     */
    private readonly int $fileTtl = 86400;

    /**
     * @var int Maximum export file size (50MB)
     */
    private readonly int $maxFileSize = 52428800;

    /**
     * @var array Supported export formats
     */
    private readonly array $supportedFormats = ['png', 'pdf'];
    ...
    public function __construct(private readonly FraudControlService $fraudControlService,
        private readonly FilesystemManager $storage, ?string $disk, private readonly Request $request, private readonly LoggerInterface $logger)
    {
        if ($disk) {
            $this->disk = $disk;
        }
    ...
    public function exportGeoHeatmapToPng(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Starting geo-heatmap PNG export', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'metadata' => $metadata,
            ...
            $url = $this->storage->disk($this->disk)->temporaryUrl(
                $storagePath,
                \CarbonImmutable::now()->addSeconds($this->fileTtl)
            );

            $this->logger->$this->logger->info('Geo-heatmap PNG export completed', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'file_size' => $fileSize,
            ...
            return [
                ...
                'expires_at' => \CarbonImmutable::now()->addSeconds($this->fileTtl)->toIso8601String(),
                'correlation_id' => $correlationId,
            ];
    ...
    public function exportGeoHeatmapToPdf(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Starting geo-heatmap PDF export', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
            ...
            $this->logger->$this->logger->info('Geo-heatmap PDF export completed', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'file_size' => $fileSize,
                'storage_path' => $storagePath,
    ...
    public function exportClickHeatmapToPng(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Starting click-heatmap PNG export', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'correlation_id' => $correlationId,
            ]);
            ...
            $this->logger->$this->logger->info('Click-heatmap PNG export completed', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'file_size' => $fileSize,
                'correlation_id' => $correlationId,
    ...
    public function exportClickHeatmapToPdf(
        ...
        try {
            ...
            $this->logger->$this->logger->info('Starting click-heatmap PDF export', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'correlation_id' => $correlationId,
            ]);
            ...
            $this->logger->$this->logger->info('Click-heatmap PDF export completed', [
                'tenant_id' => $tenantId,
                'filename' => $filename,
                'file_size' => $fileSize,
                'correlation_id' => $correlationId,
     ...
     *
     * @return Filesystem Storage disk
     */
    ...
    {
        return \CarbonImmutable::now()->timestamp.'-'.Str::random(8);
    }
}
