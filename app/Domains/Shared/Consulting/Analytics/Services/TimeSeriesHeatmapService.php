...
final readonly class TimeSeriesHeatmapService
{
    ...
    public function getGeoTimeSeries(
        ...
        if ($cached = cache()->get($cacheKey)) {
            $this->logger->$this->logger->info('[TimeSeriesHeatmap] Cache hit', [
                'cache_key' => $cacheKey,
        ...
        try {
            ...
            $this->logger->$this->logger->info('[TimeSeriesHeatmap] Geo heatmap generated', [
                'tenant_id' => $tenantId,
                'vertical' => $vertical,
                'aggregation' => $aggregation,
    ...
    public function getClickTimeSeries(
        ...
        // Try cache first
        if ($cached = cache()->get($cacheKey)) {
            $this->logger->$this->logger->info('[TimeSeriesHeatmap] Cache hit (click)', [
        ...
        try {
            ...
            $this->logger->$this->logger->info('[TimeSeriesHeatmap] Click heatmap generated', [
                'tenant_id' => $tenantId,
                'vertical' => $vertical,
    ...
    public function invalidateCache(int $tenantId, string $vertical = '*'): void
    {
        ...
        $this->logger->$this->logger->info('[TimeSeriesHeatmap] Cache invalidated', [
            'tenant_id' => $tenantId,
            'vertical' => $vertical,
            'correlation_id' => $this->correlationId,
        ]);
