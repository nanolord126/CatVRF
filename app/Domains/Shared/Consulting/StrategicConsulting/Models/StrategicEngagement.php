...
/**
 * StrategicEngagement — CatVRF 2026 Component.
 *
 ...
 *
 * @see https://catvrf.ru/docs/strategicengagement
 */
...
final class StrategicEngagement extends Model
{
    ...
    protected $table = 'strategic_engagements';

    protected $fillable = ['uuid', 'tenant_id', 'consultant_id', 'client_id', 'correlation_id', 'status', 'total_kopecks', 'payout_kopecks', 'payment_status', 'engagement_type', 'hours_spent', 'due_date', 'tags'];

    protected $casts = ['total_kopecks' => 'integer', 'payout_kopecks' => 'integer', 'hours_spent' => 'integer', 'due_date' => 'datetime', 'tags' => 'json'];

    protected static function booted()
    {
        self::addGlobalScope('tenant', fn ($q) => $q->where('strategic_engagements.tenant_id', tenant()->id));
    }
