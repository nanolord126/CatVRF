...
/**
 * NeurotechProject — CatVRF 2026 Component.
 *
 ...
 *
 * @see https://catvrf.ru/docs/neurotechproject
 */

namespace App\Domains\Consulting\NeurotechConsulting\Models;
...
final class NeurotechProject extends Model
{
    use HasUuids;
    use SoftDeletes;
    use TenantScoped;
    ...
    protected $table = 'neurotech_projects';

    protected $fillable = ['uuid', 'tenant_id', 'expert_id', 'client_id', 'correlation_id', 'status', 'total_kopecks', 'payout_kopecks', 'payment_status', 'project_type', 'hours_spent', 'due_date', 'tags'];

    protected $casts = ['total_kopecks' => 'integer', 'payout_kopecks' => 'integer', 'hours_spent' => 'integer', 'due_date' => 'datetime', 'tags' => 'json'];

    protected static function booted()
    {
        self::addGlobalScope('tenant', fn ($q) => $q->where('neurotech_projects.tenant_id', tenant()->id));
    }
