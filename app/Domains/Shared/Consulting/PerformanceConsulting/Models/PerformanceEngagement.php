...
/**
 * PerformanceEngagement — CatVRF 2026 Component.
 *
 ...
 *
 * @see https://catvrf.ru/docs/performanceengagement
 */
...
final class PerformanceEngagement extends Model
{
    ...
    protected $table = 'performance_engagements';

    protected $fillable = ['uuid', 'tenant_id', 'coach_id', 'client_id', 'correlation_id', 'status', 'total_kopecks', 'payout_kopecks', 'payment_status', 'engagement_type', 'hours_spent', 'due_date', 'tags'];

    protected $casts = ['total_kopecks' => 'integer', 'payout_kopecks' => 'integer', 'hours_spent' => 'integer', 'due_date' => 'datetime', 'tags' => 'json'];

    protected static function booted()
    {
        self::addGlobalScope('tenant', fn ($q) => $q->where('performance_engagements.tenant_id', tenant()->id));
    }
