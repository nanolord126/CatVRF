...
final readonly class PetHealthConstructor
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        return $this->cache->tags(['veterinary', 'ai', 'constructor'])->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($petId, $photo, $symptoms, $context): array {
    ...
    public function analyzeAndSuggest(
        ...
        $this->logger->$this->logger->info('PetHealthConstructor: Starting analysis', [
            'pet_id' => $petId,
            'pet_uuid' => $pet->uuid,
        ...
        $this->logger->$this->logger->info('PetHealthConstructor: Analysis complete', [
            'pet_id' => $petId,
