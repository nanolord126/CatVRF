...
final readonly class WalletFraudCheckAdapter implements FraudCheckPort
{
    ...
    public function verifyOperation(
        ...
        try {
            ...
            $this->log->channel('fraud_alert')->info('Wallet operation fraud check executed', [
            ...
            if ($shouldBlock) {
                $this->log->channel('fraud_alert')->warning('Wallet operation blocked by ML fraud system', [
                    'correlation_id' => $correlationId,
                    'wallet_id'      => $walletId,
                    'operation_type' => $operationType,
                    'ml_score'       => $fraudScore,
            ...
            $this->log->channel('fraud_alert')->error('Wallet fraud check mechanism encountered an internal failure', [
                'correlation_id' => $correlationId,
