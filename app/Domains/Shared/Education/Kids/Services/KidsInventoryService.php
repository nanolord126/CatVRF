...
final readonly class KidsInventoryService
{
    ...
    {
        $this->logger->$this->logger->info('Attempting to add kids product', [
            'sku' => $dto->sku,
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('Kids product added successfully', [
    ...
    {
        $this->logger->$this->logger->info('Attaching toy metadata', [
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('Toy metadata attached', [
                'toy_id' => $toy->id,
                'correlation_id' => $dto->correlation_id,
            ]);
    ...
    public function deductStock(int $productId, int $quantity, string $reason, string $correlationId): void
    {
        $this->db->transaction(function () use ($productId, $quantity, $reason, $correlationId) {
            ...
            $this->logger->$this->logger->info('Stock deducted', [
    ...
    public function isToyCertified(int $productId): bool
    {
        ...
        return in_array('EAC', $certificates, true) || in_array('GOST', $certificates, true) || in_array('ISO', $certificates, true);
    }
}
