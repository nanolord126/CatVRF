...
final readonly class KidsCenterService
{
    ...
    {
        $this->logger->$this->logger->info('Attempting to create kids center', [
            'name' => $dto->name,
        ...
        return $this->db->transaction(function () use ($dto) {
            ...
            $this->logger->$this->logger->info('Kids center created successfully', [
                'center_id' => $center->id,
                'correlation_id' => $dto->correlation_id,
            ]);
    ...
    public function bookEventSlot(int $eventId, int $childCount, string $correlationId): void
    {
        $this->db->transaction(function () use ($eventId, $childCount, $correlationId) {
            ...
            $this->logger->$this->logger->info('Event slot booked', [
                'event_id' => $eventId,
    ...
    {
        $this->logger->$this->logger->info('Safety verification requested', [
            'center_id' => $centerId,
            'correlation_id' => $correlationId,
        ]);
        ...
        $this->logger->$this->logger->info('Center safety verified', [
            'center_id' => $centerId,
            'correlation_id' => $correlationId,
        ]);
    }
    ...
    public function startChildVisit(int $centerId, int $userId, string $correlationId): void
    {
        ...
        $this->logger->$this->logger->info('Child visit started', [
            'center_id' => $centerId,
            'user_id' => $userId,
            'correlation_id' => $correlationId,
        ]);
