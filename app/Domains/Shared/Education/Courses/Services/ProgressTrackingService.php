...
final readonly class ProgressTrackingService
{
    ...
    public function trackLessonWatch(
        ...
        try {
            $this->logger->$this->logger->info('Tracking lesson watch time', [
            ...
            $progress = $this->db->transaction(function () use ($enrollmentId, $lessonId, $watchTimeSeconds) {
            ...
            $this->logger->$this->logger->info('Lesson watch time tracked', [
    ...
    public function markLessonComplete(
        ...
        try {
            $this->logger->$this->logger->info('Marking lesson as complete', [
                'enrollment_id' => $enrollmentId,
                'lesson_id' => $lessonId,
                'correlation_id' => $correlationId,
            ]);
            ...
            $progress = $this->db->transaction(function () use ($enrollmentId, $lessonId, $correlationId) {
                ...
                LessonCompleted::$this->bus->dispatch($progress, $correlationId);

                return $progress;
            });

            $this->logger->$this->logger->info('Lesson marked as complete', [
                'progress_id' => $progress->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function getEnrollmentProgress(int $enrollmentId, string $correlationId = ''): array
    {
        ...
        try {
            $this->logger->$this->logger->info('Getting enrollment progress', [
                'enrollment_id' => $enrollmentId,
                'correlation_id' => $correlationId,
            ]);
