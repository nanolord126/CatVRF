...
final readonly class CourseService
{
    ...
    public function enroll(int $userId, int $courseId, bool $isB2B = false, array $meta = [], string $correlationId = ''): Enrollment
    {
        ...
        return $this->db->transaction(function () use ($userId, $courseId, $isB2B, $correlationId) {
            ...
            $this->logger->$this->logger->info('Courses: user enrolled', ['enrollment_id' => $enrollment->id, 'corr' => $correlationId]);
    ...
    public function updateProgress(int $enrollmentId, int $lessonId, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($enrollment, $course, $correlationId) {
            ...
            if ($newProgress >= 100 && ! $enrollment->certificate_id) {
                ...
                $this->logger->$this->logger->info('Courses: certificate issued', ['user_id' => $enrollment->user_id, 'cert' => $certificate->uuid]);
            }
        });
    }
    ...
    public function getLiveLessonLink(int $lessonId, int $userId): string
    {
        ...
        $this->logger->$this->logger->info('Courses: access live room', ['user_id' => $userId, 'lesson_id' => $lessonId]);
