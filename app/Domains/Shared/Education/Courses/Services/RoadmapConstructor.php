...
final readonly class RoadmapConstructor
{
    ...
    {
        $this->logger->$this->logger->info('RoadmapConstructor: starting edu path', [
            'user_id' => $userId,
            'goal' => $goal,
        ...
        return $this->db->transaction(function () use ($userId, $goal, $params) {
            ...
            $this->logger->$this->logger->info('RoadmapConstructor: roadmap finished', [
                'user_id' => $userId,
                'correlation_id' => $this->correlationId,
            ]);
    ...
    private function generateRoadmapSteps(string $goal, string $level, Collection $courses): array
    {
