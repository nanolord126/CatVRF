...
final class EnrollmentReminderJob
{
    ...
    {
        try {
            $this->logger->$this->logger->info('Running enrollment reminder job', [
                'correlation_id' => $this->correlationId,
            ]);

            // Find enrollments inactive for more than 7 days
            $stalledEnrollments = Enrollment::where('status', 'active')
                ->where('last_accessed_at', '<', CarbonImmutable::now()->subDays(7))
                ->get();
            ...
            foreach ($stalledEnrollments as $enrollment) {
                try {
                    ...
                    $this->logger->$this->logger->info('Reminder sent to student', [
            ...
            $this->logger->$this->logger->info('Enrollment reminder job completed', [
