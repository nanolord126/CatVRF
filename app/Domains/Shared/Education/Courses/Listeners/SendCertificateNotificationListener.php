...
final class SendCertificateNotificationListener
{
    ...
    {
        try {
            $this->logger->$this->logger->info('Sending certificate notification', [
                'certificate_id' => $event->certificate->id,
                'correlation_id' => $event->correlationId,
            ]);

            // Send email/notification to student
            // $this->notificationManager->send($student, new CertificateIssuedNotification($certificate));

            $this->logger->$this->logger->info('Certificate notification sent', [
                'certificate_id' => $event->certificate->id,
                'correlation_id' => $event->correlationId,
            ]);
        } catch (Throwable $e) {
