...
final class UpdateCourseProgress
{
    ...
    public function handle(LessonCompleted $event): void
    {
        ...
        if (! in_array($event->lesson->id, $completedLessons, true)) {
        ...
        $this->logger->$this->logger->info('Student course progress updated', [
        ...
        if ($newProgressPercent >= 100) {
            ...
            $this->logger->$this->logger->info('Course 100% Completed - Certificate eligible', [
                'enrollment_id' => $enrollment->id,
                'correlation_id' => $correlationId,
            ]);
        }
