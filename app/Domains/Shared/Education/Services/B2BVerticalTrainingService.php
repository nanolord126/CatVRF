...
final readonly class B2BVerticalTrainingService
{
    ...
    public function enrollEmployeeInRequiredCourses(
        ...
        $this->log->warning('B2B Vertical Training: Enrolling employee in required courses', [
        ...
        foreach ($requiredCourses as $verticalCourse) {
            try {
                ...
                $this->log->error('B2B Vertical Training: Failed to enroll employee in course', [
                    'employee_id' => $employee->id,
    ...
    public function enrollEmployeeInCourse(
        ...
        $this->log->warning('B2B Vertical Training: Enrolling employee in course', [
            'employee_id' => $employee->id,
            'course_id' => $course->id,
            'contract_id' => $contract->id,
            'correlation_id' => $correlationId,
        ...
        if ($verticalCourse) {
            $this->events->dispatch(new EmployeeEnrolledInVerticalCourse(
                $employee,
                $verticalCourse,
                $enrollment,
                $correlationId
    ...
    public function getCompanyProgressForVertical(int $tenantId, string $vertical): Collection
    {
        ...
        return $employees->map(function ($employee) use ($vertical, $enrollments, $verticalCourses) {
            $employeeEnrollments = $enrollments->get($employee->id, new Collection());
    ...
    public function createVerticalCourse(array $data, string $correlationId): VerticalCourse
    {
        ...
        $this->log->warning('B2B Vertical Training: Creating vertical course', [
            'vertical' => $data['vertical'],
            'course_id' => $data['course_id'],
            'correlation_id' => $correlationId,
        ]);

        $verticalCourse = VerticalCourse::create([
            'uuid' => \Illuminate\Support\Str::uuid(),
            'tenant_id' => $data['tenant_id'] ?? $this->config->get('app.default_tenant_id', 0),
            'course_id' => $data['course_id'],
            'vertical' => $data['vertical'],
        ...
        $this->events->dispatch(new VerticalCourseCreated($verticalCourse, $correlationId));
    ...
    public function updateVerticalCourse(VerticalCourse $verticalCourse, array $data): VerticalCourse
    {
        ...
        $this->log->warning('B2B Vertical Training: Updated vertical course', [
            'vertical_course_id' => $verticalCourse->id,
            'vertical' => $verticalCourse->vertical,
            'changes' => $changes,
        ]);

        $this->events->dispatch(new VerticalCourseUpdated($verticalCourse, $changes));
    ...
    public function deleteVerticalCourse(VerticalCourse $verticalCourse): bool
    {
        ...
        $this->log->warning('B2B Vertical Training: Deleted vertical course', [
            'vertical' => $vertical,
            'course_id' => $courseId,
        ]);

        $this->events->dispatch(new VerticalCourseDeleted($verticalCourse));
