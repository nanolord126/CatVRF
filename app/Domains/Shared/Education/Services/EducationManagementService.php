...
final readonly class EducationManagementService
{
    ...
    {
        $this->logger->$this->logger->info('Education B2C: Direct enrollment process started', [
            'user_id' => $user->id,
            'course_id' => $course->id,
        ...
        return $this->db->transaction(function () use ($user, $course, $correlationId) {
            ...
            $this->logger->$this->logger->info('Education B2C: Enrollment completed', [
    ...
    {
        $this->logger->$this->logger->info('Education B2B: Contract-based enrollment started', [
            'user_id' => $user->id,
        ...
        return $this->db->transaction(function () use ($user, $contract, $course, $correlationId) {
            ...
            $this->logger->$this->logger->info('Education B2B: Contract slot consumed. Enrollment active.', [
                'enrollment_id' => $enrollment->id,
                'contract_id' => $contract->id,
                'correlation_id' => $this->request?->header('X-Correlation-ID', Str::uuid()->toString()),
            ]);
    ...
    {
        $this->logger->$this->logger->info('Education B2B: Creating new corporate agreement', [
