...
final readonly class EducationFraudDetectionService
{
    ...
    {
        $enrollment = $this->db->table('enrollments')->where('id', $enrollmentId)->first();
        ...
        $detection = new CheatingDetectionDto(
            ...
            detectedAt: CarbonImmutable::now()->toIso8601String(),
        );

        $this->redis->setex($cacheKey, self::CACHE_TTL, json_encode($detection->toArray()));
    ...
    {
        $review = $this->db->table('education_course_reviews')->where('id', $reviewId)->first();
    ...
    {
        $activityEvents = $this->db->table('education_activity_logs')
    ...
    {
        $avgTimePerModule = $this->db->table('education_module_progress')
            ->where('enrollment_id', $enrollmentId)
    ...
    {
        $answers = $this->db->table('education_quiz_answers')
            ->where('enrollment_id', $enrollmentId)
            ->limit(100)
            ->get();
    ...
    {
        $ips = $this->db->table('education_activity_logs')
            ->where('enrollment_id', $enrollmentId)
            ->where('user_id', $userId)
    ...
    {
        $recentReviews = $this->db->table('education_course_reviews')
            ->where('user_id', $userId)
            ->where('created_at', '>=', CarbonImmutable::now()->subHours(24))
            ->count();
    ...
    private function analyzeReviewTiming(int $reviewId, int $userId): array
    {
        $enrollment = $this->db->table('enrollments')
            ->where('user_id', $userId)
            ->where('completed_at', '>=', CarbonImmutable::now()->subMinutes(5))
            ->first();
    ...
    {
        $this->db->table('education_fraud_records')->insert([
            'id' => (string) Str::uuid(),
        ...
        $this->log->channel('audit')->warning('Cheating detected', [
            'correlation_id' => $detection->correlationId,
            'user_id' => $detection->userId,
            'enrollment_id' => $detection->enrollmentId,
            'severity' => $detection->severity,
    ...
    private function handleReviewFraudDetection(ReviewFraudDto $detection): void
    {
        ...
        $this->db->table('education_course_reviews')
        ...
        $this->log->channel('audit')->warning('Review fraud detected', [
            'correlation_id' => $detection->correlationId,
            'user_id' => $detection->userId,
            'review_id' => $detection->reviewId,
            'severity' => $detection->severity,
    ...
    private function blockUser(int $userId, int $tenantId): void
    {
        $this->db->table('users')
            ->where('id', $userId)
            ->update(['is_blocked' => true, 'blocked_at' => CarbonImmutable::now()]);

        $this->log->channel('audit')->critical('User blocked for cheating', [
            'user_id' => $userId,
            'tenant_id' => $tenantId,
        ]);
    }
