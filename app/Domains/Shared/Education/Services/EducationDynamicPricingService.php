...
final readonly class EducationDynamicPricingService
{
    ...
    public function calculateDynamicPrice(CalculatePriceDto $dto): PriceAdjustmentDto
    {
        ...
        if ($cachedResult !== null) {
            $this->log->channel('audit')->$this->logger->info('Dynamic price served from cache', [
        ...
        return $this->db->transaction(function () use ($dto, $cacheKey) {
            ...
            $priceAdjustment = new PriceAdjustmentDto(
                ...
                factors: $factors,
                validUntil: CarbonImmutable::now()->addHours(24)->toIso8601String(),
                isFlashSale: $isFlashSale,
                generatedAt: CarbonImmutable::now()->toIso8601String(),
            );

            $this->redis->setex($cacheKey, self::CACHE_TTL, json_encode($priceAdjustment->toArray()));
    ...
    public function triggerFlashSale(int $courseId, int $discountPercent, string $correlationId): PriceAdjustmentDto
    {
        ...
        $priceAdjustment = new PriceAdjustmentDto(
            ...
            validUntil: CarbonImmutable::now()->addHours(6)->toIso8601String(),
            isFlashSale: true,
            generatedAt: CarbonImmutable::now()->toIso8601String(),
        );
    ...
    {
        $similarCoursesCount = $this->db->table('courses')
    ...
    private function calculateUserSegmentFactor(?string $userSegment, ?int $userId): float
    {
        ...
        if ($userId !== null) {
            $totalEnrollments = $this->db->table('enrollments')
    ...
    {
        $this->db->table('education_price_history')->insert([
            'id' => (string) Str::uuid(),
            'tenant_id' => $dto->tenantId,
