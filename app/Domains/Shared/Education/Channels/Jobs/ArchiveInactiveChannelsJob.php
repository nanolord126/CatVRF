...
final class ArchiveInactiveChannelsJob implements ShouldQueue
{
    ...
    public function handle(ChannelService $channelService): void
    {
        ...
        $threshold        = CarbonImmutable::now()->subDays($inactiveDays);
        $warnThreshold    = CarbonImmutable::now()->subDays($inactiveDays - $warnBeforeDays);

        $this->logger->$this->logger->info('ArchiveInactiveChannelsJob started', [
        ...
        BusinessChannel::withoutGlobalScopes()
            ...
            ->chunk(100, function ($channels) use ($correlationId, &$warnCount): void {
                foreach ($channels as $channel) {
                    try {
                        $this->eventDispatcher->dispatch(new ChannelArchived(
                            $channel,
                            'inactivity_warning',
                            $correlationId
                        ));
        ...
        $this->logger->$this->logger->info('ArchiveInactiveChannelsJob completed', [
            'correlation_id' => $correlationId,
