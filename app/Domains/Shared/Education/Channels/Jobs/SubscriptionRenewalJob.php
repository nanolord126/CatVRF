...
use App\Notifications\ChannelPlanExpiringSoonNotification;
...
final class SubscriptionRenewalJob
{
    ...
    public function handle(ChannelTariffService $tariffService): void
    {
        ...
        $this->logger->$this->logger->info('SubscriptionRenewalJob started', [
            'correlation_id' => $correlationId,
        ]);

        // 1. Предупреждения (за N дней до истечения)
        $expiringSoon = ChannelSubscriptionUsage::where('status', 'active')
            ->whereBetween('expires_at', [CarbonImmutable::now(), CarbonImmutable::now()->addDays($warnDays)])
            ->with(['channel', 'plan'])
            ->get();

        foreach ($expiringSoon as $usage) {
            $this->logger->$this->logger->info('Channel plan expiring soon', [
                'correlation_id' => $correlationId,
            ...
            if ($usage->channel && $usage->channel->owner) {
                $usage->channel->owner->notify(
                    new ChannelPlanExpiringSoonNotification(
                        $usage,
                        $warnDays,
                        $correlationId
                    )
        ...
        $this->logger->$this->logger->info('SubscriptionRenewalJob completed', [
            'correlation_id' => $correlationId,
            'renewed'        => $renewed,
            'failed'         => $failed,
