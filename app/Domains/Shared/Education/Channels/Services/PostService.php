...
final readonly class PostService
{
    ...
    public function createPost(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Post created', [
                'correlation_id' => $correlationId,
                'tenant_id'      => $channel->tenant_id,
                'channel_id'     => $channel->id,
                'post_id'        => $post->id,
    ...
    public function publishPost(
        ...
        $this->db->transaction(function () use ($post, $moderatedBy, $correlationId): void {
            ...
            $this->logger->$this->logger->info('Post published', [
                'correlation_id' => $correlationId,
        ...
        $this->eventDispatcher->dispatch(new PostPublished($post, $correlationId));
    ...
    public function archivePost(Post $post, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($post, $correlationId): void {
            ...
            $this->logger->$this->logger->info('Post archived', [
                'correlation_id' => $correlationId,
                'post_id'        => $post->id,
                'tenant_id'      => $post->tenant_id,
            ]);
    ...
    private function attachMedia(Post $post, UploadedFile $file, int $sortOrder): PostMedia
    {
        ...
        $type      = match (true) {
            ...
            in_array($mime, ['video/mp4', 'video/quicktime'], true) => 'shorts',
            default                                  => 'image',
        };
