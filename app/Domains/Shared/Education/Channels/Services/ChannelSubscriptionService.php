...
final readonly class ChannelSubscriptionService
{
    ...
    public function subscribe(
        ...
        return $this->db->transaction(function () use ($userId, $channel, $visibilityPreference, $correlationId): ChannelSubscriber {
            ...
            $this->logger->$this->logger->info('User subscribed to channel', [
                'correlation_id' => $correlationId,
                'user_id'        => $userId,
                'channel_id'     => $channel->id,
            ...
            $this->eventDispatcher->dispatch(new ChannelSubscribed($channel, $userId, $correlationId));
    ...
    public function unsubscribe(
        ...
        $this->db->transaction(function () use ($userId, $channel, $correlationId): void {
            $updated = ChannelSubscriber::where('channel_id', $channel->id)
                ...
                ->update(['unsubscribed_at' => CarbonImmutable::now()]);
            ...
            if ($updated > 0) {
                ...
                $this->logger->$this->logger->info('User unsubscribed from channel', [
                    'correlation_id' => $correlationId,
                    'user_id'        => $userId,
                    'channel_id'     => $channel->id,
                    'tenant_id'      => $channel->tenant_id,
     ...
     */
    public function getSubscribedChannels(int $userId): Collection
    {
    ...
    public function getPersonalFeed(int $userId, string $audience = 'all', int $perPage = 15)
    {
        ...
        if ($subscribedChannelIds->isEmpty()) {
            return Post::withoutGlobalScopes()->whereRaw('1=0')->paginate($perPage);
        }
