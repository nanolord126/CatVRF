...
final readonly class StreamService
{
    ...
    public function createStream(
        ...
        return $this->db->transaction(function () use ($bloggerId, $title, $description, $scheduledAt, $settings, $correlationId) {
            ...
            // Audit log
            $this->logger->$this->logger->info('Stream created', [
                'stream_id' => $stream->id,
                'blogger_id' => $bloggerId,
                'title' => $title,
                'correlation_id' => $correlationId,
            ...
            $this->eventDispatcher->dispatch(new StreamCreated($stream));
    ...
    public function startStream(int $streamId, string $correlationId = ''): Stream
    {
        ...
        return $this->db->transaction(function () use ($stream, $correlationId) {
            ...
            // Audit log
            $this->logger->$this->logger->info('Stream started', [
                'stream_id' => $stream->id,
                'blogger_id' => $stream->blogger_id,
                'correlation_id' => $correlationId,
            ]);

            $this->eventDispatcher->dispatch(new StreamStarted($stream));
    ...
    public function endStream(int $streamId, string $correlationId = ''): Stream
    {
        ...
        return $this->db->transaction(function () use ($stream, $correlationId) {
            $durationSeconds = $stream->started_at ? CarbonImmutable::now()->diffInSeconds($stream->started_at) : 0;
            ...
            // Audit log
            $this->logger->$this->logger->info('Stream ended', [
                'stream_id' => $stream->id,
                'duration_seconds' => $durationSeconds,
            ...
            $this->eventDispatcher->dispatch(new StreamEnded($stream));
     ...
     */
    public function getActiveStreams(): Collection
    {
     ...
     */
    public function getBloggerStreams(int $bloggerId, ?string $status = null): Collection
    {
