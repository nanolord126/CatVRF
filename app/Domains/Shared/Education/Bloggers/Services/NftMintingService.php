...
final readonly class NftMintingService
{
    ...
    public function createGift(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('NFT gift created', [
                'gift_id' => $gift->id,
            ...
            if ($this->config->get('bloggers.nft_gifts.auto_mint_enabled')) {
                MintNftGiftJob::$this->bus->dispatch($gift->id, $correlationId)
                    ->onQueue('nft-minting')
                    ->delay(CarbonImmutable::now()->addSeconds(5));
            }
    ...
    public function mintGift(int $giftId, string $correlationId = ''): NftGift
    {
        ...
        try {
            // Acquire lock
            if (! $this->redis->connection()->set($lockKey, $lockId, 'EX', self::REDIS_LOCK_TIMEOUT, 'NX')) {
            ...
            return $this->db->transaction(function () use ($gift, $correlationId) {
                ...
                try {
                    ...
                    $gift->update([
                        ...
                        'minted_at' => CarbonImmutable::now(),
                        'nft_address' => $simulatedNftAddress,
                        'ton_tx_hash' => $simulatedTxHash,
                        'upgrade_eligible_at' => CarbonImmutable::now()->addDays(14),
                    ]);

                    $this->logger->$this->logger->info('NFT gift minted', [
                        'gift_id' => $gift->id,
                        'nft_address' => $simulatedNftAddress,
                        'ton_tx_hash' => $simulatedTxHash,
                        'correlation_id' => $correlationId,
    ...
    public function upgradeToCollectorNft(int $giftId, string $correlationId = ''): NftGift
    {
        ...
        return $this->db->transaction(function () use ($gift, $correlationId) {
            ...
            $this->logger->$this->logger->info('NFT gift upgraded to collector', [
                'gift_id' => $gift->id,
                'correlation_id' => $correlationId,
            ]);
     ...
     */
    public function getFailedMintAttempts(): Collection
    {
