...
final class ProfileController extends Controller
{
    ...
    public function update(Request $request): JsonResponse
    {
        ...
        $this->db->transaction(function () use ($profile, $validated, $request) {
            ...
            $this->logger->$this->logger->info('Blogger updated profile', [
    ...
    public function updateBanking(Request $request): JsonResponse
    {
        ...
        $this->db->transaction(function () use ($profile, $validated) {
            ...
            $this->logger->$this->logger->info('Blogger updated banking info', [
                'blogger_id' => $profile->id,
                'correlation_id' => $request->header('X-Correlation-ID') ?? \Str::uuid(),
    ...
    public function delete(Request $request): JsonResponse
    {
        ...
        $this->db->transaction(function () use ($profile, $validated) {
            ...
            $this->logger->$this->logger->info('Blogger account deactivated', [
                'blogger_id' => $profile->id,
                'correlation_id' => $request->header('X-Correlation-ID') ?? \Str::uuid(),
    ...
    public function requestPayout(Request $request): JsonResponse
    {
        ...
        $this->db->transaction(function () use ($profile, $validated) {
            ...
            $this->logger->$this->logger->info('Blogger requested payout', [
                'blogger_id' => $profile->id,
                'amount' => $validated['amount'],
                'correlation_id' => $request->header('X-Correlation-ID') ?? \Str::uuid(),
            ]);
        ...
        return new \Illuminate\Http\JsonResponse([
            ...
            'amount' => $validated['amount'],
            'estimated_payment' => CarbonImmutable::now()->addDays(4),
        ]);
    }
}
