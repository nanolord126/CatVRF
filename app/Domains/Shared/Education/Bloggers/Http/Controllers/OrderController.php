...
use App\Domains\Content\Bloggers\Models\StreamOrder;
...
final class OrderController extends Controller
{
    ...
    public function store(CreateOrderRequest $request, string $roomId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Live order created', [
    ...
    public function confirmPayment(ConfirmPaymentRequest $request, int $orderId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Order payment confirmed', [
                'correlation_id' => $correlationId,
                'order_id' => $orderId,
                'user_id' => $userId,
            ]);
    ...
    {
        try {
            $order = StreamOrder::with('product', 'stream')
                ->findOrFail($orderId);
    ...
    public function getUserOrders(Request $request): JsonResponse
    {
        try {
            ...
            $orders = StreamOrder::where('user_id', $userId)
                ->where('tenant_id', tenant()->id)
                ->with('product', 'stream')
