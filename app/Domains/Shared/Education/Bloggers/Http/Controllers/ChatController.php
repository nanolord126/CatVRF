...
use App\Domains\Content\Bloggers\Events\ChatMessagePosted;
...
final class ChatController extends Controller
{
    ...
    public function send(SendChatMessageRequest $request, string $roomId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Chat message sent', [
                'correlation_id' => $correlationId,
                'stream_id' => $stream->id,
                'user_id' => $userId,
            ...
            broadcast(new ChatMessagePosted(
                roomId: $roomId,
                message: $message,
            ))->toOthers();
    ...
    public function delete(int $messageId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Chat message deleted', [
                'correlation_id' => $correlationId,
    ...
    public function pin(int $messageId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('Chat message pinned', [
                'correlation_id' => $correlationId,
                'message_id' => $messageId,
