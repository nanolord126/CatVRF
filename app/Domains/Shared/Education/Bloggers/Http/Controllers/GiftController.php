...
use App\Domains\Content\Bloggers\Jobs\MintNftGiftJob;
...
final class GiftController extends Controller
{
    ...
    public function upgrade(int $giftId): JsonResponse
    {
        try {
            ...
            $this->logger->$this->logger->info('NFT gift upgraded', [
                'correlation_id' => $correlationId,
                'gift_id' => $giftId,
                'user_id' => $userId,
            ]);
    ...
    public function retryMinting(int $giftId): JsonResponse
    {
        try {
            ...
            // Queue for reminting
            MintNftGiftJob::$this->bus->dispatch($gift)
            ...
            $this->logger->$this->logger->info('NFT minting retried', [
                'correlation_id' => $correlationId,
                'gift_id' => $giftId,
            ]);
