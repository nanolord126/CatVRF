...
final readonly class SurgePricingService
{
    ...
    public function calculateSurge(float $lat, float $lon, string $vertical): array
    {
        ...
        return $this->cache->remember($cacheKey, 300, function () use ($lat, $lon) {
            ...
            $activeSurge = SurgeZone::where('geo_zone_id', $geoZone->id)
                ...
                ->where(function ($query) {
                    $query->whereNull('active_from')->orWhere('active_from', '<=', CarbonImmutable::now());
                })
                ->where(function ($query) {
                    $query->whereNull('active_until')->orWhere('active_until', '>=', CarbonImmutable::now());
                })
    ...
    public function createDynamicSurge(int $geoZoneId, float $multiplier, string $reason, int $minutes, string $correlationId): SurgeZone
    {
        $surge = SurgeZone::create([
            ...
            'active_until' => CarbonImmutable::now()->addMinutes($minutes),
            'correlation_id' => $correlationId,
        ]);

        $this->logger->$this->logger->info('Dynamic surge created', [
            'geo_zone_id' => $geoZoneId,
            'multiplier' => $multiplier,
            'reason' => $reason,
            'correlation_id' => $correlationId,
