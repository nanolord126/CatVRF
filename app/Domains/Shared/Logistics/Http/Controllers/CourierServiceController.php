...
final class CourierServiceController extends Controller
{
    ...
    public function updateShipmentStatus(int $shipmentId): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($shipmentId, $correlationId) {
                ...
                $this->logger->$this->logger->info('Shipment status updated', ['shipment_id' => $shipmentId, 'correlation_id' => $correlationId]);
            });
    ...
    public function verifyCourier(int $courierId): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($courier, $correlationId) {
                ...
                $this->logger->$this->logger->info('Courier verified', ['courier_id' => $courier->id, 'correlation_id' => $correlationId]);
            });
    ...
    public function delete(int $id): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($courier, $correlationId) {
                $courier->delete();
                $this->logger->$this->logger->info('Courier deleted', ['courier_id' => $id, 'correlation_id' => $correlationId]);
            });
