...
final readonly class UserTasteService
{
    ...
    public function getColdStartRecommendations(int $userId, string $correlationId = ''): array
    {
        ...
        $trendingCategories = $this->db->table('order_items')
            ...
            ->where('orders.created_at', '>=', $this->carbon->CarbonImmutable::now()->subDays(7))
            ->groupBy('category_id')
        ...
        foreach ($popularProducts as $product) {
            if (in_array($product->category_id, $trendingCategories, true)) {
        ...
        $this->logger->$this->logger->info('Cold start recommendations generated', [
            'user_id' => $userId,
