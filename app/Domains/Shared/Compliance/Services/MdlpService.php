...
final readonly class MdlpService
{
    ...
    public function verifyKiz(string $kizCode, string $token, string $correlationId = ''): bool
    {
        ...
        try {
            ...
            ComplianceRecord::create([
                ...
                'verified_at' => $this->carbon->CarbonImmutable::now(),
                'response_data' => $response->json(),
                'correlation_id' => $correlationId,
            ]);

            $this->logger->$this->logger->info('MDLP KIZ verification completed', [
                'kiz' => $kizCode,
                'is_valid' => $isValid,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function withdrawFromCirculation(string $kizCode, string $token, string $correlationId = ''): bool
    {
        ...
        try {
            ...
            $this->logger->$this->logger->info('MDLP KIZ withdrawn from circulation', [
                'kiz' => $kizCode,
                'success' => $isSuccess,
                'correlation_id' => $correlationId,
            ]);
