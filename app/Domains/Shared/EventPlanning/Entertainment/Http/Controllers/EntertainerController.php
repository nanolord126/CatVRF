...
use App\\Domains\\Shared\EventPlanning\Entertainment\Models\EntertainmentEvent;
...
final class EntertainerController extends Controller
{
    ...
    {
        try {
            $events = EntertainmentEvent::where('entertainer_id', $id)
    ...
    public function register(): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($correlationId) {
                ...
                $this->logger->$this->logger->info('Entertainer registered', [
    ...
    public function updateProfile(): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($entertainer, $correlationId) {
                ...
                $this->logger->$this->logger->info('Entertainer profile updated', [
                    'entertainer_id' => $entertainer->id,
                    'correlation_id' => $correlationId,
                ]);
            });
    ...
    public function updateSchedule(): JsonResponse
    {
        try {
            ...
            $this->db->transaction(function () use ($entertainer, $correlationId) {
                ...
                $this->logger->$this->logger->info('Entertainer schedule updated', [
                    'entertainer_id' => $entertainer->id,
                    'correlation_id' => $correlationId,
                ]);
            });
