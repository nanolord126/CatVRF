...
namespace App\Domains\Shared\EventPlanning\TeamBuilding\Services;

use App\Domains\Shared\EventPlanning\TeamBuilding\Models\TeamBuildingEvent;
...
final readonly class TeamBuildingService
{
    ...
    public function createEvent(
        ...
        return $this->db->transaction(function () use ($facilitatorId, $eventType, $hoursSpent, $participantsCount, $priceKopecks, $correlationId, $userId): TeamBuildingEvent {
            ...
            $this->logger->$this->logger->info('Team building event created', [
                'event_id' => $event->id,
                'correlation_id' => $correlationId,
            ]);
     ...
     */
    public function getUserEvents(int $clientId, int $limit = 10): Collection
    {
