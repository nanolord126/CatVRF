...
use App\Domains\Pet\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $petOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->petOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Pet order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->petOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->petOrderService->getDeliveryEstimate($address);
