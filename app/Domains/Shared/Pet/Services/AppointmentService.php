        ...
        try {
            return $this->db->transaction(function () use ($data, $correlationId) {
                ...
                $appointment = PetAppointment::create([
                    ...$data,
                    'tenant_id' => tenant()->id,
                    'appointment_number' => 'APT-'.CarbonImmutable::now()->format('Ym').'-'.Str::random(6),
                    'commission_amount' => ($data['price'] ?? 0) * 0.14,
                    'correlation_id' => $correlationId,
                    'uuid' => Str::uuid(),
                ]);

                AppointmentBooked::$this->bus->dispatch($appointment, $correlationId);

                $this->logger->$this->logger->info('Pet appointment created', [
    ...
    public function cancelAppointment(PetAppointment $appointment, ?string $correlationId = null): PetAppointment
    {
        ...
        try {
            return $this->db->transaction(function () use ($appointment, $correlationId) {
                ...
                $this->logger->$this->logger->info('Pet appointment cancelled', [
                    'appointment_id' => $appointment->id,
                    'correlation_id' => $correlationId,
                ]);
