...
final readonly class PetVerticalService
{
    ...
    public function createAppointment(array $data): PetAppointment
    {
        ...
        return $this->db->transaction(function () use ($data, $correlationId) {
            ...
            $this->logger->$this->logger->info('Creating pet appointment', [
                'pet_id' => $data['pet_id'],
            ...
            $this->logger->$this->logger->info('Pet appointment created successfully', [
    ...
    public function completeAppointment(int $appointmentId): bool
    {
        return $this->db->transaction(function () use ($appointmentId) {
            ...
            $appointment->update([
                'status' => 'completed',
                'payment_status' => 'paid',
                'metadata' => array_merge($appointment->metadata ?? [], ['completed_at' => CarbonImmutable::now()]),
            ]);
            ...
            $this->logger->$this->logger->info('Pet appointment completed', [
                'appointment_id' => $appointment->id,
     ...
     */
    public function analyzeHealthFromPhoto(UploadedFile $photo, int $petId): array
    {
        $correlationId = (string) Str::uuid();
        $this->logger->$this->logger->info('AI Health analysis started', ['pet_id' => $petId, 'correlation_id' => $correlationId]);
    ...
    public function purchaseProduct(int $productId, int $quantity): bool
    {
        return $this->db->transaction(function () use ($productId, $quantity) {
            ...
            $this->logger->$this->logger->info('Pet product purchased', [
