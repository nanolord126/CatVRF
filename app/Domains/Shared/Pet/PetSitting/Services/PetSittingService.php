...
final readonly class PetSittingService
{
    ...
    public function createBooking(int $sitterId, $startDate, $endDate, $petNames, string $correlationId = ''): SittingBooking
    {
        $correlationId = $correlationId ?: (string) Str::uuid();
        if ($this->rateLimiter->tooManyAttempts('petsit:book:'.$this->guard->id(), 14)) {
            throw new \RuntimeException('Too many', 429);
        }$this->rateLimiter->hit('petsit:book:'.$this->guard->id(), 3600);

        return $this->db->transaction(function () use ($sitterId, $startDate, $endDate, $petNames, $correlationId) {
            $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'petsit_booking', amount: 0, correlationId: $correlationId ?? '');
            if ($fraud['decision'] === 'block') {
                throw new \RuntimeException('Security', 403);
            }$b = SittingBooking::create(['uuid' => Str::uuid(), 'tenant_id' => tenant()->id, 'sitter_id' => $sitterId, 'owner_id' => $this->guard->id() ?? 0, 'correlation_id' => $correlationId, 'status' => 'pending_payment', 'total_kopecks' => $total, 'payout_kopecks' => $total - (int) ($total * 0.14), 'payment_status' => 'pending', 'start_date' => $startDate, 'end_date' => $endDate, 'pet_names' => $petNames, 'tags' => ['petsit' => true]]);
            $this->logger->$this->logger->info('Pet sitting booking created', ['booking_id' => $b->id, 'correlation_id' => $correlationId]);

            return $b;
        });
    }

    public function completeBooking(int $bookingId, string $correlationId = ''): SittingBooking
    {
        $correlationId = $correlationId ?: (string) Str::uuid();

        return $this->db->transaction(function () use ($bookingId, $correlationId) {
            $b = SittingBooking::findOrFail($bookingId);
    ...
    public function cancelBooking(int $bookingId, string $correlationId = ''): SittingBooking
    {
        $correlationId = $correlationId ?: (string) Str::uuid();
...
return $b;
    ...
    public function getBooking(int $bookingId): SittingBooking
    {
        return SittingBooking::findOrFail($bookingId);
    }

    public function getUserBookings(int $ownerId)
    {
        return SittingBooking::where('owner_id', $ownerId)->orderBy('created_at', 'desc')->take(10)->get();
    }
