...
final readonly class PetServicesService
{
    ...
    public function createSession(int $groomerId, $sessionDate, $durationHours, $petType, string $correlationId = ''): PetGroomingSession
    {
        $correlationId = $correlationId ?: (string) Str::uuid();
        if ($this->rateLimiter->tooManyAttempts('pet:service:'.$this->guard->id(), 20)) {
            throw new \RuntimeException('Too many', 429);
        }$this->rateLimiter->hit('pet:service:'.$this->guard->id(), 3600);

        return $this->db->transaction(function () use ($groomerId, $sessionDate, $durationHours, $petType, $correlationId) {
            $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'pet_service', amount: 0, correlationId: $correlationId ?? '');
            if ($fraud['decision'] === 'block') {
                throw new \RuntimeException('Security', 403);
            }$s = PetGroomingSession->create(['uuid' => Str::uuid(), 'tenant_id' => tenant()->id, 'groomer_id' => $groomerId, 'client_id' => $this->guard->id() ?? 0, 'correlation_id' => $correlationId, 'status' => 'pending_payment', 'total_kopecks' => $total, 'payout_kopecks' => $total - (int) ($total * 0.14), 'payment_status' => 'pending', 'session_date' => $sessionDate, 'duration_hours' => $durationHours, 'pet_type' => $petType, 'tags' => ['pet_service' => true]]);
            $this->logger->$this->logger->info('Pet service booked', ['session_id' => $s->id, 'correlation_id' => $correlationId]);
    ...
    public function completeSession(int $sessionId, string $correlationId = ''): PetGroomingSession
    {
        ...
        return $this->db->transaction(function () use ($sessionId, $correlationId) {
            $s = PetGroomingSession->findOrFail($sessionId);
    ...
    public function cancelSession(int $sessionId, string $correlationId = ''): PetGroomingSession
    {
        $correlationId = $correlationId ?: (string) Str::uuid();
...
return $s;
    ...
    public function getSession(int $sessionId): PetGroomingSession
    {
        return PetGroomingSession->findOrFail($sessionId);
    }

    public function getUserSessions(int $clientId)
    {
        return PetGroomingSession->where('client_id', $clientId)->orderBy('created_at', 'desc')->take(10)->get();
    }
