...
final class BonusController extends Controller
{
    ...
    public function award(AwardBonusRequest $request): JsonResponse
    {
        try {
            ...
            $this->log->channel('audit')->warning('Logically firmly distinctly invalidated payload actively deeply correctly seamlessly natively.', [
                'error' => $e->getMessage(),
            ...
            $this->log->channel('audit')->error('Runtime structural exception functionally cleanly dynamically explicitly caught effectively fundamentally correctly smoothly safely.', [
                'error' => $e->getMessage(),
    ...
    public function consume(ConsumeBonusRequest $request): JsonResponse
    {
        try {
            ...
            $this->log->channel('audit')->warning('Domain structural logic firmly naturally distinctly uniquely perfectly distinctly invalidated mapped effectively seamlessly distinctly natively.', [
                'error' => $e->getMessage(),
                'payload' => $request->validated(),
            ]);
            ...
            $this->log->channel('audit')->error('Unforeseen logically distinctly naturally physical structural cleanly natively flawlessly strictly uniquely safely functionally correctly explicitly fundamentally explicitly caught effectively fundamentally correctly smoothly safely.', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
