...
final readonly class FraudMLService
{
    ...
    public function scoreOperation(OperationDto $dto): float
    {
        ...
        if ($this->config->get('fraud-ml.drift.real_time_enabled', false)) {
            try {
                $driftResult = $this->modelDriftService->monitorRealTime(
                    features: $features,
                    prediction: $activeScore,
                    modelType: 'fraud_ml_ensemble',
                    verticalCode: $dto->vertical_code ?? 'default'
                );

                if ($driftResult->driftDetected) {
                    $this->logger->warning('Model drift detected during prediction', [
                ...
                // Non-blocking: log error but don't fail the prediction
                $this->logger->warning('Drift monitoring failed', [
                    'error' => $e->getMessage(),
                    'correlation_id' => $dto->correlation_id,
                ]);
            }
        ...
        $this->logger->$this->logger->info('Fraud ML Score calculated', [
            'correlation_id' => $dto->correlation_id,
     ...
     * @param  Carbon  $dateFrom  Start date
     * @param  Carbon  $dateTo  End date
     * @return Collection Training data
     */
    public function gatherTrainingData(Carbon $dateFrom, Carbon $dateTo): Collection
    {
     ...
     *
     * @param  Collection  $trainingData  Training data
     * @return string Model version
     */
    public function trainModel(Collection $trainingData): string
    {
        // In production, this would call Python ML service for actual training
        $version = 'v'.CarbonImmutable::now()->format('Y-m-d-His');

        $this->logger->$this->logger->info('Model trained', [
            'version' => $version,
    ...
    public function switchToModel(string $modelVersion): void
    {
        ...
        cache(['fraud_model_active_version' => $modelVersion], CarbonImmutable::now()->addHours(24));
        ...
        $this->logger->$this->logger->info('Model switched', [
    ...
    private function performShadowInference(array $features, OperationDto $dto): void
    {
        ...
        foreach ($shadowModels as $shadowModel) {
            try {
                ...
                $this->logger->$this->logger->info('FraudML shadow prediction', [
                    'correlation_id' => $dto->correlation_id,
                    'operation' => $dto->operation_type,
