...
use App\Domains\FraudML\Services\PaymentFeatureDriftMonitor;
...
final readonly class PaymentFraudMLService
{
    ...
    public function __construct(
        ...
        private readonly PaymentFraudMLShadowService $shadowService,
        private readonly PaymentFeatureDriftMonitor $driftMonitor,
    ) {
    }
    ...
    public function scorePayment(PaymentFraudMLDto $dto): array
    {
        ...
        if ($cached !== null) {
            ...
            $this->logger->$this->logger->info('Payment fraud score loaded from cache', [
        ...
        $this->logger->$this->logger->info('Payment fraud score calculated', [
            'idempotency_key' => $dto->idempotency_key,
            'correlation_id' => $dto->correlation_id,
            'vertical_code' => $dto->vertical_code,
    ...
    public function invalidateCache(string $idempotencyKey): void
    {
        ...
        $this->logger->$this->logger->info('Payment fraud score cache invalidated', [
    ...
    {
        try {
            $shadowResults = $this->shadowService->performShadowInference($dto, $features);

            if ($shadowResults !== null && count($shadowResults) > 0) {
                $this->logger->$this->logger->info('Payment fraud shadow inference completed', [
                    'correlation_id' => $dto->correlation_id,
    ...
    private function recordFeaturesForDriftMonitoring(array $features, PaymentFraudMLDto $dto): void
    {
        try {
            ...
            $this->driftMonitor->recordFeature(
