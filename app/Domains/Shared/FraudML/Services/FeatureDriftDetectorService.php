...
final readonly class FeatureDriftDetectorService
{
    ...
    public function detectDriftForFeature(
        ...
        $this->logger->$this->logger->info('Feature drift detection started', [
            'feature' => $featureName,
            'vertical' => $vertical,
    ...
    public function storeReferenceDistribution(string $featureName, string $vertical, array $referenceData): bool
    {
        ...
        try {
            $this->cache->tags(['ml', 'feature_drift', "vertical:{$vertical}"])
                ->put($cacheKey, $referenceData, CarbonImmutable::now()->addHours(self::CACHE_TTL_HOURS));

            $this->logger->$this->logger->info('Reference distribution stored', [
                'feature' => $featureName,
                'vertical' => $vertical,
    ...
    public function getReferenceDistribution(string $featureName, string $vertical): ?array
    {
        ...
        $data = $this->cache->tags(['ml', 'feature_drift', "vertical:{$vertical}"])
            ->get($cacheKey);
    ...
    {
        $this->cache->tags(['ml', 'feature_drift', "vertical:{$vertical}"])->flush();

        $this->logger->$this->logger->info('Reference distribution cache invalidated', [
            'vertical' => $vertical,
        ]);
    }
