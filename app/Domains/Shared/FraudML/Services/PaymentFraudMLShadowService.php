...
final readonly class PaymentFraudMLShadowService
{
    ...
    public function performShadowInference(PaymentFraudMLDto $dto, array $features): ?array
    {
        ...
        foreach ($shadowModels as $shadowModel) {
            try {
                ...
                $this->log->warning('Shadow inference failed for payment', [
    ...
    public function setShadowTrafficSplit(int $percentage): void
    {
        ...
        $this->cache->put(self::SHADOW_TRAFFIC_SPLIT_KEY, $percentage, CarbonImmutable::now()->addHours(24));

        $this->log->$this->logger->info('Payment fraud shadow traffic split updated', [
    ...
    public function getShadowModelStatistics(): array
    {
        ...
        foreach ($shadowModels as $model) {
            $stats[] = [
                ...
                'shadow_period_hours' => CarbonImmutable::now()->diffInHours($model->created_at),
    ...
    public function promoteShadowModel(string $modelVersion): bool
    {
        ...
        if ($shadowModel === null) {
            $this->log->error('Shadow model not found for promotion', [
        ...
        if (! $this->isShadowModelReady($shadowModel)) {
            $this->log->warning('Shadow model not ready for promotion', [
                'model_version' => $modelVersion,
                'shadow_predictions_count' => $shadowModel->shadow_predictions_count,
                'shadow_period_hours' => CarbonImmutable::now()->diffInHours($shadowModel->created_at),
            ]);
        ...
        $this->log->$this->logger->info('Payment fraud shadow model promoted', [
            'model_version' => $modelVersion,
            'shadow_predictions_count' => $shadowModel->shadow_predictions_count,
        ]);
    ...
    private function shouldShadowRequest(PaymentFraudMLDto $dto): bool
    {
        ...
        $trafficSplit = $this->cache->get(self::SHADOW_TRAFFIC_SPLIT_KEY, 10);
    ...
    private function isShadowModelReady(FraudModelVersion $shadowModel): bool
    {
        ...
        $shadowPeriodHours = CarbonImmutable::now()->diffInHours($shadowModel->created_at);
    ...
    {
        $this->log->$this->logger->info('Payment fraud shadow prediction', [
            'correlation_id' => $dto->correlation_id,
