...
final class ModelVersionUpdatedListener
{
    ...
    {
        $this->log->channel('audit')->$this->logger->info('ModelVersionUpdated event received', [
    ...
    private function invalidateCache(ModelVersionUpdated $event): void
    {
        ...
        $this->cache->tags(['fraud_ml', 'model_versions'])->flush();

        $this->log->debug('Fraud model cache invalidated', [
            'model_version' => $event->modelVersion->version,
            'action' => $event->action,
        ]);
    }
    ...
    private function updateMetrics(ModelVersionUpdated $event): void
    {
        ...
        if ($event->action === 'promoted') {
            ...
            $metrics['ml_model_promoted_timestamp'] = CarbonImmutable::now()->timestamp;
        }

        $this->log->debug('ML metrics updated', [
            'metrics' => $metrics,
            'correlation_id' => $event->correlationId,
        ]);
    }
    ...
    private function notifyDependentServices(ModelVersionUpdated $event): void
    {
        ...
        $this->log->$this->logger->info('Notifying dependent services about model update', [
            'model_version' => $event->modelVersion->version,
            'action' => $event->action,
