...
use App\Domains\Marketplace\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $marketplaceOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->marketplaceOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Marketplace order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->marketplaceOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->marketplaceOrderService->getDeliveryEstimate($address);
