...
namespace App\Domains\Marketplace\Services\AI;
...
final readonly class MarketplaceConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'marketplace_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:Marketplace:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('Marketplace', $fullProfile, $userId);

            $this->logger->$this->logger->info('Marketplace AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Анализ торговой площадки. Подбери оптимальные листинги по запросу покупателя.',
            ];
        });
    }
}
