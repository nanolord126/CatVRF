...
namespace App\Domains\Shared\Insurance\AssuranceServices\Services;

use App\Domains\Shared\Insurance\AssuranceServices\Models\QualityAudit;
...
final readonly class AssuranceServicesService
{
    ...
    public function createAudit(
        ...
        return $this->db->transaction(function () use ($auditorId, $auditType, $hoursSpent, $dueDate, $correlationId): QualityAudit {
            ...
            $this->logger->$this->logger->info('Quality audit created', [
                'audit_id'       => $audit->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function completeAudit(int $auditId, string $correlationId = ''): QualityAudit
    {
        ...
        return $this->db->transaction(function () use ($auditId, $correlationId): QualityAudit {
            ...
            $this->logger->$this->logger->info('Quality audit completed', [
                'audit_id'       => $audit->id,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function cancelAudit(int $auditId, string $correlationId = ''): QualityAudit
    {
        ...
        return $this->db->transaction(function () use ($auditId, $correlationId): QualityAudit {
            ...
            $this->logger->$this->logger->info('Quality audit cancelled', [
                'audit_id'       => $audit->id,
                'refunded'       => $wasPaid,
                'correlation_id' => $correlationId,
            ]);
     ...
     *
     * @return Collection<int, QualityAudit>
     */
    public function getUserAudits(int $clientId, int $limit = 10): Collection
    {
