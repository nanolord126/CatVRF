...
namespace App\Domains\Shared\Insurance\Http\Controllers;
...
final class InsuranceController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $items = $this->db->table('insurance_policies')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            'type' => 'required|string', 'coverage_amount' => 'required|numeric|min:0', 'premium' => 'required|numeric|min:0', 'start_date' => 'required|date', 'end_date' => 'required|date|after:start_date',
        ]);
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        $item = $this->db->table('insurance_policies')
            ->where('id', $id)
            ->where('tenant_id', $request->get('tenant_id'))
            ->first();

        if ($item === null) {
            return new JsonResponse(['correlation_id' => $correlationId, 'message' => 'Страховой полис не найден(а)'], 404);
        }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $validated = $request->validate([
            'type' => 'sometimes|string', 'coverage_amount' => 'sometimes|numeric|min:0', 'premium' => 'sometimes|numeric|min:0', 'status' => 'sometimes|string|in:active,expired,cancelled',
        ]);
