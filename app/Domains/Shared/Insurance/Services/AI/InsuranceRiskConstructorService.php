...
namespace App\Domains\Shared\Insurance\Services\AI;
...
final readonly class InsuranceRiskConstructorService
{
    ...
    public function analyzeAndRecommend(array $riskData, int $userId): array
    {
        ...
        // 1. AI — анализ данных
        $inputData = array_merge($this->getInputData($riskData, $userId), ['risk_profile' => true]);
        $inputJson = json_encode($inputData);

        // Критично: анонимизация финансовых данных перед отправкой в OpenAI
        $anonymizedInput = $this->anonymizeFinancialData($inputJson);

        try {
            $response = $this->circuitBreaker->call(function () use ($anonymizedInput) {
                return $this->openai->chat([
                    ['role' => 'system', 'content' => 'Анализ рисков и подбор оптимального страхового продукта. Определи: объект страхования, риски, бюджет, срок. Рекомендуй полисы, страховые суммы, франшизы.'],
            ...
            throw new \RuntimeException('Failed to get risk analysis. Please try again later.');
        }
        ...
        // 3. Разбор ответа AI
        $risk_profile = $this->parseAnalysis($analysisText);

        // 4. Персонализация по вкусам
        $risk_profile['taste_enrichment'] = $tasteProfile->toArray();
        ...
        $this->saveToUserProfile($userId, 'insurance', $risk_profile, $correlationId);
    ...
    private function anonymizeFinancialData(string $data): string
    {
        // Критично для финансовой вертикали - строгая анонимизация ПДн
        $patterns = [
    ...
    private function getInputData(array $riskData, int $userId): array
    {
        return [
            'insured_object' => $riskData['insured_object'] ?? null,
            'risks' => $riskData['risks'] ?? [],
            'budget' => $riskData['budget'] ?? null,
            'term' => $riskData['term'] ?? null,
        ];
    }
}
