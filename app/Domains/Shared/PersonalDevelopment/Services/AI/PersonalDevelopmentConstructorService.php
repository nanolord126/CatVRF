...
namespace App\Domains\PersonalDevelopment\Services\AI;
...
final readonly class PersonalDevelopmentConstructorService
{
    ...
    public function analyzeAndRecommend(array $payload, int $userId): array
    {
        ...
        $this->fraud->check(userId: $this->guard->id() ?? 0, operationType: 'personaldevelopment_ai_constructor', amount: 0, correlationId: $correlationId ?? '');

        $cacheKey = 'user_ai_designs:PersonalDevelopment:'.$userId.':'.md5(serialize($payload));
        ...
        return $this->cache->remember($cacheKey, CarbonImmutable::now()->addHour(), function () use ($payload, $userId, $correlationId) {
            ...
            $recommendations = $this->recommendation->getForVertical('PersonalDevelopment', $fullProfile, $userId);

            $this->logger->$this->logger->info('PersonalDevelopment AI constructor used', [
                'user_id'        => $userId,
            ...
            return [
                ...
                'correlation_id'  => $correlationId,
                'prompt_hint'     => 'Персональный план развития. Определи цели, сильные стороны и подбери инструменты.',
            ];
        });
    }
}
