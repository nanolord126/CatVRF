...
final readonly class ClinicService
{
    ...
    public function bookAppointment(int $userId, int $doctorId, int $serviceId, Carbon $dateTime, string $correlationId = ''): MedicalAppointment
    {
        ...
        return $this->db->transaction(function () use ($userId, $doctorId, $serviceId, $dateTime, $correlationId) {
            ...
            $this->logger->$this->logger->info('Medical: appointment created', ['appointment_id' => $appointment->id, 'corr' => $correlationId]);
    ...
    public function finishAppointment(int $appointmentId, array $results, ?string $prescription = null, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($appointment, $results, $prescription, $correlationId) {
            ...
            $this->logger->$this->logger->info('Medical: appointment finished', ['appointment_id' => $appointment->id]);
        });
    }
}
