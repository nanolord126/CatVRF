...
use App\Domains\Shared\Medical\Models\MedicalClinic;
...
final class CalculateClinicEarningsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $this->logger->error('Failed to calculate clinic earnings', [
                'error' => $e->getMessage(),
    ...
    private function calculateClinicEarnings(MedicalClinic $clinic, int $month, int $year): void
    {
        try {
            ...
            $this->logger->error('Failed to calculate earnings for clinic', [
                'clinic_id' => $clinic->id,
                'error' => $e->getMessage(),
                'correlation_id' => $this->correlationId,
            ]);
