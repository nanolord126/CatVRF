 ...
 *
 * Канон: fraud-check → $this->db->transaction → audit → event.
 */
final readonly class PayoutService
{
    ...
    public function calculateAndInitiatePayoutsForTenant(int $tenantId, CarbonImmutable $periodEnd): void
    {
        ...
        $this->logger->$this->logger->info('Payout calculation started', [
            'tenant_id'      => $tenantId,
        ...
        if ($totalEarnings <= 0) {
            $this->logger->$this->logger->info('No earnings for payout', [
                'tenant_id'      => $tenantId,
                'correlation_id' => $correlationId,
            ]);
    ...
    private function aggregateEarningsFromAllVerticals(
        ...
        foreach ($this->calculators as $calculator) {
            ...
            $this->logger->$this->logger->info('Vertical earnings calculated', [
                'tenant_id'      => $tenantId,
                'vertical'       => $vertical,
                'earnings'       => $earnings,
                'correlation_id' => $correlationId,
