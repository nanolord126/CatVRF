...
use App\Domains\Finances\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    public function __construct(
        ...
        private readonly OrderService $financesOrderService,
    ) {
        parent::__construct();
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $data = $request->all();
        $data['vertical'] = 'finances';

        $validation = $this->financesOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Finances order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->financesOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->financesOrderService->getDeliveryEstimate($address);
