...
final class PayoutsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make(
                ...
                ->description('Запланировано на '.CarbonImmutable::now()->addMonth()->startOfMonth()->format('d.m.Y'))
            ...
            Stat::make(
                ...
                ->description('Обработано '.CarbonImmutable::now()->startOfMonth()->format('d.m.Y'))
                ->color('success'),
            ...
            Stat::make(
                ...
                ->description(CarbonImmutable::now()->format('Y').' год')
                ->color('primary'),
        ];
    }
    ...
    private function getYearToDatePayoutsAmount(): int
    {
        return (int) FinanceRecord::query()
            ->where('type', 'payout')
            ->where('status', 'completed')
            ->whereYear('created_at', CarbonImmutable::now()->year)
            ->sum('amount');
    }
