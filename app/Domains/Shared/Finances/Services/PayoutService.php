 ...
 * CatVRF Canon 2026: все мутации — $this->db->transaction + fraud-check + correlation_id.
 */
final readonly class PayoutService
{
    ...
    public function initiatePayout(
        ...
        $this->logger->$this->logger->info('Payout initiation started', [
        ...
        if ($amountKopecks <= 0) {
            $this->logger->$this->logger->info('Payout skipped — zero or negative amount', [
                'wallet_id'      => $tenantWalletId,
                'correlation_id' => $correlationId,
            ]);
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Payout initiated successfully', [
                'wallet_id'      => $tenantWalletId,
                'amount'         => $amountKopecks,
                'period'         => $periodLabel,
                'correlation_id' => $correlationId,
    ...
    public function confirmPayout(
        ...
        $this->logger->$this->logger->info('Payout confirmed', [
            'wallet_id'      => $tenantWalletId,
            'amount'         => $amountKopecks,
            'correlation_id' => $correlationId,
        ]);
