...
final class FinanceRecordPolicy
{
     ...
     */
    public function $this->viewFactory->make(User $user, FinanceRecord $financeRecord): Response
    {
        if ($user->tenant_id !== $financeRecord->tenant_id) {
            return PolicyResponse::deny('Нет доступа к записям другого тенанта.');
        }

        if (! $this->canAccessBusinessGroup($user, $financeRecord)) {
            return PolicyResponse::deny('Нет доступа к записям этого филиала.');
        }
    ...
    {
        if ($user->tenant_id !== $financeRecord->tenant_id) {
            return PolicyResponse::deny('Нельзя редактировать записи другого тенанта.');
        }

        if (! $this->canAccessBusinessGroup($user, $financeRecord)) {
            return PolicyResponse::deny('Нельзя редактировать записи этого филиала.');
        }
    ...
    {
        if ($user->tenant_id !== $financeRecord->tenant_id) {
            return PolicyResponse::deny('Нельзя удалять записи другого тенанта.');
        }

        if (! $this->canAccessBusinessGroup($user, $financeRecord)) {
            return PolicyResponse::deny('Нельзя удалять записи этого филиала.');
        }

        if (($financeRecord->status ?? '') === 'completed') {
            return PolicyResponse::deny('Нельзя удалять завершённые финансовые записи.');
        }
    ...
    {
        if ($user->tenant_id !== $financeRecord->tenant_id) {
            return PolicyResponse::deny('Нельзя восстанавливать записи другого тенанта.');
        }

        if (! $this->canAccessBusinessGroup($user, $financeRecord)) {
            return PolicyResponse::deny('Нельзя восстанавливать записи этого филиала.');
        }
