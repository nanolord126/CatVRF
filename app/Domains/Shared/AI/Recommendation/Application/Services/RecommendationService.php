...
final readonly class RecommendationService
{
    ...
    public function getForUser(int $tenantId, int $userId, ?string $vertical, array $context, string $correlationId): Collection
    {
        ...
        $this->redis->setex($cacheKey, 300, json_encode($recommendations->toArray()));

        $this->logAction('recommendation_generated', 'Recommendation', null, [
            'user_id' => $userId,
            'vertical' => $vert,
    ...
    {
        $this->logAction('recommendation_item_scored', 'RecommendationItem', $itemId, [
            'user_id' => $userId,
            'correlation_id' => $correlationId,
        ], $userId, $tenantId);
