...
namespace App\Domains\Shared\Legal\Services\AI;
...
final readonly class LegalDocumentConstructorService
{
    ...
    public function analyzeAndRecommend(array $legalData, int $userId): array
    {
        ...
        try {
            ...
            throw new \RuntimeException('Failed to generate legal document. Please try again later.');
        }
        } catch (\Throwable $e) {
            $this->logger->error('OpenAI API call failed', [
                'error' => $e->getMessage(),
        ...
        $this->logger->$this->logger->info('LegalDocumentConstructorService used', [
            'user_id'        => $userId,
            'vertical'       => 'legal',
