...
final class EloquentCommissionRuleRepository implements CommissionRuleRepositoryInterface
{
    ...
    public function findByVerticalAndTenant(string $vertical, int $tenantId): ?CommissionRule
    {
        ...
        return $this->cache->remember($cacheKey, self::CACHE_TTL, function () use ($vertical, $tenantId) {
    ...
    {
        $this->cache->forget($this->getCacheKey($vertical, $tenantId));
        $this->cache->forget("commission_rules:tenant:{$tenantId}:all_active");
    }
