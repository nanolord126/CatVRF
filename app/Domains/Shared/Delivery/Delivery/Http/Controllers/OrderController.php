...
final class OrderController extends UniversalOrderController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $data = $request->all();
        $data['vertical'] = 'delivery';

        $validation = $this->deliveryOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Delivery order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->deliveryOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->deliveryOrderService->getDeliveryEstimate($address);
