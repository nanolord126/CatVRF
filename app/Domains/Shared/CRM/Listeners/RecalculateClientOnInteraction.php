...
final class RecalculateClientOnInteraction implements ShouldQueue
{
    ...
    public function handle(CrmInteractionRecorded $event): void
    {
        ...
        $this->logger->$this->logger->info('CRM: recalculating client stats after interaction', [
            'client_id' => $client->id,
        ...
        $this->logger->$this->logger->info('CRM: client stats recalculated and re-segmented', [
            'client_id' => $client->id,
            'correlation_id' => $event->correlationId,
        ]);
    }
