...
final readonly class FoodCrmService
{
    ...
    public function createFoodProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Food CRM profile created', [
    ...
    public function recordFoodOrder(
        ...
        $this->db->transaction(function () use ($client, $dishes, $amount, $correlationId, $channel, $rating): void {
                ...
                foreach ($dishes as $dish) {
                    ...
                    foreach ($favorites as &$fav) {
                        if (($fav['name'] ?? '') === ($dish['name'] ?? '')) {
                            ...
                            $fav['last_ordered'] = CarbonImmutable::now()->toDateString();
                            $found = true;
                            break;
                        }
                    }
                    ...
                    if (! $found) {
                        $favorites[] = [
                            'name' => $dish['name'] ?? '',
                            'order_count' => 1,
                            'last_ordered' => CarbonImmutable::now()->toDateString(),
                        ];
                    }
                }
            ...
            $this->logger->$this->logger->info('Food order recorded', [
                'client_id' => $client->id,
                'amount' => $amount,
