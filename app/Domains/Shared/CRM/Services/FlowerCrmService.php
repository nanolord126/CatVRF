...
final readonly class FlowerCrmService
{
    ...
    public function createFlowerProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Flower CRM profile created', [
    ...
    public function recordFlowerOrder(
        ...
        $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Flower CRM order recorded', [
                'client_id' => $client->id,
                'recipient' => $recipientName,
                'amount' => $amount,
                'occasion' => $occasion,
    ...
    public function addOccasion(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Flower CRM occasion added', [
                'profile_id' => $profile->id,
                'occasion' => $name,
                'date' => $date,
                'correlation_id' => $correlationId,
    ...
    public function getUpcomingOccasions(int $tenantId, int $daysAhead = 7): Collection
    {
        ...
        foreach ($clients as $client) {
            ...
            foreach ($profile->occasions ?? [] as $occasion) {
                ...
                $thisYear = $parsed->copy()->year(CarbonImmutable::now()->year);

                if ($thisYear->isPast()) {
                    $thisYear->addYear();
                }

                $daysUntil = CarbonImmutable::now()->diffInDays($thisYear, false);
    ...
    public function getUpcomingCorporateHolidays(int $tenantId, int $daysAhead = 14): Collection
    {
        ...
        foreach ($clients as $client) {
            ...
            foreach ($profile->corporate_holidays ?? [] as $holiday) {
                ...
                $daysUntil = CarbonImmutable::now()->diffInDays($parsed, false);
    ...
    private function updateFrequentRecipient(CrmFlowerClientProfile $profile, string $name, ?string $address): void
    {
        ...
        foreach ($recipients as &$recipient) {
            if (mb_strtolower($recipient['name'] ?? '') === mb_strtolower($name)) {
                ...
                $recipient['last_order_at'] = CarbonImmutable::now()->toIso8601String();
        ...
        if (! $found) {
            $recipients[] = [
                ...
                'address' => $address,
                'orders_count' => 1,
                'last_order_at' => CarbonImmutable::now()->toIso8601String(),
            ];
        }
