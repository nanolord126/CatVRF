...
final readonly class PetCrmService
{
    ...
    public function createPetProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Pet CRM profile created', [
    ...
    public function addPet(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Pet added to profile', [
                'profile_id' => $profile->id,
                'pet_name' => $name,
                'species' => $species,
                'correlation_id' => $correlationId,
    ...
    public function recordVaccination(
        ...
        return $this->db->transaction(function () use ($profile, $petName, $vaccineName, $correlationId, $nextDoseDate): CrmPetProfile {
            ...
            $this->logger->$this->logger->info('Pet vaccination recorded', [
                'profile_id' => $profile->id,
                'pet' => $petName,
                'vaccine' => $vaccineName,
                'correlation_id' => $correlationId,
    ...
    public function recordGrooming(
        ...
        return $this->db->transaction(function () use ($profile, $petName, $serviceType, $nextGroomingDate): CrmPetProfile {
            ...
            ->filter(function (CrmPetProfile $profile) use ($daysAhead): bool {
                foreach ($profile->vaccination_schedule ?? [] as $v) {
                    if (isset($v['next_dose']) && $v['next_dose'] !== null) {
                        $nextDose = Carbon::parse($v['next_dose']);
                        if ($nextDose->isBetween(CarbonImmutable::now(), CarbonImmutable::now()->addDays($daysAhead))) {
                            return true;
                        }
                    }
                }
