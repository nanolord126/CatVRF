...
final readonly class HotelCrmService
{
    ...
    public function createGuestProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Hotel CRM guest profile created', [
    ...
    public function recordStay(
        ...
        $this->db->transaction(function () use (
            ...
            if ($profile instanceof CrmHotelGuestProfile) {
                ...
                $profile->update(['last_stay_at' => CarbonImmutable::now()]);
            ...
            $client->update([
                'last_order_at' => CarbonImmutable::now(),
            ...
            $this->logger->$this->logger->info('Hotel CRM stay recorded', [
                'client_id' => $client->id,
                'room_type' => $roomType,
                'nights' => $nights,
                'amount' => $totalAmount,
    ...
    public function getGuestsForCheckinReminder(
        ...
        $targetTime = CarbonImmutable::now()->addHours($hoursBefore);
        $windowStart = $targetTime->copy()->subMinutes(30);
        $windowEnd = $targetTime->copy()->addMinutes(30);

        $this->logger->$this->logger->info('Searching guests for checkin reminder', [
            'tenant_id' => $tenantId,
    ...
    public function getGuestsForPostCheckoutReview(
        ...
        return CrmClient::query()
            ...
            ->filter(function (CrmClient $client): bool {
                ...
                $hoursSince = $checkout->diffInHours(CarbonImmutable::now());
    ...
    public function getUpgradeCandidates(int $tenantId, string $correlationId): Collection
    {
        return CrmClient::query()
            ...
            ->filter(function (CrmClient $client): bool {
                ...
                $hoursUntilCheckin = CarbonImmutable::now()->diffInHours(Carbon::parse($nextCheckin), false);
    ...
    private function updateAverageRating(CrmHotelGuestProfile $profile, float $newRating): void
    {
        ...
        $this->logger->channel('audit')->$this->logger->info(class_basename(self::class).': Record updated', [
            'id' => $profile->id ?? null,
            'correlation_id' => $correlationId,
        ]);
    }
