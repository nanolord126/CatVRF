...
final readonly class FurnitureCrmService
{
    ...
    public function createFurnitureProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Furniture CRM profile created', [
    ...
    public function saveRoomMeasurements(
        ...
        return $this->db->transaction(function () use ($profile, $roomName, $dimensions, $correlationId): CrmFurnitureProfile {
            ...
            $rooms[$roomName] = array_merge($dimensions, ['measured_at' => CarbonImmutable::now()->toDateString()]);
            ...
            $this->logger->$this->logger->info('Room measurements saved', [
                'profile_id' => $profile->id,
                'room' => $roomName,
                'correlation_id' => $correlationId,
            ]);
        ...
        return $this->db->transaction(function () use ($profile, $stageName, $status, $correlationId, $stageSpent, $comment): CrmFurnitureProfile {
            ...
            foreach ($stages as &$stage) {
                if (($stage['name'] ?? '') === $stageName) {
                    $stage['status'] = $status;
                    $stage['updated_at'] = CarbonImmutable::now()->toDateString();
                    if ($stageSpent !== null) {
            ...
            if (! $found) {
                $stages[] = [
                    ...
                    'updated_at' => CarbonImmutable::now()->toDateString(),
                ];
            }
