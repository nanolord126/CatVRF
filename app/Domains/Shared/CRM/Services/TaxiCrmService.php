...
final readonly class TaxiCrmService
{
    ...
    public function createTaxiProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Taxi CRM profile created', [
        ...
        return $this->db->transaction(function () use (
            ...
            foreach ($routes as &$route) {
                if (($route['key'] ?? '') === $routeKey) {
                    ...
                    $route['last_used'] = CarbonImmutable::now()->toDateString();
                    $routeFound = true;
                    break;
                }
            }
            ...
            if (! $routeFound) {
                $routes[] = [
                    ...
                    'count' => 1,
                    'last_used' => CarbonImmutable::now()->toDateString(),
                ];
            }

            $patterns = $profile->ride_time_patterns ?? [];
            $hourKey = (string) CarbonImmutable::now()->hour;
    ...
    public function saveAddress(
        ...
        return $this->db->transaction(function () use ($profile, $type, $address): CrmTaxiProfile {
            $data = match ($type) {
    ...
    public function analyzeRidePatterns(CrmTaxiProfile $profile): array
    {
        ...
        $topRoutes = new Collection($profile->frequent_routes ?? [])
            ->sortByDesc('count')
            ->take(5)
            ->values()
            ->toArray();
