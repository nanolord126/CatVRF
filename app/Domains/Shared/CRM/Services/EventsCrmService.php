...
final readonly class EventsCrmService
{
    ...
    public function createEventProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Events CRM profile created', [
    ...
    public function planEvent(
        ...
        return $this->db->transaction(function () use (
            ...
            $upcoming[] = [
                ...
                'created_at' => CarbonImmutable::now()->toDateString(),
            ];
                ...
                if (($event['event_id'] ?? '') === $eventId) {
                    ...
                    foreach ($checklist as &$item) {
                        if (($item['task'] ?? '') === $checklistItem) {
                            ...
                            $item['completed_at'] = $completed ? CarbonImmutable::now()->toDateString() : null;
                            break;
                        }
                    }
                    unset($item);
        ...
        return $this->db->transaction(function () use ($profile, $eventId, $correlationId, $rating, $feedback): CrmEventProfile {
            ...
            foreach ($upcoming as $event) {
                if (($event['event_id'] ?? '') === $eventId) {
                    ...
                    $event['completed_at'] = CarbonImmutable::now()->toDateString();
                    $past[] = $event;
                } else {
                    $newUpcoming[] = $event;
                }
            ...
            $this->logger->$this->logger->info('Event completed', [
                'profile_id' => $profile->id,
                'event_id' => $eventId,
                'rating' => $rating,
                'correlation_id' => $correlationId,
    ...
    public function addPreferredVendor(
        ...
        return $this->db->transaction(function () use ($profile, $vendorType, $vendorName, $contactInfo, $rating): CrmEventProfile {
            ...
            $list[] = [
                ...
                'rating' => $rating,
                'type' => $vendorType,
                'added_at' => CarbonImmutable::now()->toDateString(),
            ];
            ...
            ->filter(function (CrmEventProfile $profile) use ($daysAhead): bool {
                foreach ($profile->upcoming_events ?? [] as $event) {
                    if (isset($event['date'])) {
                        ...
                        if ($eventDate->isBetween(CarbonImmutable::now(), CarbonImmutable::now()->addDays($daysAhead))) {
                            return true;
                        }
                    }
                }
