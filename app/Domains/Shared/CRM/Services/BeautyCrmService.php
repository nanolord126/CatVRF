...
final readonly class BeautyCrmService
{
    ...
    public function createBeautyProfile(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Beauty CRM profile created', [
    ...
    public function updateMedicalCard(
        ...
        return $this->db->transaction(function () use ($profile, $data, $correlationId): CrmBeautyProfile {
            ...
            $this->logger->$this->logger->info('Beauty CRM medical card updated', [
                'profile_id' => $profile->id,
    ...
    public function addBeforeAfterPhoto(
        ...
        return $this->db->transaction(function () use (
            ...
            $this->logger->$this->logger->info('Beauty CRM before/after photo added', [
                'profile_id' => $profile->id,
                'procedure' => $procedure,
                'correlation_id' => $correlationId,
            ]);
     ...
     */
    public function getUpcomingBirthdays(int $tenantId, int $daysAhead = 7): Collection
    {
        return CrmClient::query()
            ...
            ->filter(function (CrmClient $client) use ($daysAhead): bool {
                ...
                return $bday->diffInDays(CarbonImmutable::now()) <= $daysAhead;
            });
    }
