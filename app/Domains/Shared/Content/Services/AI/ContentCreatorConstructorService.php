...
final readonly class ContentCreatorConstructorService
{
    ...
    public function analyzeAndRecommend(array $contentData, int $userId): array
    {
        ...
        // 1. AI — анализ данных
        $inputData = array_merge($this->getInputData($contentData, $userId), ['content_profile' => true]);
        ...
        try {
            ...
            throw new \RuntimeException('Failed to get content plan. Please try again later.');
        }
        ...
        $this->logger->$this->logger->info('ContentCreatorConstructorService used', [
            'user_id'        => $userId,
            'vertical'       => 'content',
    ...
    private function getInputData(array $contentData, int $userId): array
    {
        return [
            'topic' => $contentData['topic'] ?? null,
            'audience' => $contentData['audience'] ?? null,
            'platforms' => $contentData['platforms'] ?? [],
            'tone' => $contentData['tone'] ?? null,
            'frequency' => $contentData['frequency'] ?? null,
        ];
    }
}
