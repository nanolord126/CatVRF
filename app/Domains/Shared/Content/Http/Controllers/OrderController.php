...
use App\Domains\Content\Services\OrderService;
...
final class OrderController extends UniversalOrderController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $validation = $this->contentOrderService->validateOrder($data, $correlationId);

        if (! $validation['valid']) {
            $this->log->channel('audit')->warning('Content order validation failed', [
        ...
        if ($response->status() === 201) {
            ...
            $this->contentOrderService->sendOrderConfirmation(
                $data['user_id'],
                $data['id'],
                $correlationId
            );
    ...
    public function getDeliveryEstimate(Request $request): JsonResponse
    {
        ...
        $estimate = $this->contentOrderService->getDeliveryEstimate($address);
