...
namespace App\Domains\SupplyChainFinance\Services;
use App\Domains\SupplyChainFinance\Models\SCFAdvisor;
use App\Domains\SupplyChainFinance\Models\SCFEngagement;
...
return $this->db->transaction(function()use($advisorId,$engagementType,$hoursSpent,$dueDate,$correlationId){$a=SCFAdvisor::findOrFail($advisorId);$total=(int)($a->price_kopecks_per_hour*$hoursSpent);$fraud=$this->fraud->check(['user_id'=>auth()->id()??0,'operation_type'=>'scf','correlation_id'=>$correlationId,'amount'=>$total]);if($fraud['decision']==='block')throw new \RuntimeException("Security",403);$e=SCFEngagement::create(['uuid'=>Str::uuid(),'tenant_id'=>tenant()->id,'advisor_id'=>$advisorId,'client_id'=>auth()->id()??0,'correlation_id'=>$correlationId,'status'=>'pending_payment','total_kopecks'=>$total,'payout_kopecks'=>$total-(int)($total*0.14),'payment_status'=>'pending','engagement_type'=>$engagementType,'hours_spent'=>$hoursSpent,'due_date'=>$dueDate,'tags'=>['scf'=>true]]);$this->log->channel('audit')->info('SCF engagement created',['engagement_id'=>$e->id,'correlation_id'=>$correlationId]);return $e;});
}
public function completeEngagement(int $engagementId,string $correlationId=""):SCFEngagement{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($engagementId,$correlationId){$e=SCFEngagement::findOrFail($engagementId);if($e->payment_status!=='completed')throw new \RuntimeException("Not paid",400);$e->update(['status'=>'completed','correlation_id'=>$correlationId]);$this->wallet->credit(tenant()->id,$e->payout_kopecks,'scf_payout',['correlation_id'=>$correlationId,'engagement_id'=>$e->id]);$this->log->channel('audit')->info('SCF engagement completed',['engagement_id'=>$e->id]);return $e;});}
public function cancelEngagement(int $engagementId,string $correlationId=""):SCFEngagement{$correlationId=$correlationId?:(string)Str::uuid();return $this->db->transaction(function()use($engagementId,$correlationId){$e=SCFEngagement::findOrFail($engagementId);if($e->status==='completed')throw new \RuntimeException("Cannot cancel",400);$e->update(['status'=>'cancelled','payment_status'=>'refunded','correlation_id'=>$correlationId]);if($e->payment_status==='completed')$this->wallet->credit(tenant()->id,$e->total_kopecks,'scf_refund',['correlation_id'=>$correlationId,'engagement_id'=>$e->id]);$this->log->channel('audit')->info('SCF engagement cancelled',['engagement_id'=>$e->id]);return $e;});}
