...
final readonly class TravelBookingService
{
    ...
    public function bookTour(int $tourId, array $data, string $correlationId = ''): TravelBooking
    {
        ...
        return $this->db->transaction(function () use ($tourId, $data, $correlationId) {
            ...
            $this->logger->$this->logger->info('Travel: External HOLD requested', ['booking_id' => $booking->id, 'carrier' => 'Aeroflot', 'hotel' => 'Hilton Riyadh']);

            $this->logger->$this->logger->info('Travel: tour booked', ['booking_id' => $booking->id, 'agency' => $tour->agency->id, 'corr' => $correlationId]);
    ...
    public function finishTour(int $bookingId, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($booking, $correlationId) {
            ...
            $this->logger->$this->logger->info('Travel: payout processed', ['booking_id' => $booking->id, 'agency' => $booking->tour->agency->id]);
        });
    }
}
