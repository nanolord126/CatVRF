...
final class B2BHotelController extends Controller
{
    ...
    public function createStorefront(Request $request): JsonResponse
    {
        try {
            ...
            $this->db->transaction(fn () => B2BHotelStorefront::create(['uuid' => Str::uuid(), 'tenant_id' => $request->user()->tenant_id] + $v + ['correlation_id' => $cId]) && $this->logger->$this->logger->info('Hotel B2B: Storefront created', ['inn' => $v['inn'], 'correlation_id' => $cId]));
