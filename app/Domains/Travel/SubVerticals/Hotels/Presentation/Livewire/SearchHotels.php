...
namespace App\Domains\Travel\SubVerticals\Hotels\Presentation\Livewire;
...
final class SearchHotels extends Component
{
    ...
    #[Url(as: 'check_in')]
    private readonly string $checkInDate = '';

    #[Url(as: 'check_out')]
    private readonly string $checkOutDate = '';

    #[Url(as: 'guests')]
    private readonly int $capacity = 1;

    #[Url(as: 'sort')]
    private readonly string $sortBy = 'price_asc';

    // ===== Фильтры =====

    private readonly int $minPrice = 0;

    private readonly int $maxPrice = 0;

    private readonly float $minRating = 0.0;

    private readonly array $selectedAmenities = [];

    // ===== Состояние =====

    private readonly bool $isLoading = false;

    private readonly bool $hasSearched = false;

    private readonly string $errorMessage = '';

    private readonly Collection $hotels;

    private readonly int $totalFound = 0;
    ...
    public function mount(): void
    {
        ...
        $this->checkInDate  = CarbonImmutable::now()->format('Y-m-d');
        $this->checkOutDate = CarbonImmutable::now()->addDay()->format('Y-m-d');
    }
    ...
    {
        return $this->viewFactory->make('livewire.hotels.search-hotels');
    }
