...
final readonly class HotelBookingOrchestratorService
{
    ...
    private const LOCK_DURATION_SECONDS = 30;
    private const CACHE_PREFIX = 'hotels:booking:';
    private const AVAILABILITY_LOCK_PREFIX = 'hotels:availability:lock:';
    private const HOLD_PREFIX = 'hotels:hold:';
    ...
    public function createBookingWithHold(CreateBookingDto $dto, string $correlationId): array
    {
        ...
        $lockAcquired = $this->cache->lock($availabilityLockKey, self::LOCK_DURATION_SECONDS)->acquire();
        ...
        try {
            $result = $this->db->transaction(function () use (
                ...
                $booking->update([
                    ...
                    'hold_expires_at' => CarbonImmutable::now()->addMinutes(self::HOLD_DURATION_MINUTES),
                    'metadata' => [
                        'is_b2b' => $isB2B,
                        'inn' => $data['inn'] ?? null,
                ...
                $this->logger->$this->logger->info('Booking created with hold', [
            ...
            $this->cache->lock($availabilityLockKey)->release();
        }
    }
        ...
        $result = $this->db->transaction(function () use ($booking, $correlationId) {
            $booking->update([
                ...
                'metadata' => array_merge($booking->metadata ?? [], [
                    ...
                    'payment_confirmed_at' => CarbonImmutable::now()->toIso8601String(),
                ]),
            ]);
            ...
            $this->logger->$this->logger->info('Booking payment confirmed', [
                'booking_id' => $booking->id,
    ...
    public function instantCheckIn(int $bookingId, string $biometricToken, string $correlationId): array
    {
        ...
        $result = $this->db->transaction(function () use ($booking, $correlationId) {
            ...
            $this->logger->$this->logger->info('Instant check-in completed', [
                'booking_id' => $booking->id,
    ...
    public function instantCheckOut(int $bookingId, string $correlationId): array
    {
        ...
        $result = $this->db->transaction(function () use ($booking, $correlationId) {
            ...
            $booking->update([
                ...
                'checked_out_at' => CarbonImmutable::now(),
            ...
            $this->logger->$this->logger->info('Instant check-out completed', [
                'booking_id' => $booking->id,
                'final_bill' => $finalBill,
                'correlation_id' => $correlationId,
            ]);
    ...
    private function calculateDynamicPrice(
        ...
        $this->logger->$this->logger->info('Dynamic price calculated', [
            'room_type_id' => $roomTypeId,
    ...
    private function createInstantHold(
        ...
        $holdData = [
            ...
            'expires_at' => CarbonImmutable::now()->addMinutes(self::HOLD_DURATION_MINUTES)->toIso8601String(),
        ];

        $this->cache->put($holdKey, $holdData, self::HOLD_DURATION_MINUTES * 60);

        $this->logger->$this->logger->info('Instant hold created', [
            'hold_token' => $holdToken,
            'room_type_id' => $roomTypeId,
    ...
    private function calculateBookingFraudScore(array $data): float
    {
        ...
        $daysToCheckIn = $checkInDate ? Carbon::parse($checkInDate)->diffInDays(CarbonImmutable::now()) : 0;
        if ($daysToCheckIn < 1) {
            $score += 0.25;
    ...
    private function integrateCRM(Booking $booking, string $status, string $correlationId): void
    {
        ...
        $this->logger->$this->logger->info('CRM integration completed', [
            'booking_id' => $booking->id,
            'status' => $status,
            'correlation_id' => $correlationId,
        ]);
