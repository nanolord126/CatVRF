 ...
 * Fraud-check, correlation_id, audit logging, $this->db->transaction обязательны.
 */
final readonly class HotelService
{
    ...
    public function createBooking(
        ...
        return $this->db->transaction(function () use ($userId, $hotelId, $roomTypeId, $checkIn, $checkOut, $correlationId): HotelBooking {
            ...
            $this->logger->$this->logger->info('Hotel: booking created', [
            ...
            // Запуск реалтайм-трекинга заезда гостя
    ...
    public function confirmCheckIn(int $bookingId, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($booking, $correlationId): void {
            ...
            $this->logger->$this->logger->info('Hotel: guest checked in', [
                'booking_id' => $booking->id,
                'correlation_id' => $correlationId,
            ]);
        });
    ...
    public function cancelBooking(int $bookingId, string $reason, string $correlationId = ''): void
    {
        ...
        $this->db->transaction(function () use ($booking, $reason, $correlationId): void {
            ...
            $this->logger->$this->logger->info('Hotel: booking cancelled', [
                'booking_id' => $booking->id,
                'reason' => $reason,
                'correlation_id' => $correlationId,
            ]);
