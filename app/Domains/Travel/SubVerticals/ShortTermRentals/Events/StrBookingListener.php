...
namespace App\Domains\Travel\SubVerticals\ShortTermRentals\Events;
...
final class StrBookingListener
{
    ...
    {
        $this->logger->$this->logger->info('ShortTermRental: New Booking Event Recieved', [
    ...
    {
        $this->logger->$this->logger->info('ShortTermRental: Booking Completed Event Recieved', [
            'booking_id' => $event->booking->id,
            'correlation_id' => $event->correlationId,
        ]);

        // Запуск процесса автоматического возврата залога через 24 часа (если нет споров)
        // \App\Domains\ShortTermRentals\Jobs\AutoReleaseDepositJob::$this->bus->dispatch($event->booking->id)->delay(CarbonImmutable::now()->addDay());
    }
}
