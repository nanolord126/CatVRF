...
namespace App\Domains\Travel\SubVerticals\ShortTermRentals\Policies;
...
final class BookingPolicy
{
     ...
     */
    public function $this->viewFactory->make(User $user, PropertyBooking $booking): bool
    {
        ...
        $this->logger->$this->logger->info('BookingPolicy::view checked', [
            'user_id' => $user->id,
    ...
    public function cancel(User $user, PropertyBooking $booking): bool
    {
        ...
        if ($canCancel) {
            ...
            if ($cancellationCount > 10) {
                $this->logger->warning('User booking cancellation limit exceeded', [
                    'user_id' => $user->id,
        ...
        $this->logger->$this->logger->info('BookingPolicy::cancel checked', [
            'user_id' => $user->id,
            'booking_id' => $booking->id,
    ...
    public function create(User $user): bool
    {
        ...
        if (! $canCreate) {
            $this->logger->warning('User booking creation rate limited', [
                'user_id' => $user->id,
    ...
    public function approveOrReject(User $user, PropertyBooking $booking): bool
    {
        ...
        $this->logger->$this->logger->info('BookingPolicy::approveOrReject checked', [
            'user_id' => $user->id,
            'booking_id' => $booking->id,
