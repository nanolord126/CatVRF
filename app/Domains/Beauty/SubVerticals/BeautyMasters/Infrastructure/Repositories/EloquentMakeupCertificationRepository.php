...
final readonly class EloquentMakeupCertificationRepository implements MakeupCertificationRepositoryInterface
{
    public function findById(int $id): ?MakeupCertification
    {
        $model = MakeupCertificationModel::find($id);
        if ($model === null) {
            return null;
        }
    ...
    {
        return MakeupCertificationModel::where('master_id', $masterId)
            ->get()
            ->map(fn (MakeupCertificationModel $model) => $this->modelToEntity($model));
    }
    ...
    {
        $model = MakeupCertificationModel::where('master_id', $masterId)
            ->where('status', CertificationStatus::ACTIVE->value)
    ...
    public function findExpiringSoon(int $daysThreshold = 60): Collection
    {
        ...
        return MakeupCertificationModel::where('expiry_date', '<=', $threshold)
    ...
    {
        return MakeupCertificationModel::where('expiry_date', '<', now()->toDateString())
            ->where('status', CertificationStatus::ACTIVE->value)
            ->get()
            ->map(fn (MakeupCertificationModel $model) => $this->modelToEntity($model));
    }
    ...
    public function save(MakeupCertification $certification): MakeupCertification
    {
        ...
        if ($certification->id === 0) {
            $model = MakeupCertificationModel::create($data);
        } else {
            $model = MakeupCertificationModel::findOrFail($certification->id);
            $model->update($data);
        }
    ...
    {
        return MakeupCertificationModel::destroy($id) > 0;
    }

    private function modelToEntity(MakeupCertificationModel $model): MakeupCertification
    {
        return new MakeupCertification(
            id: $model->id,
