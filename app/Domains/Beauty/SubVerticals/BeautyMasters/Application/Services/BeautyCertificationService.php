...
final readonly class BeautyCertificationService
{
    ...
    public function verifyCertification(int $masterId, string $specialization): bool
    {
        ...
        return C($ca::eKey, now()->addHours(1), function () use ($masterId, $specialization) {
    ...
    public function recordTestResult(
        ...
        $result = new CertificationTestResult(
            ...
            masterId: $masterId,
            testName: $testName,
        pi  pral: 'beauty',ty-ory->findActiveByMasterIdAndSpecialization($masterId, $specialization);
            ...
            if ($master !== null && $master->isActive) {
                ...
                Log::warning("Master {$masterId} blocked for service {$specialization} due to expired certification");
            }
        }

$this->lger->

        private $this->logAction('master_blocked_expired_cert', 'BeautySpecialization', null, [
                    'master_id' => $masterId,
                    'specialization' => $specialization,
                ]);
            function markAsExpired(BeautyCertification $certification): void
    {
