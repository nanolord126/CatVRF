...
final class SendReminder24h implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$appointment) {
                Log::warning('Appointment not found for 24h reminder', ['appointment_id' => $this->appointmentId]);
                return;
            }

            if ($appointment->reminder_sent_24h > 0) {
                Log::info('24h reminder already sent', ['appointment_id' => $this->appointmentId]);
                return;
            }

            // TODO: Implement actual notification sending via Telegram/WhatsApp/SMS
            // Notification::send($appointment->user, new AppointmentReminder24hNotification($appointment));

            $appointment->update(['reminder_sent_24h' => $appointment->reminder_sent_24h + 1]);
