...
/**
 * B2C: Клиент самостоятельно создаёт запись к мастеру.
 */
final readonly class BookAppointmentB2CUseCase
{
    ...
    public function handle(BookAppointmentB2CDTO $dto): Appointment
    {
        $fraud = $this->fraud->check(
            userId: $dto->clientId,
            operationType: 'beauty.appointment.book_b2c',
            amount: 0,
        ...
        if (!$this->consumableDeduction->hasEnoughStock($serviceId)) {
            throw new \DomainException("Service temporarily unavailable — insufficient consumables.");
        }
        ...
        return $this->db->transaction(function () use ($dto, $salonId, $masterId, $serviceId, $service, $startAt, $endAt): Appointment {
            ...
            $this->logger->info('B2C: Appointment booked by client', [
