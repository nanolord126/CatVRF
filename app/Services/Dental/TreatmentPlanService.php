...
final readonly class TreatmentPlanService
{
    ...
    public function createPlan(array $data): DentalTreatmentPlan
    {
        return $this->db->transaction(function () use ($data) {
            // 1. Audit Check
            $this->logger->channel('audit')->$this->logger->info('Creating dental treatment plan', [
    ...
    public function updatePlan(int $id, array $data): DentalTreatmentPlan
    {
        return $this->db->transaction(function () use ($id, $data) {
            ...
            // Audit
            $this->logger->channel('audit')->$this->logger->info('Updating dental treatment plan', [
                'plan_id' => $id,
    ...
    public function addStep(int $planId, array $stepData): void
    {
        $this->db->transaction(function () use ($planId, $stepData) {
            ...
            // Log
            $this->logger->channel('audit')->$this->logger->info('Adding step to treatment plan', [
                'plan_id' => $planId,
        ...
        $this->db->transaction(function () use ($planId, $stepUuid) {
            ...
            foreach ($steps as &$step) {
                if ($step['id'] === $stepUuid) {
                    ...
                    $step['completed_at'] = CarbonImmutable::now()->toIso8601String();
                    $found = true;
                    break;
                }
            }
            ...
            $this->logger->channel('audit')->$this->logger->info('Treatment plan step marked completed', [
                'plan_id' => $planId,
                'step_uuid' => $stepUuid,
                'correlation_id' => $this->correlationId(),
            ]);
