...
final readonly class AIDentalConstructor
{
    ...
    {
        return $this->db->transaction(function () use ($photoPath, $budget) {
            // 1. Audit Check
            $this->logger->channel('audit')->$this->logger->info('AI Dental Smile Analysis started', [
                'photo' => $photoPath,
                'budget' => $budget,
    ...
    public function generatePreview(string $originalPhotoPath, array $treatmentServices): string
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('AI Dental Preview generated', [
    ...
    {
        return $this->db->transaction(function () use ($planId, $newBudget) {
            ...
            if (empty($analysis)) {
                throw new \RuntimeException('No AI analysis snapshot found for refinement');
            }

            $suggestedServices = $this->matchServicesWithAnalysis($analysis, $newBudget);

            $this->logger->channel('audit')->$this->logger->info('Refining Dental Treatment Plan with AI', [
                'plan_id' => $planId,
    ...
    private function matchServicesWithAnalysis(array $analysis, int $budget): Collection
    {
        ...
        if (in_array('Crowding', $analysis['detected_issues'], true)) {
        ...
        if (in_array('Discoloration', $analysis['detected_issues'], true) || $analysis['tooth_color'] > 'A2') {
            $whitening = DentalModel::where('category', 'Therapy')->where('name', 'like', '%Whitening%')->first();
            if ($whitening) {
                $services->push($whitening);
            }
        }

        // 3. Caries -> Therapy
        if (in_array('Possible Caries', $analysis['detected_issues'], true)) {
            $caries = DentalModel::where('category', 'Therapy')->where('name', 'like', '%Filling%')->first();
            if ($caries) {
                $services->push($caries);
            }
        }
