...
final readonly class FeatureDriftMetricsService
{
    ...
    public function exportPrometheusMetrics(): string
    {
        ...
        $latestReport = $this->cache->get(self::CACHE_KEY_PREFIX.'latest_report');
        ...
        $metrics[] = sprintf(
            'fraud_ml_feature_drift_psi_score{vertical_code="%s"} %f',
        ...
        $metrics[] = sprintf(
            'fraud_ml_feature_drift_ks_score{vertical_code="%s"} %f',
            $latestReport['vertical_code'] ?? 'unknown',
        ...
        $metrics[] = sprintf(
            'fra_ml_drifted_features_count{vertical_code="%s"} %d',
            $latestReport['vertical_code'] ?? 'unknown',
        ...
        $metrics[] = sprintf(
            'fra_ml_moderate_drift_features_count{vertical_code="%s"} %d',
            $latestReport['vertical_code'] ?? 'unknown',
        ...
        $metrics[] = sprintf(
            'fra_ml_overall_drift_detected{vertical_code="%s"} %d',
            $latestReport['vertical_code'] ?? 'unknown',
        ...
        $metrics[] = sprintf(
            'fra_ml_drift_check_timestamp{vertical_code="%s"} %d',
            $latestReport['vertical_code'] ?? 'unknown',
    ...
    {
        $this->cache->put(self::CACHE_KEY_PREFIX.'latest_report', $driftReport, self::CACHE_TTL);
    ...
    public function recordFeatureDrift(string $featureName, string $metricType, float $score, string $verticalCode = 'default'): void
    {
        ...
        $this->cache->put($key, $history, self::CACHE_TTL);
    }
