...
{
    protected readonly ?FeatureDriftDetectorService $driftDetector = null;

    protected readonly ?PrometheusMetricsService $prometheus = null;

    protected readonly string $verticalCode = 'default';

    protected readonly array $monitoredFeatures = [];

    protected readonly ?CacheManager $driftCache = null;

    protected readonly ?Repository $driftConfig = null;

    protected readonly ?LogManager $driftLog = null;

    protected readonly ?Dispatcher $driftDispatcher = null;
    ...
    public function setDriftDetectionEnabled(bool $enabled): void
    {
        ...
        $this->driftLog->$this->logger->info('Drift detection toggled', [
    ...
    {
        $this->driftDetectionEnabled = $this->driftConfig->get('feature_drift.enabled', true);

        // Load vertical-specific monitored features from config
        $config = $this->driftConfig->get("feature_drift.verticals.{$this->verticalCode}", []);
        ...
        if (empty($this->monitoredFeatures)) {
            ...
            $this->monitoredFeatures = $this->driftConfig->get('feature_drift.default_features', [
                'frequency',
                'value_avg',
                'duration_avg',
            ]);
        ...
        $this->driftLog->debug('Drift detection initialized', [
            'vertical' => $this->verticalCode,
    ...
    protected function checkAllFeaturesDrift(array $featuresData): array
    {
        ...
        try {
            ...
            if ($result['summary']['overall_drift_detected'] ?? false) {
                $this->driftDispatcher?->dispatch(new SignificantFeatureDriftDetected($result, $correlationId));

                $this->driftLog->warning('Significant feature drift detected', [
                    'vertical' => $this->verticalCode,
            ...
            $this->driftLog->error('Feature drift detection failed', [
                'vertical' => $this->verticalCode,
                'error' => $e->getMessage(),
                'correlation_id' => $correlationId,
            ]);
    ...
    protected function checkFeatureDrift(string $featureName, array $expected, array $actual): array
    {
        ...
        try {
            ...
            if ($result['combined']['drift_detected'] ?? false) {
                $this->driftDispatcher?->dispatch(new SignificantFeatureDriftDetected($result, Uuid::uuid4()->toString()));
            }

            return $result;

        } catch (\Throwable $e) {
            $this->driftLog->error('Single feature drift detection failed', [
                'vertical' => $this->verticalCode,
                'feature' => $featureName,
                'error' => $e->getMessage(),
            ]);
    ...
    protected function invalidateReferenceCache(): void
    {
        ...
        $this->driftLog->$this->logger->info('Reference cache invalidated', [
            'vertical' => $this->verticalCode,
        ]);
    }
    ...
    {
        $config = $this->driftConfig->get("feature_drift.verticals.{$this->verticalCode}", []);
        $defaultThresholds = $this->driftConfig->get('feature_drift.default_thresholds', [
            'psi_critical' => 0.25,
            'psi_moderate' => 0.1,
            'ks_alpha' => 0.05,
            'js_critical' => 0.3,
