...
final readonly class UserTasteProfileService
{
    ...
    public function getOrCreateProfile(int $userId, int $tenantId, ?string $correlationId = null): UserTasteProfile
    {
        try {
            ...
            if (! $profile) {
                ...
                $this->logger->channel('audit')->$this->logger->info('User taste profile created', [
                    'user_id' => $userId,
                    'tenant_id' => $tenantId,
    ...
    public function recordInteraction(
        ...
        try {
            ...
            $metadata = $profile->metadata ?? [];
            $metadata['last_interaction_at'] = CarbonImmutable::now()->toIso8601String();
            $metadata['total_interactions'] = ($metadata['total_interactions'] ?? 0) + 1;

            // Добавить в history
            $history = $profile->history ?? [];
            $todayKey = CarbonImmutable::now()->toDateString();
            ...
            $profile->update([
                'metadata' => $metadata,
                'history' => $history,
                'last_interaction_at' => CarbonImmutable::now(),
            ]);

            $this->logger->channel('audit')->$this->logger->info('User taste interaction recorded', [
                'user_id' => $userId,
    ...
    public function setPersonalizationEnabled(int $userId, int $tenantId, bool $enabled): UserTasteProfile
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('Personalization toggled', [
            'user_id' => $userId,
            'enabled' => $enabled,
        ]);
