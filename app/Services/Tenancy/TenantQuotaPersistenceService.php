...
final readonly class TenantQuotaPersistenceService
{
    ...
    public function flushToDatabase(): int
    {
        ...
        $this->logger->$this->logger->info('Quota usage flushed to database', [
    ...
    {
        return $this->db->table('tenant_quota_usage')
            ->where('tenant_id', $tenantId)
            ->where('resource_type', $resourceType)
            ->where('recorded_at', '>=', CarbonImmutable::now()->subDays($days))
            ->orderBy('recorded_at')
            ->get()
            ->toArray();
    }
    ...
    public function getAggregateUsage(string $resourceType, string $period = 'daily'): array
    {
        $groupBy = match($period) {
            'hourly' => $this->db->raw('DATE_FORMAT(recorded_at, "%Y-%m-%d %H:00:00")'),
            'daily' => $this->db->raw('DATE(recorded_at)'),
            'monthly' => $this->db->raw('DATE_FORMAT(recorded_at, "%Y-%m")'),
            default => $this->db->raw('DATE(recorded_at)'),
        };
        ...
        return $this->db->table('tenant_quota_usage')
            ->select([
                'tenant_id',
                $groupBy.' as period',
                $this->db->raw('SUM(usage) as total_usage'),
                $this->db->raw('COUNT(*) as record_count'),
            ])
            ->where('resource_type', $resourceType)
            ->where('recorded_at', '>=', CarbonImmutable::now()->subDays(30))
    ...
    public function ensureTableExists(): void
    {
        ...
        if (! $schema->hasTable('tenant_quota_usage')) {
            ...
            $this->logger->$this->logger->info('tenant_quota_usage table created');
        }
    }
    ...
    private function flushKeyToDatabase(string $key): int
    {
        ...
        try {
            $this->db->table('tenant_quota_usage')->insert([
