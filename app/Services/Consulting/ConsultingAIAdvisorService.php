...
final readonly class ConsultingAIAdvisorService
{
    ...
    public function generateStrategy(int $clientId, array $goals): array
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('AI Business Strategy Generation', [
            'client_id' => $clientId,
            'goals' => $goals,
    ...
    public function analyzeBusinessGap(int $firmId, array $metrics): array
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('AI Gap Analysis', [
            'firm_id' => $firmId,
    ...
    {
        $this->logger->channel('audit')->$this->logger->info('AI Budget Forecast Predictor', [
            'client_id' => $clientId,
            'horizon' => $daysAhead,
            'correlation_id' => $this->correlationId(),
        ]);
    ...
    {
        $this->logger->channel('audit')->$this->logger->info('AI Intervention Recorded', [
            'type' => $type,
            'payload' => $payload,
            'correlation_id' => $this->correlationId(),
        ]);
