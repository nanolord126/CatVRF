...
final readonly class ConsultingRatingService
{
    ...
    public function submitSessionReview(int $sessionId, int $rating, string $comment, int $userId): ConsultingReview
    {
        ...
        return $this->db->transaction(function () use ($sessionId, $rating, $comment, $userId) {
            ...
            if ($session->client_id !== $userId) {
                throw new AccessDeniedHttpException('User did not participate in this session.');
            }
            ...
            $this->logger->channel('audit')->$this->logger->info('Submitting Consulting Review', [
    ...
    {
        $this->logger->channel('audit')->$this->logger->info('Recalculating Consultant Rating', [
    ...
    {
        $this->logger->channel('audit')->$this->logger->info('Recalculating Firm Rating', [
            'firm_id' => $firmId,
            'correlation_id' => $this->correlationId(),
        ]);
