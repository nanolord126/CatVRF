...
final readonly class MeshService
{
    ...
    public function createRoom(Event $stream): string
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('WebRTC room created', [
            'stream_id' => $stream->id,
    ...
    public function joinRoom(
        ...
        $this->logger->channel('audit')->$this->logger->info('Peer joined stream', [
            'peer_id' => $peerId,
            'stream_id' => $stream->id,
            'user_id' => $user->id,
            'tenant_id' => $stream->tenant_id,
    ...
    public function markConnected(string $peerId): void
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('Peer connection established', [
            'peer_id' => $peerId,
        ]);
    }
    ...
    public function cleanupClosedConnections(int $olderThanMinutes = 60): int
    {
        return $this->db->transaction(function () use ($olderThanMinutes): int {
            $cutoffTime = CarbonImmutable::now()->subMinutes($olderThanMinutes);
            ...
            if ($deleted > 0) {
                $this->logger->channel('audit')->$this->logger->info('Cleaned up closed peer connections', [
                    'count' => $deleted,
