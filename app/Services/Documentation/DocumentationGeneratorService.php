...
final readonly class DocumentationGeneratorService
{
    ...
    public static function generateDeploymentGuide(): array
    {
        return [
            ...
            'version' => '3.0.0',
            'last_updated' => CarbonImmutable::now()->toDateString(),
            'sections' => [
                [
                    'title' => 'Prerequisites',
                    'content' => [
    ...
    {
        $path = base_path('storage' . DIRECTORY_SEPARATOR . 'docs');
        ...
        $this->logger->channel('documentation')->$this->logger->info('Documentation saved', [
            'type' => $type,
            'format' => $format,
            'path' => $filepath,
        ]);
