...
final readonly class RealtimeChatService
{
    ...
    public function deleteMessage(
        ...
        try {
            ...
            $message['deleted_at'] = CarbonImmutable::now()->toIso8601String();

            $this->cache->put($messageKey, $message, self::CHAT_TTL);

            $this->logger->channel('audit')->$this->logger->info('Chat message deleted', [
                'correlation_id' => $correlationId,
                'message_id' => $messageId,
                'room_id' => $roomId,
            ]);
    ...
    public function editMessage(
        ...
        try {
            ...
            $message['edited_at'] = CarbonImmutable::now()->toIso8601String();

            $this->cache->put($messageKey, $message, self::CHAT_TTL);

            $this->logger->channel('audit')->$this->logger->info('Chat message edited', [
                'correlation_id' => $correlationId,
                'message_id' => $messageId,
            ]);
    ...
    public function addMember(
        ...
        try {
            ...
            if (! in_array($userId, $room['members'], true)) {
                $room['members'][] = $userId;
                $this->cache->put($roomKey, $room, self::ACTIVE_ROOMS_TTL);
            }
