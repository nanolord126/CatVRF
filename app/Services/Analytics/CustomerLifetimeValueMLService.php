...
/**
 * Customer Lifetime Value ML Service
 * Вычисление прогноза LTV и прогнозирование оттока (churn)
 * 
 * @package App\Services\Analytics
 * @category ML / Customer Analytics
 */
final class CustomerLifetimeValueMLService
{
    ...
    public function predictChurnProbability(int $userId): array
    {
        try {
            ...
            $this->log->channel('analytics_errors')->error('Churn prediction failed', [
                'user_id' => $userId,
                'error' => $e->getMessage()
            ]);
            return [
