...
final readonly class PriceSuggestionMLService
{
    ...
    public function getSuggestedPrice(int $productId, int $tenantId): array
    {
        ...
        return $this->cache->remember($cacheKey, self::CACHE_TTL, function () use ($productId) {
            try {
    ...
    {
        $currentMonth = (int) CarbonImmutable::now()->format('m');
        $currentDayOfWeek = (int) CarbonImmutable::now()->format('w');
