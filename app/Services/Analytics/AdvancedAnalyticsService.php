...
final readonly class AdvancedAnalyticsService
{
    ...
    public function getMetricsByPeriod(
        ...
        if ($cached !== null) {
            $this->logger->channel('analytics')->$this->logger->info('Metrics retrieved from cache', [
        ...
        switch ($metricType) {
            ...
            case 'aov': // Average Order Value
                ...
                break;
            default:
                $this->logger->channel('analytics')->warning('Unknown metric type', [
                    'correlation_id' => $correlationId,
                    'metric_type' => $metricType,
                ]);
        ...
        $this->logger->channel('analytics')->$this->logger->info('Metrics calculated', [
            'correlation_id' => $correlationId,
            'tenant_id' => $tenantId,
            'metric_type' => $metricType,
    ...
    public function predictFutureTrend(
        ...
        $startDate = CarbonImmutable::now()->subDays(self::SAMPLE_DATA_POINTS)->format('Y-m-d');
        ...
        $this->logger->channel('analytics')->$this->logger->info('Forecast generated', [
            'correlation_id' => $correlationId,
            'tenant_id' => $tenantId,
            'metric_type' => $metricType,
    ...
    public function generateCustomReport(
        ...
        $this->logger->channel('analytics')->$this->logger->info('Custom report generated', [
            'correlation_id' => $correlationId,
            'tenant_id' => $tenantId,
    ...
    public function calculateKPIs(int $tenantId, array $context = []): array
    {
        ...
        $this->logger->channel('analytics')->$this->logger->info('KPIs calculated', [
            'correlation_id' => $correlationId,
            'tenant_id' => $tenantId,
        ]);
    ...
    public function getComparativeAnalysis(
        ...
        $this->logger->channel('analytics')->$this->logger->info('Comparative analysis completed', [
            'correlation_id' => $correlationId,
            'tenant_id' => $tenantId,
        ]);
    ...
    public function exportDataToCSV(array $data): string
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $csv .= "\"{$key}\",\"{$value}\",".CarbonImmutable::now()->toIso8601String()."\n";
        }
    ...
    {
        return match ($period) {
            '7_days_ago' => ['start' => CarbonImmutable::now()->subDays(7)->format('Y-m-d'), 'end' => CarbonImmutable::now()->format('Y-m-d')],
            '30_days_ago' => ['start' => CarbonImmutable::now()->subDays(30)->format('Y-m-d'), 'end' => CarbonImmutable::now()->format('Y-m-d')],
            'current' => ['start' => CarbonImmutable::now()->format('Y-m-d'), 'end' => CarbonImmutable::now()->format('Y-m-d')],
            default => ['start' => CarbonImmutable::now()->subDays(30)->format('Y-m-d'), 'end' => CarbonImmutable::now()->format('Y-m-d')],
        };
    }
    ...
    private function getChurnRate(int $tenantId, int $days): float
    {
        $prevPeriodUsers = $this->db->table('payment_transactions')
            ->where('tenant_id', $tenantId)
            ->where('created_at', '<', CarbonImmutable::now()->subDays($days))
            ->where('created_at', '>=', CarbonImmutable::now()->subDays($days * 2))
            ->distinct('client_id')
            ->count('client_id');
    ...
    {
        $days = max(1, (int) Carbon::parse($start)->diffInDays(Carbon::parse($end)));
    ...
    private function getPeakRevenueDay(int $tenantId, string $start, string $end): string
    {
        ...
        return $row?->day ?? CarbonImmutable::now()->format('Y-m-d');
    }
