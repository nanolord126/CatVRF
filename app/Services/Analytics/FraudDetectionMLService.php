...
final readonly class FraudDetectionMLService
{
    ...
    public function scorePaymentAttempt(
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Payment fraud score calculated', [
                'user_id' => $userId,
    ...
    private function computeAnomalyScore(array $features): float
    {
        ...
        if ($features['payments_1_min'] > 0) {
            $score += 0.15;
        }
        if ($features['payments_5_min'] > 3) {
            $score += 0.20;
        }
        if ($features['payments_1_hour'] > 10) {
            $score += 0.15;
        }

        // Необычная сумма
        if ($features['amount_vs_avg_30d'] > 5.0) {
            $score += 0.20;
        }
        ...
        if ($features['device_changes_7d'] > 5) {
            $score += 0.15;
        }
        if ($features['ip_changes_7d'] > 10) {
            $score += 0.15;
        }
        ...
        if ($features['refund_count_30d'] > 3) {
            $score += 0.20;
        }
        if ($features['decline_rate_30d'] > 0.30) {
            $score += 0.20;
        }

        // Первый платёж с большой суммой
        if ($features['is_first_payment'] === 1 && $features['amount'] > 10000) {
            $score += 0.20;
        }

        // Молодой аккаунт с большой суммой
        if ($features['account_age_days'] < 7 && $features['amount'] > 5000) {
            $score += 0.15;
        }
    ...
    private function getUserPaymentHistory(int $userId): array
    {
        return $this->cache->remember("user_payment_history:{$userId}", self::CACHE_TTL, function () use ($userId) {
            ...
            return [
                ...
                'account_age_days' => CarbonImmutable::now()->diffInDays($this->getUserCreatedAt($userId)),
    ...
    private function getUserCreatedAt(int $userId): Carbon
    {
        ...
        return $createdAt !== null ? Carbon::parse((string) $createdAt) : CarbonImmutable::now();
    }
