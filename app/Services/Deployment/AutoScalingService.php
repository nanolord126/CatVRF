...
final readonly class AutoScalingService
{
    ...
    {
        $this->logger->channel('deployment')->$this->logger->info('Scaling application', [
        ...
        return [
            ...
            'estimated_completion' => CarbonImmutable::now()->addMinutes(2)->toDateTimeString(),
        ];
    }
    ...
    public static function configureAutoScaling(bool $enabled, array $config = []): array
    {
        ...
        $this->logger->channel('deployment')->$this->logger->info('Auto scaling configured', [
            'enabled' => $enabled,
            'config' => $finalConfig,
            'correlation_id' => $this->request->header('X-Correlation-ID', $this->correlationId ?? ''),
        ]);
        ...
        return [
            ...
            'configured_at' => CarbonImmutable::now()->toDateTimeString(),
        ];
    }
    ...
    public static function getScalingHistory(int $limit = 50): array
    {
        return [
            'scaling_events' => [
                [
                    'timestamp' => CarbonImmutable::now()->subHours(2)->toDateTimeString(),
                    'action' => 'scale_up',
                    'from_instances' => 2,
                    'to_instances' => 3,
                ...
                [
                    'timestamp' => CarbonImmutable::now()->subHours(1)->toDateTimeString(),
                    'action' => 'scale_down',
                    'from_instances' => 3,
                    'to_instances' => 2,
