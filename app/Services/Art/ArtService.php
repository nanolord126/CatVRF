...
final readonly class ArtService
{
    ...
    public function registerArtwork(array $data): Artwork
    {
        ...
        return $this->db->transaction(function () use ($data) {
            ...
            $this->logger->channel('audit')->$this->logger->info('Artwork registered', [
                'id' => $artwork->id,
                'uuid' => $artwork->uuid,
                'correlation_id' => $this->correlationId(),
            ]);
    ...
    public function purchaseArtwork(int $userId, int $artworkId, string $type = 'b2c'): ArtOrder
    {
        ...
        return $this->db->transaction(function () use ($userId, $artwork, $type) {
            ...
            $this->wallet->debit($userId, $totalCents, BalanceTransactionType::WITHDRAWAL, $this->correlationId(), null, null, null);

            $this->wallet->credit($artwork->gallery_id ?? $artwork->user_id, $totalCents - $commissionCents, BalanceTransactionType::PAYOUT, $this->correlationId(), null, null, ['artwork_id' => $artwork->id]);
