...
final readonly class CourierService
{
    ...
    public function registerCourier(
        ...
        try {
            ...
            if (! $this->rateLimiter->allowTenant($tenantId, 'courier:register', 50, 3600)) {
                throw new TooManyRequestsHttpException(message: 'Rate limit exceeded for courier registration');
            }

            $this->logger->channel('audit')->$this->logger->info('Courier registration started', [
                'tenant_id' => $tenantId,
                'name' => $data['name'] ?? '',
            ...
            $result = $this->db->transaction(function () use ($tenantId, $data, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Courier registered', [
                    'tenant_id' => $tenantId,
    ...
    public function getCurrentLocation(
        ...
        try {
            ...
            return [
                ...
                'last_updated' => $courier->last_location_at?->toIso8601String() ?? CarbonImmutable::now()->toIso8601String(),
            ];
        } catch (Throwable $e) {
    ...
    public function assignDelivery(
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Delivery assignment started', [
                'courier_id' => $courierId,
                'delivery_order_id' => $deliveryOrderId,
                'correlation_id' => $correlationId,
            ]);
            ...
            $result = $this->db->transaction(function () use ($courierId, $deliveryOrderId, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Delivery assigned', [
                    'delivery_order_id' => $deliveryOrderId,
                    'courier_id' => $courierId,
                    'correlation_id' => $correlationId,
                ]);
                ...
                return [
                    ...
                    'status' => 'assigned',
                    'estimated_time' => CarbonImmutable::now()->addMinutes(25)->toIso8601String(),
                ];
            });
    ...
    public function completeDelivery(
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Delivery completion started', [
                'courier_id' => $courierId,
                'delivery_order_id' => $deliveryOrderId,
                'rating' => $ratingScore,
                'correlation_id' => $correlationId,
            ...
            $this->db->transaction(function () use ($courierId, $deliveryOrderId, $ratingScore, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Delivery completed', [
                    'delivery_order_id' => $deliveryOrderId,
                    'courier_id' => $courierId,
