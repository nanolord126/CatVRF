...
final readonly class AutomatedTestingService
{
    ...
    public static function runTests(string $suite = 'all'): array
    {
        ...
        $results['completed_at'] = CarbonImmutable::now()->toDateTimeString();
        $results['duration_seconds'] = round($duration, 2);

        $results['summary'] = self::calculateTestSummary($results);

        $this->logger->channel('testing')->$this->logger->info('Test suite completed', [
            'suite' => $suite,
    ...
    public static function getTestHistory(int $limit = 20): array
    {
        return [
            'runs' => [
                [
                    'id' => 'run_001',
                    'timestamp' => CarbonImmutable::now()->subMinutes(30)->toDateTimeString(),
                    'status' => 'passed',
                    'total_tests' => 391,
                    'passed' => 390,
                    'failed' => 1,
    ...
    {
        $this->logger->channel('testing')->$this->logger->info('Running test file', [
            'file' => $testFile,
