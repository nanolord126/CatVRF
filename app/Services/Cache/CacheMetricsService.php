...
final readonly class CacheMetricsService
{
    ...
    public function recordCacheInvalidation(array $tags, string $reason): void
    {
        ...
        $this->logger->$this->logger->info('Cache invalidation recorded', [
            'tags' => $tags,
            'reason' => $reason,
        ]);
    }
    ...
    private function extractVerticalFromTags(array $tags): string
    {
        ...
        foreach ($tags as $tag) {
            if (in_array($tag, ['medical', 'beauty', 'food', 'fashion', 'travel', 'auto', 'hotels', 'fitness', 'sports', 'luxury', 'insurance', 'legal', 'logistics', 'education', 'delivery', 'payment', 'analytics', 'consulting', 'content', 'freelance', 'event_planning', 'staff', 'inventory', 'taxi', 'tickets', 'wallet', 'pet', 'wedding_planning', 'veterinary', 'toys_and_games', 'advertising', 'car_rental', 'finances', 'flowers', 'furniture', 'pharmacy', 'photography', 'short_term_rentals', 'sports_nutrition', 'personal_development', 'home_services', 'gardening', 'geo', 'geo_logistics', 'grocery_and_delivery', 'farm_direct', 'meat_shops', 'office_catering', 'party_supplies', 'confectionery', 'construction_and_repair', 'cleaning_services', 'communication', 'books_and_literature', 'collectibles', 'hobby_and_craft', 'household_goods', 'marketplace', 'music_and_instruments', 'vegan_products', 'art'], true)) {
                return $tag;
            }
        }
