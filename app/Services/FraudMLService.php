...
final readonly class FraudMLService
{
    ...
    public function scoreRepairOrder(AutoRepairOrder $order, string $correlationId): float
    {
        ...
        $previousRepairs = AutoRepairOrder::where('vehicle_id', $order->vehicle_id)
            ->where('created_at', '>', CarbonImmutable::now()->subDays(3))
            ->count();
        if ($previousRepairs > 1) {
            $score += 0.4;
        }

        $this->logger->channel('fraud_alert')->$this->logger->info('Repair Order Fraud Score', [
    ...
    public function scoreTaxiRide(TaxiRide $ride, string $correlationId): float
    {
        ...
        $this->logger->channel('fraud_alert')->$this->logger->info('Taxi Ride Fraud Score', [
            'ride_uuid' => $ride->uuid,
            'score' => $score,
            'correlation_id' => $correlationId,
        ]);
