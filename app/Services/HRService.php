    ...
    public function createEmployee(
        ...
        try {
            ...
            if (! $this->rateLimiter->allowTenant($tenantId, 'employee:create', 100, 60)) {
                throw new TooManyRequestsHttpException(message: 'Rate limit exceeded for employee creation');
            }

            $this->logger->channel('audit')->$this->logger->info('Employee creation started', [
                'tenant_id' => $tenantId,
                'name' => $data['name'] ?? '',
            ...
            $result = $this->db->transaction(function () use ($tenantId, $data, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Employee created successfully', [
                    'tenant_id' => $tenantId,
    ...
    public function calculateAndPaySalaries(
        ...
        try {
            ...
            if (! $this->rateLimiter->allowTenant($tenantId, 'salary:calculate', 10, 3600)) {
                throw new TooManyRequestsHttpException(message: 'Rate limit exceeded for salary calculation');
            }

            $this->logger->channel('audit')->$this->logger->info('Salary calculation started', [
                'tenant_id' => $tenantId,
                'year' => $year,
                'month' => $month,
                'correlation_id' => $correlationId,
            ...
            $result = $this->db->transaction(function () use ($tenantId, $correlationId) {
                ...
                $this->logger->channel('audit')->$this->logger->info('Salary processed', [
                    'tenant_id' => $tenantId,
