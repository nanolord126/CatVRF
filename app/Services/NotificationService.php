...
final readonly class NotificationService
{
    ...
    public function send(
        ...
        try {
            ...
            if ($this->shouldSendEmail($type)) {
                try {
                    // $this->mailer->to($user->email)->queue(new NotificationMailable($notification));
                } catch (Throwable $e) {
            ...
            $this->logger->channel('audit')->$this->logger->info('Notification queued successfully', [
    ...
    public function sendDailyReport(int $tenantId, string $correlationId = ''): bool
    {
        ...
        try {
            $this->logger->channel('audit')->$this->logger->info('Daily report sending', [
                'tenant_id' => $tenantId,
                'correlation_id' => $correlationId,
            ]);
            ...
            $this->logger->channel('audit')->$this->logger->info('Daily report sent successfully', [
                'tenant_id' => $tenantId,
                'correlation_id' => $correlationId,
            ]);
    ...
    public function sendWeeklyReport(int $tenantId, string $correlationId = ''): bool
    {
        ...
        try {
            $this->logger->channel('audit')->$this->logger->info('Weekly report sending', [
                'tenant_id' => $tenantId,
                'correlation_id' => $correlationId,
            ]);
            ...
            $this->logger->channel('audit')->$this->logger->info('Weekly report sent successfully', [
                'tenant_id' => $tenantId,
                'correlation_id' => $correlationId,
            ]);
    ...
    {
        return in_array($type, ['payment_confirmed', 'order_shipped', 'daily_report', 'weekly_report'], true);
    }
    ...
    {
        return in_array($type, ['payment_confirmed', 'order_shipped'], true);
    }
    ...
    {
        return in_array($type, ['payment_confirmed', 'order_shipped', 'low_stock'], true);
    }
