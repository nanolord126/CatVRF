...
final readonly class CollaborationService
{
    ...
    public function startEditingSession(
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Collaboration session started', [
                'correlation_id' => $correlationId,
                'session_id' => $sessionId,
                'user_id' => $userId,
                'tenant_id' => $tenantId,
    ...
    public function endEditingSession(
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Collaboration session ended', [
                'correlation_id' => $correlationId,
                'session_id' => $sessionId,
                'tenant_id' => $tenantId,
                'document' => "{$documentType}:{$documentId}",
    ...
    public function updateCursorPosition(
        ...
        try {
            ...
            $sessionData['last_heartbeat'] = CarbonImmutable::now()->toIso8601String();
    ...
    public function cleanupStaleSession(int $tenantId): void
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('Cleanup stale collaboration sessions', [
            'tenant_id' => $tenantId,
        ]);
    }
}
