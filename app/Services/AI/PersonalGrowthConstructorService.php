...
final readonly class PersonalGrowthConstructorService
{
     ...
     * @throws Throwable
     */
    public function generateGrowthPlan(User $user, array $input): array
    {
        $correlationId = $this->request->header('X-Correlation-ID') ?? Str::uuid()->toString();

        $this->logger->channel('audit')->$this->logger->info('AI Growth Constructor: Starting plan generation', [
            'user_id' => $user->id,
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('AI Growth Constructor: Plan successfully generated', [
                'user_id' => $user->id,
     ...
     */
    private function analyzeGoalsWithAI(User $user, array $input, array $profile): array
    {
     ...
     */
    private function saveAIConstruction(User $user, array $input, array $plan, string $correlationId): void
    {
