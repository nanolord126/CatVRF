...
/**
 * AIConstructorService — главный сервис AI-конструкторов.
 * Управление созданием AI-конструкций (beauty, interior, fashion, food).
 */
final readonly class AIConstructorService
{
    ...
    public function construct(
        ...
        $this->fraud->check(
            userId: $user->id,
            operationType: "ai_constructor_{$type}",
            amount: 0,
        ...
        // 3. Сохранить фото
        $photoPath = $photo->store('ai-constructions', 'public');

        // 4. Извлечь предпочтения
        [$explicitPrefs, $implicitPrefs] = $this->extractUsedPreferences($tasteProfile, $type, [], $params);

        // 5. Моковый анализ (в реальности - вызов LLM/Vision API)
        $analysis = [
            'colors' => $params['detected_colors'] ?? [],
            'styles' => $params['detected_styles'] ?? [],
            'elements' => $params['detected_elements'] ?? [],
            'confidence' => 0.85,
        ];

        // 6. Запустить конкретный конструктор
        $constructorResult = $this->runConstructor($type, $analysis, $explicitPrefs, $implicitPrefs, $params);

        // 7. Сохранить результаты в БД (в транзакции)
        return $this->db->transaction(function () use ($user, $type, $photoPath, $analysis, $constructorResult, $tasteProfile, $explicitPrefs, $implicitPrefs, $correlationId) {
            try {
                $this->logger->channel('audit')->$this->logger->info("AI Constructor started: {$type}", [
                ...
                $construction = AIConstruction::create([
                    ...
                    'user_id' => $user->id,
                    'tenant_id' => $user->current_tenant_id ?? null,
                    'type' => $type,
                    'correlation_id' => $correlationId,
                    'input_data' => [
                        'params' => $params,
                    ...
                    'taste_profile_used' => $tasteProfile->toArray(),
                ...
                $this->logger->channel('audit')->$this->logger->info('AI Constructor completed successfully', [
                    'correlation_id' => $correlationId,
                    'user_id' => $user->id,
    ...
    public function deleteConstruction(AIConstruction $construction): void
    {
        ...
        $this->logger->channel('audit')->$this->logger->info('AI Construction deleted', [
    ...
    private function runConstructor(
        ...
        // В реальности здесь вызываются конкретные конструкторы
        // Для MVP возвращаем моковый результат
        return [
            'data' => [
                'type' => $type,
                'style' => $explicit['style'] ?? 'modern',
