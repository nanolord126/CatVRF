...
final readonly class RateLimiterService
{
    ...
    private const SEARCH_LIGHT_LIMIT = 1000;    // Light search requests per hour
    private const SEARCH_HEAVY_LIMIT = 100;     // Heavy search requests per hour
    private const HOUR_SECONDS = 3600;          // 1 hour in seconds
    private const REFERRAL_LIMIT = 5;           // Referral claim attempts per hour
    private const WEBHOOK_RETRY_LIMIT = 100;    // Webhook retry attempts per hour
    ...
    {
        return $this->redis->exists("burst_ban:{$tenantId}:{$userId}") === 1
            || $this->redis->exists("rate_limit:payment_init:{$tenantId}:{$userId}:burst_ban") === 1;
    }
    ...
    private function checkSlidingWindow(
        ...
        // Проверить burst ban
        if ($this->redis->exists("{$key}:burst_ban") === 1) {
    ...
    private function handleRateLimitExceeded(
        ...
        $rejectionCount = $this->redis->incr($rejectionKey);
        $this->redis->expire($rejectionKey, 60);  // TTL 1 минута
