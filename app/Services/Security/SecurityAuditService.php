...
final readonly class SecurityAuditService
{
    ...
    public static function runFullAudit(): array
    {
        ...
        $audit['completed_at'] = CarbonImmutable::now()->toDateTimeString();
        $audit['duration_seconds'] = round($duration, 2);

        // Вычисляем итоги
        $audit['summary'] = self::calculateSummary($audit);

        $this->logger->channel('security')->$this->logger->info('Security audit completed', [
            'audit_id' => $audit['id'],
    ...
    public static function getAuditHistory(int $limit = 10): array
    {
        return [
            'audits' => [
                [
                    'id' => 'audit_001',
                    'completed_at' => CarbonImmutable::now()->subDays(1)->toDateTimeString(),
                    'issues_found' => 2,
                    'security_score' => 92,
                ],
                [
                    'id' => 'audit_002',
                    'completed_at' => CarbonImmutable::now()->subDays(7)->toDateTimeString(),
                    'issues_found' => 5,
                    'security_score' => 88,
                ],
            ],
