...
final readonly class DatabaseIndexingService
{
    ...
    public static function createRecommendedIndexes(): array
    {
        ...
        $this->logger->channel('performance')->$this->logger->info('Indexes creation summary', [
            'created' => $created,
            'failed' => $failed,
            'errors' => $errors,
        ]);
    ...
    public static function createIndex(string $table, array $columns, bool $unique = false): bool
    {
        ...
        try {
            ...
            $this->logger->channel('performance')->$this->logger->info('Index created', [
                'table' => $table,
                'index' => $indexName,
                'columns' => $columns,
            ]);
    ...
    public static function dropUnusedIndexes(): array
    {
        ...
        foreach ($results as $row) {
            try {
                ...
                $this->logger->channel('performance')->$this->logger->info('Unused index dropped', [
    ...
    public static function getIndexCreationScript(): string
    {
        ...
        $script .= '-- Generated: '.CarbonImmutable::now()->toDateTimeString()."\n\n";
