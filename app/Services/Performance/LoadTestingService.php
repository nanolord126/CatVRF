...
final readonly class LoadTestingService
{
    ...
    public static function stressTest(
        ...
        $this->logger->channel('performance')->$this->logger->info('Load test started', [
            'url' => $url,
        ...
        for ($i = 0; $i < $requestsCount; $i++) {
            ...
            if (count($promises) >= $concurrency) {
                $this->http->pool(fn ($pool) => $promises);
                $promises = [];
            }
        }
        ...
        $this->logger->channel('performance')->$this->logger->info('Load test completed', $results);
