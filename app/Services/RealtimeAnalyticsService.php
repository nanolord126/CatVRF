...
final readonly class RealtimeAnalyticsService
{
    ...
    public function trackEvent(int $tenantId, string $eventType, array $data = []): void
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Analytics event tracked', [
                'tenant_id' => $tenantId,
    ...
    public function getRevenueChartData(int $tenantId, int $hours = 24): array
    {
        ...
        for ($i = $hours - 1; $i >= 0; $i--) {
            $hour = CarbonImmutable::now()->subHours($i);
    ...
    {
        $hour = CarbonImmutable::now()->format('Y-m-d-H');
        $events = [];
    ...
    public function aggregateDailyStats(int $tenantId, string $date): void
    {
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('Daily stats aggregated', [
                'tenant_id' => $tenantId,
                'date' => $date,
