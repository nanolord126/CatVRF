...
final readonly class SearchRankingService
{
    ...
    private function isNewUser(array $userProfile): bool
    {
        return ($userProfile['is_new'] ?? false) || ($userProfile['purchase_count'] ?? 0) < 3;
    }

    /**
     * Rank by popularity + rating (for new users)
     */
    private function rankByPopularity(array $items): array
    {
        $scored = [];
        ...
        foreach ($items as $item) {
            ...
            // Rating (0-5) → normalize to 0-100
            $score += ($item['rating'] ?? 0) * 20;

            // Purchase count (log scale)
            $score += log(($item['purchase_count'] ?? 1) + 1) * 10;

            // Review count
            $score += ($item['review_count'] ?? 0) * 2;
    ...
    /**
     * Rank mixed: 30% personalized, 70% popularity
     */
    private function rankMixed(array $items, float $personalizedWeight = 0.3): array
    {
        $popularity = $this->rankByPopularity($items);
        $personalizedWeight = min($personalizedWeight, 0.7); // Max 70% personalized
        ...
        while ($mixIndex < count($items)) {
            if (rand(0, 100) < ($personalizedWeight * 100)) {
                $result[] = $popularity[$personalizedIndex] ?? $popularity[0];
                $personalizedIndex++;
            } else {
                $result[] = $popularity[$mixIndex] ?? $popularity[0];
                $mixIndex++;
            }
        }
    ...
    /**
     * Rank by embeddings + behavior + geo (for regular users)
     */
    private function rankByEmbeddings(array $items, array $userProfile, array $context): array
    {
        $scored = [];
        ...
        foreach ($items as $item) {
            ...
            $embeddingSimilarity = $this->getEmbeddingSimilarity($item, $userProfile) ?? 0;
            $score += $embeddingSimilarity * 0.4;

            // 2. Category preference (30%)
            $categoryScore = $this->getCategoryScore($item, $userProfile);
            $score += $categoryScore * 0.3;

            // 3. Geographic relevance (15%)
            $geoScore = $this->getGeoScore($item, $userProfile);
            $score += $geoScore * 0.15;

            // 4. Popularity (15%)
            $popularityScore = ($item['rating'] ?? 0) * 20 + log(($item['purchase_count'] ?? 1) + 1);
            $score += ($popularityScore / 100) * 0.15;
    ...
    private function getEmbeddingSimilarity(array $item, array $userProfile): ?float
    {
        // In production: use pgvector or Typesense
        // For now, simplified version
        $itemEmbedding = $item['embedding'] ?? null;
        if (!$itemEmbedding) {
            return null;
        }

        $userEmbedding = Cache::get("user_embedding:{$userProfile['id']}")
            ?? $this->generateUserEmbedding($userProfile);

        // Calculate cosine similarity (0-1)
        return $this->cosineSimilarity($itemEmbedding, $userEmbedding);
    }

    private function getCategoryScore(array $item, array $userProfile): float
    {
        $preferredCategories = $userProfile['preferred_categories'] ?? [];
        $itemCategory = $item['category_id'] ?? null;

        if (!$itemCategory || !$preferredCategories) {
            return 0.5; // Neutral
        }

        return in_array($itemCategory, $preferredCategories, true) ? 1.0 : 0.3;
    }

    private function getGeoScore(array $item, array $userProfile): float
    {
        $userGeo = $userProfile['geo_location'] ?? null;
        $itemGeo = $item['geo_point'] ?? null;
        ...
        $distance = $this->haversineDistance($userGeo, $itemGeo);

        // If within 10km: 1.0, 100km: 0.3
        if ($distance <= 10) {
            return 1.0;
        } elseif ($distance >= 100) {
            return 0.3;
        ...
        return 1.0 - (($distance - 10) / 90) * 0.7;
    }

    private function getUserPreferences(int $userId): array
    {
        return Cache::remember("user_preferences:{$userId}", 86400, function () use ($userId) {
            return DB::table('orders')
                ->where('user_id', $userId)
                ->where('status', 'completed')
                ->select('category_id')
    ...
    private function generateUserEmbedding(array $userProfile): ?array
    {
    ...
    private function haversineDistance(string $geo1, string $geo2): float
    {
        // Parse geospatial points
        // Simplified: return distance in km
