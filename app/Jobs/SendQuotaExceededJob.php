...
final readonly class SendQuotaExceededJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! $user) {
            $this->log->warning('User not found for quota exceeded notification', [
                'user_id' => $this->userId,
        ...
        try {
            $this->mailer->to($user->email)->send(new QuotaExceededMail(
                $this->tenantId,
                $this->resourceType,
                $this->quotaData
            ));

            $this->log->$this->logger->info('Quota exceeded email sent', [
                'tenant_id' => $this->tenantId,
                'user_id' => $this->userId,
                'resource_type' => $this->resourceType,
            ]);
        } catch (\Throwable $e) {
            $this->log->error('Failed to send quota exceeded email', [
                'tenant_id' => $this->tenantId,
                'user_id' => $this->userId,
                'error' => $e->getMessage(),
            ]);
