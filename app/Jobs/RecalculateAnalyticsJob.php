...
final class RecalculateAnalyticsJob implements ShouldQueue
{
    ...
    public function handle(AdvancedAnalyticsService $analyticsService): void
    {
        ...
        try {
            $this->logger->channel('audit')->$this->logger->info('Analytics recalculation started', [
            ...
            $this->logger->channel('audit')->$this->logger->info('Analytics recalculation completed', [
                'correlation_id' => $correlationId,
                'tenant_id' => $this->tenantId,
            ]);
        } catch (\Exception $e) {
