...
final class DemandForecastJob implements ShouldQueue
{
    ...
    {
        $this->logger->channel('audit')->$this->logger->info('DemandForecastJob started', [
        ...
        try {
            ...
            $this->logger->channel('audit')->$this->logger->info('DemandForecastJob completed successfully', [
                'correlation_id' => $this->correlationId,
    ...
    {
        $this->logger->$this->logger->info('Training demand forecast model...');
        ...
        try {
            ...
            $version = CarbonImmutable::now()->format('Y-m-d').'-v'.($this->db->table('demand_model_versions')
            ...
            if ($metrics['mape'] < 15) {
                cache(['demand_model_active_version' => $version], CarbonImmutable::now()->addDays(30));

                $this->logger->$this->logger->info('New demand forecast model activated', [
                    'version' => $version,
                    'mape' => $metrics['mape'],
                ]);
            }
    ...
    private function forecastProduct(Product $product): void
    {
        ...
        for ($i = 1; $i <= 7; $i++) {
            $forecastDate = CarbonImmutable::now()->addDays($i)->format('Y-m-d');

            // Простейшая симуляция: базовый спрос + случайный шум + сезонность выходных
            $baseDemand = 10;
            $noise = rand(-3, 5);
            $isWeekend = in_array(CarbonImmutable::now()->addDays($i)->dayOfWeek, [0, 6], true);
